...
! function(a) {
    ...
    ! function() {
    ...
    }, "Letters only please"),/*custome validation method added by HEmant Thakur */a.validator.addMethod("lettersWithSpaceOnly", function(a, b) {
        return this.optional(b) || /^[a-z\s]+$/i.test(a)
    }, "Letters with Space only please"),a.validator.addMethod("validCompanyName", function(a, b) {
        return this.optional(b) || /^[a-zA-Z0-9&\-\s]+$/i.test(a)
    }, "Letters with Space only please"),a.validator.addMethod("alphanumericWithSlash", function(a, b) {
        return this.optional(b) || /^[a-zA-Z0-9\/]+$/i.test(a)
    }, "Letters, numbers, and forwardslash only please"),a.validator.addMethod("alphanumericWithSpace", function(a, b) {
        return this.optional(b) || /^[a-zA-Z0-9\s]+$/i.test(a)
    }, "Letters, numbers, and forwardslash only please"), a.validator.addMethod("validAddress", function(a, b) {
        return this.optional(b) || /^[a-zA-Z0-9#,./\-\s]+$/i.test(a)
    }, "Valid Address with # comma and - please"), a.validator.addMethod("finYear", function(a, b) {
        return this.optional(b) || /^\d{4}-\d{4}$/.test(a)
    }, "Valid FY like 2016-2017"),a.validator.addMethod("investmentDetail", function(a, b) {				var  investment_detail = $('input[name=investment_detail]:checked').val();
        var plant_value=$('.plant_value').val();if(plant_value!=''){		
        var ntrofunit=$('#ntrofunit').val();		
        var ntrofunittype=$('#ntrofunittype').val();
        var maxCost=0;var mixCost=0;
        if(ntrofunit=='Manufacturing'){ 
            if(ntrofunittype=='micro'){
                minCost=-1;
                maxCost=0.25;
            }
            if(ntrofunittype=='small'){
                maxCost=5;
                minCost=0.25;
            }
            if(ntrofunittype=='medium'){
                maxCost=10;
                minCost=5;
            }
            if(ntrofunittype=='large'){
                maxCost=-1;
                minCost=10;
            }
        }
        ...
        }	console.log(investment_detail);
        if(maxCost>0 && maxCost!=-1 && investment_detail!='In Lakh'){
            if((plant_value>maxCost || plant_value<minCost)){				console.log('a');
             return false
            }			console.log('b');
             return true;
         }
         else if(maxCost==-1 && investment_detail!='In Lakh'){
            if(plant_value<minCost){				console.log('c');
             return false
            }			console.log('d');
             return true;
         }else{			return true;		 }
    ...
    }, "Unit Type and Investment Details did not match"),a.validator.addMethod("rentedSpaceAttributesRequired", function(a, b) {
       var Proposed_details_of_Land = $('#Proposed_details_of_Land').val();
        if(Proposed_details_of_Land=='Rented Space'){
            if(a==''){
               return false;
            }
        }
        return true;
     },"This field is required"),a.validator.addMethod("otherLandAttributesRequired", function(a, b) {
       var Proposed_details_of_Land = $('#Proposed_details_of_Land').val();
        if(Proposed_details_of_Land!='Rented Space'){
            if(a==''){
               return false;
    ...
    /*End of custom validation method */a.validator.addMethod("letterswithbasicpunc", function(a, b) {
        return this.optional(b) || /^[a-z\-.,()'"\s]+$/i.test(a)
