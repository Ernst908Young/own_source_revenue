...
static function getTotalOverAllDistrictDICPendingSTATEDDNApps($dist_id=0){
   ...
   static function getTotalGrantIssues(){
		...
		$appCount1=$command->queryRow();
                ...
                $sql="SELECT count(*) as total FROM bo_new_application_submission WHERE  application_status in('A') AND user_id not in('11') ";
		$connection=Yii::app()->db; 
		$command=$connection->createCommand($sql);
		$appCount2=$command->queryRow();
                if(isset($appCount2) && !empty($appCount2) &&  isset($appCount2['total'])){
                 if($appCount1===false)
			return false;
                 else{
                     $total_approved_caf = $appCount1['total'] + $appCount2['total'];
                     return $total_approved_caf;
                 } 
