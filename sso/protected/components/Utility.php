...
class Utility{
    ...
    static function getlastinsertedid(){
        ...
        $sql = "SELECT lr_id FROM bo_landregion ORDER BY lr_id DESC LIMIT 1";
    ...
    static function getlastinsertedcid(){
        $stub = strtolower($stub);
        $sql = "SELECT id FROM bo_country ORDER BY id DESC LIMIT 1";
        $connection = Yii::app()->db;
        $command = $connection->createCommand($sql);
        //$command->bindParam(":stub", $stub, PDO::PARAM_STR);
        $info = $command->queryRow();
    ...
    static function addcountrystate($country,$state){
        $country =Utility::swcs_mysql_escape_string($country);
        $sql = "INSERT INTO bo_landregion(lr_name,lr_type) VALUES ('$country', 'country')";
        $connection = Yii::app()->db;
        $command = $connection->createCommand($sql);
        $status=$command->execute();
        if($status){
            $id=Utility::getlastinsertedid();
            $sql = "INSERT INTO bo_country(country_name,lr_id) VALUES ('$country', $id)";
            $connection = Yii::app()->db;
            $command = $connection->createCommand($sql);
            $status=$command->execute();
            $cid=Utility::getlastinsertedcid();
            if($cid){
            foreach ($state as $state) {
                $st =Utility::swcs_mysql_escape_string($state);
               $sql = "INSERT INTO bo_landregion(lr_name,lr_type) VALUES ('$st', 'state')";
               $connection = Yii::app()->db;
               $command = $connection->createCommand($sql);
               $status=$command->execute();
               if($status){
                   $sid=Utility::getlastinsertedid();
                   $sql = "INSERT INTO bo_state(state_name,lr_id,country_id) VALUES ('$st', $sid,$cid)";
                   $connection = Yii::app()->db;
                   $command = $connection->createCommand($sql);
                   $status=$command->execute();
               }
               else
                die("could not add state");
            }
        }
        }
        else
        die("could not add country");
     ...
     * @return true/false
     */
     static function verifyOtp($email,$mobile,$otp){
         $sql = "SELECT id FROM sso_manage_otp WHERE mobile_number=:mobile AND email=:email AND otp=:otp";
        // $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
        $connection = Yii::app()->db;
       
        $command = $connection->createCommand($sql);
        $command->bindParam(":mobile", $mobile, PDO::PARAM_INT);
        $command->bindParam(":email", $email, PDO::PARAM_STR);
        $command->bindParam(":otp", $otp, PDO::PARAM_INT);
        $info = $command->queryRow();
         if ($info === FALSE) {
            return FALSE;
        } else {
     ...
     static function deleteOtp($id){
         $sql = "DELETE FROM sso_manage_otp WHERE id=:id";
        // $connection = mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
        $connection = Yii::app()->db;
        $command = $connection->createCommand($sql);
    ...
    static function getMinisterInfo(){
        $sql = "SELECT homepage_minitster_image,homepage_minister_name,homepage_minister_readmore_link,homepage_minister_designation,homepage_minsiter_message FROM bo_manage_homepage WHERE is_active = '1'";
        $connection = Yii::app()->db;
        $command = $connection->createCommand($sql);
        $result = array();
        $rows = $command->queryAll();
    ...
    static function getProgressBarInfo(){
     ...
     /**
     * Function to email id is unique or not
     * auther: Hemant thakur
     * @param string
     * @return boolean
     */

        static function checkEmailID($email){
        $sql = "SELECT * FROM sso_users WHERE is_account_active='Y' AND  email = :email";
        $connection = Yii::app()->db;
        $command = $connection->createCommand($sql);
        $command->bindParam(":email", $email, PDO::PARAM_STR);
        $info = $command->queryRow();
    ...
    /**
     * Function to get to call CDN api via vurl
     * 
     * @param string
     * @return json
     * @auther Hemant Thakur
    ...
    static function getViaCurl($url,$action,$data=null){
         $post = curl_init();
         $url=$url.DIRECTORY_SEPARATOR.$action;
     ...
     /**
     * Function to send uiid via email
     * 
     * @param number,string
     * @return boolean
     * @auther Hemant Thakur
    ...
    static function sendUIIDViaEmail($uiid,$email){

        if(!empty($uiid) && !empty($email)){
            $hmac=hash_hmac("sha1",$uiid.$email,OTP_SECRET_KEY);
            $data=array("email"=>$email,'uiid'=>$uiid,'subject'=>'UUID Number','message'=>EMAIL_MESSAGE .$uiid,'hmac'=>$hmac);
            $status=Utility::getViaCurl(BO_API_BASEURL,'sendUIIDViaEmail',$data);   
            return $status;  
        }
        return FALSE;
     ...
     * 
     * @param email,string,number,string
     * @return boolean
     * @auther Hemant Thakur
     */
    
    static function sendCustomEmail($email,$sub,$uiid,$message){
        if(!empty($message) && !empty($uiid) && !empty($email)){
            $hmac=hash_hmac("sha1",$uiid.$email,OTP_SECRET_KEY);
            $data=array("email"=>$email,'uiid'=>$uiid,'subject'=>$sub,'message'=>$message,'hmac'=>$hmac);
            $status=Utility::getViaCurl(BO_API_BASEURL,'sendUIIDViaEmail',$data);   
            return $status;  
        }
        return FALSE;
