		...
		$(".slider").each(function () {
			...
			var opt = {
				...
				slide: function (event, ui) {
					...
					$el.find('.age').html(ui.value);
					$el.find('.height').html(ui.value);
				}
			};
			...
			if (range !== undefined) {
				...
				opt.slide = function (event, ui) {
					...
					$el.find('.age').html(ui.values[0] + " - " + ui.values[1]);
					$el.find(".age_min").html(ui.values[0]);
					$el.find(".age_max").html(ui.values[1]);
					
					$el.find('.height').html(ui.values[0] + " - " + ui.values[1]);
					$el.find(".height_min").html(ui.values[0] + "'");
					$el.find(".height_max").html(ui.values[1] + "'");
				};
			}
			...
			if (range !== undefined) {
				...
				$el.find('.age').html(val[0] + ' - ' + val[1]);
				$el.find(".age_min").html(val[0]);
				$el.find(".age_max").html(val[1]);
				
				$el.find('.height').html(val[0] + ' - ' + val[1]);
				$el.find(".height_min").html(val[0] + "'");
				$el.find(".height_max").html(val[1] + "'");
			} else {
				$el.find('.amount').html($el.slider('value'));
				$el.find('.age').html($el.slider('value'));
				$el.find('.height').html($el.slider('value'));
			}
		});
	}
