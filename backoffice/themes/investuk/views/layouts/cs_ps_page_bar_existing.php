...
$FY = isset($_GET['FY'])?$_GET['FY']:"ALL";
		
if( $FY!='ALL')
{
	// From Date To Date Conditions
	$dateArr = explode("-",$FY);
	$FY = $dateArr[0]."-".$dateArr[1];	
}
			...
			<table>
				...
				<tr>
				   <!--<tr id="date" style="display:<?php //echo $display1;?>">
					...
					<td>
					<b>Currently you are viewing data for FY "<?php echo $FY;?>", If you want to change then select Financial Year : &nbsp;</b></td>
					<td>						
						<?php 
						$pp = '2015';
						$yyy = '2019';
...
<?php } ?>	
	$('.fyu').on('change',function(){
		...
		var res = financial_year.split("-");
		
		var default_from_date  = "<?php echo $default_from_date ; ?>" ;
		var default_to_date    = "<?php echo $default_to_date ; ?>" ;
		
		if(res[0]=="ALL"){
			
		}else{
			default_from_date  = res[0]+"-04-01" ;
			default_to_date    = res[1]+"-03-31" ;
		}
		
		var actionURL = "../newReport/CafProposalReportISAExisting?from_date="+default_from_date+"&to_date="+default_to_date+"&reportType=admin" ; 
