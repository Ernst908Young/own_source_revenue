...
class ApplyServiceExt
{
	static function getDocumentsDataByID($doc_id){
		$sql = "SELECT dc.docchk_id,dc.chklist_id,dtm.name as document_type,im.name as issuer_name,ibm.name as issuerby_name,dc.name as document_name,im.issuer_id,ibm.issuerby_id,dtm.doc_id,dc.prescribed_form_format
	...
	static function getUploadedDocumentsDataByID_new($doc_id,$user_id){
		@session_start();
	...
	static function getUploadedDocumentsDataByID($doc_id){
		...
		$sql = "SELECT * FROM cdn_dms_documents WHERE docchk_id='$doc_id' AND is_document_active='Y' AND user_id='$user_id' AND (doc_status='U' || doc_status='V') ORDER BY mapping_id DESC LIMIT 1";
		$connection=Yii::app()->db; 
		$command=$connection->createCommand($sql);
		$res = $command->queryRow();
		//return $res;
