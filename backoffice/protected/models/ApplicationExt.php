...
/**
	...
	*@author: Mohit Sharma
	*/
	public static function getNICCodeMicroProject($divcode=0,$startDate="",$endDate=""){
		...
		$connection=Yii::app()->db; 
		 $sql="SELECT field_value from bo_application_submission where application_status in ('A','F','P') AND Submission_id not in('22','268') $dateCondition";
...
if( isset($totalmaleemp['industry_type']) && substr($totalmaleemp['industry_type'],0,2)==$divcode)
{
                                if(isset($totalmaleemp['invstmnt_in_total']) && $totalmaleemp['invstmnt_in_total'] !=''){
                                 // $total_investment = $total_investment +  $totalmaleemp['invstmnt_in_total'];
                                    foreach ($totalmaleemp['invstmnt_in_total'] as $value) {
                                    // Count of values start here
                                    if ($totalmaleemp['invstmnt_in_total'] == "invstmnt_in_total") {
                                        $total_investment += round($value, 2);
                                    } else {
		...
		$returnArray=array("small"=>$totalSmallCount,"medium"=>$totalMediumCount,"micro"=>$totalMicroCount,"large"=>$totalLargeCount,"investment"=>$total_investment);
		//echo "<pre>";print_r($returnArray);die;
		return $returnArray;
		...
		foreach ($Fields as $key => $field) {
			if(isset($field['field_value'])){
				...
				if(isset($totalcategory['org_category']) && $totalcategory['org_category']=='Other')
					$totalOther+=1;
                                if(isset($totalcategory['org_category']) && $totalcategory['org_category']=='')
					$totalna+=1;
			}
		}
		$returnArray=array("SC"=>$totalSC,"ST"=>$totalST,"OBC"=>$totalOBC,"GENERAL"=>$totalGeneral,"WOMEN"=>$totalWomen,"ExServiceman"=>$totalExserviceman,"PhysicallyChallenged"=>$totalPhyChal,"Other"=>$totalOther,"NA"=>$totalna);
		//echo "<pre>";print_r($returnArray);die;
		return $returnArray;
...
/**
	...
	*/
	...
	public static function getUsersCAFApplicationsOfUserExisting($uid,$app_id,$submission_id=null){
		$connection=Yii::app()->db;
		$status="I";
		$Hstatus='H';
		$sql="SELECT submission_id,dept_id,application_id,field_value,application_created_date,application_status from bo_application_submission WHERE (submission_id=:submission_id AND application_id=:app_id AND user_id=:uid) AND (application_status=:status OR application_status=:Hstatus) ORDER BY submission_id DESC";
		$command=$connection->createCommand($sql);
		$command->bindParam(":uid",$uid,PDO::PARAM_INT);
		$command->bindParam(":status",$status,PDO::PARAM_STR);
		$command->bindParam(":app_id",$app_id,PDO::PARAM_STR);
	...
	public static function checkUsersPrevCAFApplications($uid,$app_id){
		...
		if($applist===false)
			...
			foreach ($applist as $app) {
				if($app['application_status']=='P' OR $app['application_status']=='F' OR $app['application_status']=='AB')
...
where $cond bo_sp_applications.created_on  >=:startdate AND bo_sp_applications.created_on <=:enddate
 AND bo_sp_applications.user_id NOT IN ('11') AND sso_service_providers.is_service_provider_active='Y' ";
        ...
        if ($deptId == 'ALL') {  // || $distId=='ALL'
			...
			$sql_ser_cnt = "SELECT ap.app_id,ap.app_name FROM bo_sp_all_applications as ap INNER JOIN sso_service_providers as sp ON sp.sp_id=ap.sp_id WHERE ap.is_app_active = 'Y' AND sp.department_id>0";
...
/**
	...
	*@author: Mohit Sharma
	*/
	public static function getNICCodeMicroProject2($divcode=0,$fromDate,$toDate){
            
            $fromToDateCondition="";
             if(isset($fromDate)){
             $fromToDateCondition .= " AND DATE(application_created_date)>='".$fromDate."'";
            }
            // To Date
            if(isset($toDate)){
             $fromToDateCondition .= " AND DATE(application_created_date)<='".$toDate."'";
            }
            
            
		$connection=Yii::app()->db; 
		 $sql="SELECT field_value from bo_application_submission where application_status in ('A') AND user_id not in('11') $fromToDateCondition AND application_id=1";
		...
		foreach ($Fields as $key => $field) {
			if(isset($field['field_value'])){
				...
				if(isset($totalmaleemp['ntrofunittype']) && $totalmaleemp['ntrofunittype']=='large')
					$totalLargeCount+=1;
                                $inv=$inv+$totalmaleemp['invstmnt_in_total'][0];

                               
			}
		}
	}
		$returnArray=array("small"=>$totalSmallCount,"medium"=>$totalMediumCount,"micro"=>$totalMicroCount,"large"=>$totalLargeCount,'inv'=>$inv);
		//echo "<pre>";print_r($returnArray);die;
		return $returnArray;
...
/**
	...
	*@author: Rahul Kumar
	*/
	public static function getNICCodeEMPLOYMENT($divcode=0,$fromDate,$toDate){
		...
		foreach ($Fields as $key => $field) {
			if(isset($field['field_value'])){
				...
				if(isset($totalmaleemp['no_of_emp_mskilled'][0]))
					$totalSkilled+=$totalmaleemp['no_of_emp_mskilled'][0]+$totalmaleemp['no_of_emp_fskilled'][0];
                                
				if(isset($totalmaleemp['no_of_emp_munskilled'][0]))
					$totalUnskilled+=$totalmaleemp['no_of_emp_munskilled'][0]+$totalmaleemp['no_of_emp_funskilled'][0];
                                
                                
				if(isset($totalmaleemp['no_of_emp_msupervisory'][0]))
					$totalSuperwiser+=$totalmaleemp['no_of_emp_msupervisory'][0]+$totalmaleemp['no_of_emp_fsupervisory'][0];
                                
                                
				if(isset($totalmaleemp['no_of_emp_mengineer'][0]))
					$totalEngineer+=$totalmaleemp['no_of_emp_mengineer'][0]+$totalmaleemp['no_of_emp_fengineer'][0];
                                
                                
				if(isset($totalmaleemp['no_of_emp_it_mprofessional'][0]))
					$totalProfessional+=$totalmaleemp['no_of_emp_it_mprofessional'][0]+$totalmaleemp['no_of_emp_it_fprofessional'][0];
                                
                                
				if(isset($totalmaleemp['no_of_emp_mmanagement'][0]))
					$totalManagement+=$totalmaleemp['no_of_emp_mmanagement'][0]+$totalmaleemp['no_of_emp_fmanagement'][0];
		...
		$returnArray=array("totalSkilled"=>$totalSkilled,"totalUnskilled"=>$totalUnskilled,"totalSuperwiser"=>$totalSuperwiser,
                    "totalEngineer"=>$totalEngineer,"totalProfessional"=>$totalProfessional,"totalManagement"=>$totalManagement);
		//echo "<pre>";print_r($returnArray);die;
		return $returnArray;
	...
	//shishir 9july2018
	public static function getNatureOfOrganisationById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT name FROM bo_organization_nature_master WHERE id=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $organisation_name = $command->queryRow();
        return $organisation_name['name'];
    }
	//shishir 9july2018
	public static function getStateNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT state_name FROM lg_code_states WHERE state_code=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $state_name = $command->queryRow();
        return $state_name['state_name'];
    }
	//shishir 9july2018
	public static function getDistrictNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT distric_name FROM bo_district WHERE district_id=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $distric_name = $command->queryRow();
        return $distric_name['distric_name'];
    }
	//shishir 9july2018
	public static function getSubDistrictNameCodeById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT sub_district_name FROM lg_code_sub_disctrct WHERE sub_district_code=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $sub_district_name = $command->queryRow();
        return $sub_district_name['sub_district_name'];
    }
	//shishir 9july2018
	public static function getSubDistrictNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT block_name FROM lg_code_blocks WHERE id=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $block_name = $command->queryRow();
        return $block_name['block_name'];
    }
	//shishir 9july2018
	public static function getVillageNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT village_name FROM lg_code_villages WHERE village_code=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $village_name = $command->queryRow();
        return $village_name['village_name'];
    }
	//shishir 9july2018
	public static function getIndustrialAreaNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT industrial_area FROM bo_industrial_area_master WHERE id=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $industrial_area = $command->queryRow();
        return $industrial_area['industrial_area'];
    }
	//shishir 10july2018
	public static function getDepartMentNameById($id) {
        ...
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $depart_name = $command->queryRow();
        return $depart_name['name'];
    }
	//shishir 10july2018
	public static function getApprovalNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT core_service_name FROM bo_information_wizard_service_parameters WHERE service_id=:id";
        $command = $connection->createCommand($sql);
        $command->bindParam(":id", $id, PDO::PARAM_INT);
        $approval_name = $command->queryRow();
        return $approval_name['core_service_name'];
    }
    ...
    public static function getServiceNameById($id) {
        $connection = Yii::app()->db;
        $sql = "SELECT core_service_name FROM bo_information_wizard_service_parameters WHERE CONCAT(service_id,'.',servicetype_additionalsubservice)=$id";
	...
	public static function getApprovalNameById2($service_id) {
        $connection = Yii::app()->db;
        $sql = "SELECT core_service_name FROM bo_information_wizard_service_parameters WHERE CONCAT(service_id, ',', servicetype_additionalsubservice)=$service_id";
        $command = $connection->createCommand($sql);        
        $approval_name = $command->queryRow();
        return $approval_name['core_service_name'];
    }
		
	public static function getRevertedCommentofNewCaf($id){
		$connection=Yii::app()->db;
		$sql="SELECT department_comment FROM bo_infowiz_form_builder_application_log WHERE app_Sub_id=:id and action_status in ('V','R','H','F')";
		$command=$connection->createCommand($sql);
		$command->bindParam(":id",$id,PDO::PARAM_INT);
		$appname=$command->queryRow();
		return $appname['department_comment'];
	}
        
        public static function getNewApplicationInfo($app_sub_id){
		// echo $app_sub_id;die;
        ...
        public static function getNewCafTracking($caf_id=0){
		$connection=Yii::app()->db; 
		 $sql="SELECT * FROM bo_infowiz_form_builder_application_log WHERE service_id = '591.0' and app_Sub_id=:caf_id AND action_status not in ('I','DP','PD') ORDER BY id ASC";
	...
	public static function getNewCafForwardedtoDept($user_id)
	{
			$deptUserData = Yii::app()->db->createCommand("SELECT bo_user.*,bo_user_role_mapping.role_id,bo_user_role_mapping.department_id FROM bo_user LEFT JOIN bo_user_role_mapping on bo_user_role_mapping.user_id=bo_user.uid where uid ='$user_id'")->queryRow();
			//Merge caf 2.0 data from here
			/* echo "<pre>";
			print_r($_SESSION); */
			$role_id = $deptUserData['role_id'];
			$dept_id = $deptUserData['dept_id'];
			...
			$sql="SELECT bo_infowiz_formbuilder_application_forward_level.*,
					...
					LEFT JOIN bo_departments on
					bo_departments.infowiz_short_code = bo_infowizard_issuerby_master.abb						
					where  
