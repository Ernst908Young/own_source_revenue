<?php
/**
* this file is used to view the forwarded application of the departments with all the comments of other departments
				...
				if($finalStatus){
					if(!empty($rolesAccess)){
						...
						if(isset($rolesAccess['access_id']) && $rolesAccess['access_id']==1)
							echo "&nbsp;<a href='#' id='verify_app' class='btn btn-primary'>Move On</a>";
						...
						if(isset($rolesAccess['access_id']) && $rolesAccess['access_id']==3)
							echo "&nbsp;<a data-toggle='modal' class='fields_properties_modal btn btn-info' href='#forward_to_multi_dept'> Step to Departments</a>";
						if(isset($rolesAccess['access_id']) && $rolesAccess['access_id']==4)
							echo "&nbsp;<a href='#' id='hold_app' class='btn btn-default'>Revert Back to Investor </a>";
	   ...
	   if(sub_id!=''){
		...
		$("#reject_app").click(function(e){
			...
			if(sub_id!=''){
				...
				var url="<?php echo Yii::app()->createUrl('/frontuser/ajax/forwardApplicationStatusReject');?>";
				var app_id="<?php echo $data['application_id'];?>";
				var data={"app_id":app_id,"sub_id":sub_id,"comments":comments};
				jQuery.ajax({
	            url: url,
