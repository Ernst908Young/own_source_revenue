 ...
 <table cellpadding="2" cellspacing="0" border="0" class="gridtable">
     ...
     <th colspan="6">Process Description</th></tr>
      <tr>
         ...
         <td colspan="1"><?php if(!empty($fields->type_of_industry)){ echo $fields->type_of_industry; }?></td>
         <td colspan="2"> Expected date of production</td>
         <td colspan="1"><?php if(!empty($fields->expected_date_of_commercial_production)){ echo $fields->expected_date_of_commercial_production; }?></td>
      </tr>
     <tr>
       <td colspan="3">Brief Description about processes</td>
       <td colspan="3"><?php if(isset($fields->Brief_Description_about_Processes)){ echo htmlspecialchars($fields->Brief_Description_about_Processes); 
           }?></td>
       }
     </tr>
   </table> 
     ...
     <tr>
      ...
      if(isset($fields->Name_of_the_Department)){
         foreach ($fields->Name_of_the_Department as $key => $value) {
            if(empty($fields->Name_of_the_Department[$key]))
                                       continue;
          ?>
      <tr>
        ...
        if(!empty($app_comments)){
          ...
          foreach ($app_comments as $key => $comment) {
            echo "<tr>
              ...
              echo "</td>
              <td colspan='6'>".@$comment['approver_comments']."</td>
            </tr>";
            $count++;
          }
        }
     ...
     */
     ...
     if(!empty($otheDeptCmnt)){
        ...
        foreach ($otheDeptCmnt as $key => $value) {
           ...
           echo "<td colspan='2'>$dept_name[department_name]</td>";
           echo "<td>";
           if(preg_match('/_/', $uname))
              echo ucwords(str_replace('_', ' ', $uname));
          else{
          	  echo ucwords($uname);
          }
          	echo "</td>";

           //echo "<tr><td colspan='1'>".$count++."</td><td colspan='2'>$uname</td><td colspan='6'>";
          	echo "<td colspan='5'>";
           if(empty($value['verifier_user_comment'])){
              echo "No comments yet</td>";
           }
           else{
