...
$pd_records = Yii::app()->db->createCommand("SELECT tbl_payment.* , p.core_service_name
    from tbl_payment 
...
ON sm.id = tbl_payment.service_id
    where payment_mode=3 $wheresc AND payment_status='pending' and p.is_active='Y' ORDER BY tbl_payment.submission_id DESC")->queryAll(); 
...
<div class="applied-status">
  <div class="row">
    <div id="tickettab" class="my-5">
        <ul>
            <li><a href="#pa" id="palink">Payment Due</a></li>
...
<div id="sample_1_wrapper" class="dataTables_wrappers dataTables_wrapper">
    <table  id="sample_1" width="100%">
       <thead> 
           ...
           <?php
        if($pd_records){
            foreach($pd_records as $key => $value) 
            {   
              $formName= '';
          ...
          if(isset($form_id) && !empty($form_id) && $form_id==1)
          {
            ...
            <tr class="ticket-row tableinside" id="<?php echo $key; ?>">
               ...
               <td class="text-center">
                 <?= ($value['payment_status']=='pending'?'Payment Pending':'') ?>
               </td>
               <td class="text-center">
                   <a href='/backoffice/cashier/default/makepayment/p_id/<?= $value['id'] ?>' style='color:blue;'>
                        View   
                   </a>
               </td>       
           </tr>                                    
