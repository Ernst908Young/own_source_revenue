...
class UploadDocumentsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','uploadFiles','create','update','admin','delete','uploadSSODeptFiles'),
	...
	private function uploadDocument($ssoApp=true){
		...
		if($ssoApp)
			$response=json_decode(DefaultUtility::postViaCurl(CDN_APIV1.'/SaveInvestorSSODocuments',$post_data));
		else
	...
	*/
	public function actionUploadFiles(){
		...
		if(isset($_GET['app_submission_id'])){
			...
			$this->render('uploadFiles',array('model'=>$model,"ssoDeptApp"=>false));			
		}
		else{
			Yii::app()->user->setFlash('Error', "Invalid request.");
			$this->redirect(array('/admin/uploadDocuments'));
	...
	public function actionUploadSSODeptFiles(){
		...
		if(isset($_GET['app_submission_id'])){
			...
			$this->render('uploadFiles',array('model'=>$model,"ssoDeptApp"=>true));			
		}
		else{
			Yii::app()->user->setFlash('Error', "Invalid request.");
			$this->redirect(array('/admin/uploadDocuments'));
