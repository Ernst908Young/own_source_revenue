	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','view','create','update','admin','delete','viewAllUsers','viewAllUsers1','downloadUserDetail','downloadUserDetailPDF','viewAllUsers3','getInvestors','getDeptInvestors','getDeptInvestors2'),
				'expression'=>'Utility::isSuperAdmin()',
			),
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('ViewAllApplications','create','DownloadAllApplications','viewAllApplicationsComments','ViewAllApplicationsComments1','downloadApplicationComments','getDistrictApplications','ViewFilteredApplications','viewAllUsers','viewAllUsers1','viewAllUsers3','getInvestors','getDeptInvestors','getDeptInvestors2','downloadUserDetail','downloadUserDetailPDF','sendCustomNotifications'),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('ViewAllApplications','create','DownloadAllApplications','viewAllApplicationsComments','downloadApplicationComments','getDistrictApplications','ViewFilteredApplications','viewAllUsers','viewAllUsers1','downloadUserDetail','downloadUserDetailPDF','sendCustomNotifications','viewAllUsers3','getInvestors','getDeptInvestors','getDeptInvestors2'),
...
WHERE su.is_account_active='Y' group by su.iuid";	
	...
	* 
	*/
	public function actionViewAllApplicationsComments1(){
		/*
		get all the submitted application
		*/
		/*$criteria=new CDbCriteria;
		...
		$criteria->params=array(":incomplete"=>"I");*/
		$apps=ApplicationSubmission::model()->findAll();
		$this->render('allApplicationsComments1',array("applications"=>$apps));
	}
	/** 
...
LEFT JOIN bo_infowizard_issuerby_master on bo_infowizard_issuerby_master.issuerby_id=bo_dept_service_application.infowiz_dept_id
where is_active='Y' AND ( iuid =$userID OR sw_user_id=$userID) group by bo_dept_service_application.infowiz_service_name having tot_service >1 order by departmentName)")->queryAll();
 ...
 where  bo_sp_applications.user_id=$userID AND bo_departments.dept_id NOT IN (select dept_id from bo_departments where infowiz_short_code IN(select abb from bo_infowizard_issuerby_master 
			...
			foreach ($data as $row) {
				...
				if(!empty($result_caf)){
					...
					foreach($result_caf as $key=>$val)
					{
						...
						if(!empty($val['application_status']) && $val['application_status']=='P')
						{
							$CAF.= '<p>CAF ID: '.@$val['submission_id'].', Pending</p>';
						}	
						if(!empty($val['application_status']) && $val['application_status']=='A')
						{
							$CAF.= '<p>CAF ID: '.@$val['submission_id'].', Approved</p>';
...
where issuerby_id IN (select distinct infowiz_dept_id from bo_dept_service_application))) group by serviceName having tot_service>0 order by departmentName")->queryAll();
 ...
 FROM bo_sp_applications WHERE user_id =$userID and sp_app_id = '280' group by serviceName having tot_service>1;")->queryAll();
	...
	{		
		...
		$fromToCondition = "  AND DATE(application_created_on)>='" . @$from_date . "' AND DATE(application_created_on)<='" . @$to_date . "' ";	
...
where issuerby_id IN (select distinct infowiz_dept_id from bo_dept_service_application)))")->queryAll();
        ...
        public function actionViewfields(){
		$this->render('fields');
	}
