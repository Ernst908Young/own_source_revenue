...
class SpApplicationHistory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('sp_app_id', 'numerical', 'integerOnly'=>true),
			array('sp_tag, app_id', 'length', 'max'=>250),
			array('application_status, approver_id, next_approver', 'length', 'max'=>200),
			array('comments, sent_dated_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('history_id, sp_app_id, sp_tag, app_id, application_status, comments, approver_id, next_approver, added_date_time, sent_dated_time', 'safe', 'on'=>'search'),
		);
	}
