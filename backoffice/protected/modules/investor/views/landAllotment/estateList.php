...
<?php if (empty($_SESSION['RESPONSE']['user_id'])) {  ?>
                    ...
                    <ul class="page-breadcrumb">
                        <li><?php $url_now =$_SERVER['REQUEST_URI']?>
                            <span class="pull-left"><a href="/backoffice/frontuser/home/serviceNew" title="Go to Departmental Services : New" class="fa fa-home homeredirect" ></a><b> <?php if(!empty(@$_SESSION['RESPONSE']['user_id']) || (isset($_GET['panel']) && $_GET['panel'] = 'investor')) 
																...
																elseif(strpos($url_now, 'landAllotment/estateList') !== false){
																	echo "Search Industrial Estate Plots(MSME)";
																}
																else{
																	if(isset($iuid) && $iuid != '')
...
<div class='portlet-title'>
    <div class='caption'>
        <i style=" font-size:20px;" class='fa fa-list'></i>List of available Estate</div>
    <div class='tools'> </div>
</div>
          ...
          <table class="table table-striped table-bordered table-hover" id="sample_2" >
            ...
            <?php
              if(empty($estateList)){
                ...
                $count=1;
                foreach ($estateList as $key => $estateDetail) {
                    ?>
                    <tr>
                <td align="center"><?=$count++;?></td>
                  <td><?=$estateDetail['distric_name'];?></td>
		<td><?=$estateDetail['land_estate_name'];?></td>
              <td><?=$estateDetail['open_till'];?></td>
               </tr>
                 <?php
               }
                }
...
?>
     <div class="row text-center" ><a href="<?=@Yii::app()->createAbsoluteUrl('/frontuser/landAllotment/stepOne');?>" class="btn btn-primary">Apply For Land Allotment</a> </div>
</div>
</div>
