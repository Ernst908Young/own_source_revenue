...
<?php
   ...
   $srn_no = $resAppData['submission_id'];
   ?>
<style>
    ...
    foreach ($decode_data['ParsedResults'] as $value) {
               ...
               foreach ($val as $v) {
                  ...
                  echo '<br>';
                   ...
                   if(in_array('Permanent Account Number', $v)){
                        ...
                        echo 'Invalid Pan Card. Please upload valid document';
                      }
                       echo '<br><br>';
      ...
      <section class="panel site-min-height dmsSkip">
         <div class="panel-body" >
            <div style="text-align:justify;padding:20px;color:red;font-size:16px;font-weight:bold;">
               <?php //$phase2_service_id = [19,20,21,22]; 
                  ...
                  echo "Note*: In case you want to provide any supporting documents for the application then please attach the same under this section.";
                  if($resAppData['service_id']=='36.0'){
                    echo " The instrument must be adjudicated before uploading.";
                  }
          ...
          <div class="form-row row">
            <div class="col-lg-6 form-group">
                <?php 
                  $doc_status_arr = ['U'=>'Pending / Approved','R'=>'Rejected'];              
                ?>
                <label>Document Status</label>
                <select name="doc_status_filter" id="doc_status_filter" class="form-control" onchange="applydocfilter($(this).val())">
                    <?php foreach ($doc_status_arr as $key => $value) { 
                      $selected_option = $key == @$_GET['doc_status'] ? "selected" : '';
                      ?>
                      <option value="<?= $key ?>" <?= $selected_option ?> ><?= $value ?></option>
                    <?php } ?>                      
                </select>  
            </div>
          </div>
                     ...
                     <?php
                        ...
                        if (count($mapped_documents_array) > 0) {
                          ...
                          foreach ($mapped_documents_array as $key => $val) { 
                              ...
                              // below code return document master records
                                $doc_datas = ApplyServiceExt::getDocumentsDataByID($val['doc_id']);
                                //  below code return records from bo_application_dms_documents_mapping
                                $dms_datas = ApplyServiceExt::getUploadedDocumentsDataByID($caf_id,$val['doc_id'],@$_GET['doc_status']);
                                if(@$dms_datas['status']=='R'){
                                        ...
                                        <div class="form form-horizontal" role="form">
                                              ...
                                              <div class="form form-horizontal" role="form">
                                                 ...
                                                 <td class="cent vln" width="35%"> 
                                                   <?php  if (!empty($dms_datas)) {  echo  $dms_datas['usercomment'];} ?>
                                                 </td>
                                              </div>
                                              <td class="vln" width="25%" style="padding-left: 25px;">
                                                 <div class="row">
                                                    ...
                                                    <div class="col-lg-12 text-left">
                                                       ...
                                                       <a href="/panchayatiraj/backoffice/doc/mydoc?view=<?php echo DefaultUtility::getDockey($dms_datas['document_file_name']); ?>" target="_blank"  title="Click to see or download uploaded document" style="float: left;  margin-top: 25px; margin-left: 10px;"><img src="/themes/investuk/assets/applicant/images/print-icon.png"></a>
                                                       <?php  } ?>
                                                    </div>
                  ...
                  <tbody>
                     <?php
                        ...
                        if (count($mapped_documents_array) > 0) {
                          ...
                          $doc_check_on_services = [2,41,29,31,27,37];
                          if(in_array($res_s['service_id'], $doc_check_on_services)){
                           $mapped_documents_array = Documentchecklist::Isdoc_required($res_s['service_id'],$caf_id,$mapped_documents_array);
            // print_r($mapped_documents_array);die;
                          ...
                          foreach ($mapped_documents_array as $key => $val) { 
                              ...
                              // below code return document master records
                                $doc_datas = ApplyServiceExt::getDocumentsDataByID($val['doc_id']);
                                //  below code return records from bo_application_dms_documents_mapping
                                $dms_datas = ApplyServiceExt::getUploadedDocumentsDataByID($caf_id,$val['doc_id'],@$_GET['doc_status']);// print_r($dms_datas);die;?>
         ...
         <?php } ?>
            ...
            <form action="/panchayatiraj/backoffice/investor/ApplyService/<?php echo $actionUrl; ?>" method="POST" id="skipDms">
            ...
            <?php 
               ...
               if($flag==1){
               ?>
            <input type="hidden" id="issig" value="0">
            <br>
            <!--div class="row">                        
               <div class="col-lg-12">
            ...
            <div class="d-flex justify-content-center my-4" align="center">
               ...
               <?php
                  ...
                  ?>            
               <a class="btn btn-primary" href="/panchayatiraj/backoffice/infowizardtwo/<?php echo $spArr['form_action_controller'];?>/updateSubForm/service_id/<?php echo $serviceID;?>/pageID/1/subID/<?php echo $_GET['caf_id'];?>/formCodeID/1/stype/old">Back</a>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
   ...
   if(issig==0){                
    $.ajax({
        type: "POST",
        dataType : 'JSON',
        url: "/panchayatiraj/backoffice/investor/services/SaveAllDocuments/sno/" + sno + "/service_id/" + service_id + "/dept_id/" + dept_id + "/app_id/" + app_id,
        success: function (data) {  
             var srn_no = "<?= base64_encode($srn_no) ?>";
             if(data.is_payment_done==true){
               window.location.href = "/panchayatiraj/backoffice/investor/home/investorWalkthrough";
             }else{
               window.location.href = "/panchayatiraj/backoffice/investor/services/payment/srn_no/" + srn_no;
             }
        }
    });
   }
    ...
    // if(es.checked){
        $.ajax({
            ...
            url: "/panchayatiraj/backoffice/investor/services/SaveAllDocuments/sno/" + sno + "/service_id/" + service_id + "/dept_id/" + dept_id + "/app_id/" + app_id,
            success: function (data) {                                                  
                window.location.href = "/panchayatiraj/backoffice/investor/services/signatory/srn_no/"+sno+"/dept_id/"+dept_id;
            }
        });
    // }else{
    ...
    // }
       ...
       $(".newdmsuploadwithmappingofreferencenvalidity").click(function () {
           $.ajax({
               type: "POST",
               url: "/panchayatiraj/backoffice/frontuser/DocumentManagement/index/doc_id/" + $(this).attr('doc_id') + "/issuer_id/" + $(this).attr('issuer_id') + "/issued_by/" + $(this).attr('issued_by') + "/doc_code/" + $(this).attr('doc_code') + "/mydoc_status/active",
               success: function (data) {
                   $('.dms-body').html(data);
               }
           });
...
<script type="text/javascript">
   ...
   function applydocfilter(doc_status){
        
        location.href = location.href + "&doc_status=" + doc_status;    
    }
</script>