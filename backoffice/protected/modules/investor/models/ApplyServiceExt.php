...
class ApplyServiceExt
{
	...
	static function getUploadedDocumentsDataByID($srn_no,$doc_id,$doc_status){
		@session_start();
		//$user_id = $_SESSION['RESPONSE']['user_id'];	
		if($doc_status=='R'){
			$sql = "SELECT * FROM bo_application_dms_documents_mapping WHERE sno=$srn_no AND doc_id=$doc_id AND ( status='R') order by mapping_id desc";
		}else{
			$sql = "SELECT * FROM bo_application_dms_documents_mapping WHERE sno=$srn_no AND doc_id=$doc_id AND (status='U' || status='V') order by mapping_id desc";
		}
