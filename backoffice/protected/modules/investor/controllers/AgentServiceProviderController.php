<?php
 /**
    ...
    * @param:
    **/
class ApplyServiceCPController extends Controller
{
    ...
    **/
	public function actionServiceListing(){
		...
		$sql_dep = "SELECT m.* FROM bo_infowizard_issuerby_master as m INNER JOIN bo_information_wizard_service_master as sm ON sm.issuerby_id=m.issuerby_id WHERE m.is_issuerby_active='Y' AND m.issuer_id='2' AND sm.to_be_used_in_construction_permit='Y' GROUP BY m.issuerby_id ORDER BY m.name ASC";
		$connection=Yii::app()->db; 
		$command=$connection->createCommand($sql_dep);
		$res_dep = $command->queryAll();
		
		$base=Yii::app()->theme->baseUrl;
		$res_d=false;
		if(isset($_GET['department_id']) && $_GET['department_id']>0){
			$department_id=$_GET['department_id'];
			$sql_d = "SELECT sm.*,sp.* FROM bo_information_wizard_service_master as sm  
					  INNER JOIN bo_information_wizard_service_parameters as sp ON sp.service_id=sm.id                         
					  INNER JOIN bo_information_wizard_pre_service_mapping as psm ON psm.service_id=CONCAT(sp.service_id,'.',sp.servicetype_additionalsubservice)                         
					  WHERE to_be_used_in_construction_permit='Y' AND sm.issuerby_id='$department_id' ORDER BY sm.service_name ASC";
			$connection=Yii::app()->db; 
		...
		$department_id=false;
		$caf_id=false;
		if(isset($_GET['final_service_id']) && $_GET['final_service_id']>0){
			$final_service_id=$_GET['final_service_id'];
			$department_id=$_GET['department_id'];
			list($service_id,$sub_service_id) = explode(".",$final_service_id);
			$caf_id=$_GET['caf_id'];
			$sql_s="SELECT * FROM bo_information_wizard_pre_service_mapping as psm  
				  INNER JOIN bo_information_wizard_service_parameters as sp ON sp.service_id='$service_id' AND sp.servicetype_additionalsubservice='$sub_service_id'                       
				  WHERE psm.service_id='$final_service_id' AND psm.status='Y' ORDER BY psm.id DESC LIMIT 1";
			$connection=Yii::app()->db; 
		...
		$this->render("service_listing",array('res_dep'=>$res_dep,'res_d'=>$res_d,'res_caf'=>$res_caf,'res_s'=>$res_s,'id'=>$final_service_id,'caf_id'=>$caf_id,'user_id'=>$user_id,'department_id'=>$department_id));
	}
	
	public function actionGetServiceListingAjax($id=0){
		$html='<select name="final_service_id" class="form-control" required><option value="">Select Service</option>';
		if($id>0){
			$sql_d = "SELECT sm.*,sp.* FROM bo_information_wizard_service_master as sm  
				  INNER JOIN bo_information_wizard_service_parameters as sp ON sp.service_id=sm.id                         
				  INNER JOIN bo_information_wizard_pre_service_mapping as psm ON psm.service_id=CONCAT(sp.service_id,'.',sp.servicetype_additionalsubservice)                         
				  WHERE to_be_used_in_construction_permit='Y' AND sm.issuerby_id='$id' ORDER BY sm.service_name ASC";
			$connection=Yii::app()->db; 
			$command=$connection->createCommand($sql_d);
			$res_d = $command->queryAll();
			if($res_d){
				foreach($res_d as $dep_arr){ 
					$service_id 	= $dep_arr['service_id'];
					$sub_service_id = $dep_arr['servicetype_additionalsubservice'];
					$final_id = $service_id.".".$sub_service_id;
					$html .= '<option value="'.$final_id.'">'.$final_id.' - '.$dep_arr['core_service_name'].'</option>';
				}
			}
		}
		$html.='</select>';
	...
	public function actionDocumentsChecklist(){
		...
		$this->render("document_checklist",array('mapped_documents_array'=>$mapped_documents_array,'iuid'=>$iuid));
	}
			...
			if(isset($swcs_service_id) && $swcs_service_id>0){
				...
				if(!empty($serviceDatas)){
					...
					if(isset($swcs_department_id)){
							...
							$sql_d = "SELECT service_provider_name FROM sso_service_providers WHERE sp_id='$swcs_department_id'";
							$connection=Yii::app()->db; 
							$command=$connection->createCommand($sql_d);
							$res_d = $command->queryRow();
							if(!empty($res_d)){
								$department_name=$res_d['service_provider_name'];
							}
					}
					...
					$sp_tag = 'Pollution&*%_679#';
					$spModel = new SpApplications;
					$spModel->sp_tag	= $sp_tag;
					$spModel->sp_app_id	= $swcs_service_id;
					$spModel->app_id	= time();
					$spModel->app_name	= $app_name;
					$spModel->app_status	= 'I';
					$spModel->app_comments	= NULL;
					if(isset($caf_id)){
						$spModel->is_applied_by_caf	= 'Y';
						$spModel->caf_id	= $caf_id;
					}
					$user_id = $_SESSION['RESPONSE']['user_id'];
					$spModel->reverted_call_back_url	= '#';
					$spModel->user_id	= $_SESSION['RESPONSE']['user_id'];
					$spModel->created_on	= date("Y-m-d H:i:s");
					$spModel->updated_on	= date("Y-m-d H:i:s");
					$spModel->is_active	= 'Y';
					$remote_ip 	= $_SERVER['REMOTE_ADDR'];
					$user_agent = $_SERVER['HTTP_USER_AGENT'];
					$spModel->remote_server	= $remote_ip;
					$spModel->user_agent	= $user_agent;
					...
					if($spModel->save()){
						...
						if(isset($dms_string)){
									if($dms_string!=''){
										...
										$documents_arr = explode("::",$dms_string); 
					...
					//echo '<hr><pre>';print_r($spModel->attributes);
					//echo '<hr><pre>';print_r($_POST);
					//echo '<hr><pre>';print_r($data); die;
				}
    ...
    * @param:
    **/
	public function actionServiceListingDuplicate(){
		@session_start();
		if(!Yii::app()->user->isGuest){
			throw new CHttpException(400, "you can't access this page");
		}
        ...
        public function actionServiceListingNoc(){
		@session_start();
		if(!Yii::app()->user->isGuest){
			throw new CHttpException(400, "you can't access this page");
		}
		...
		$this->render("service_listing_with_noc_upload",array('res_dep'=>$res_dep,'res_d'=>$res_d,'res_caf'=>$res_caf,'res_s'=>$res_s,'id'=>$final_service_id,'caf_id'=>$caf_id,'user_id'=>$user_id,'department_id'=>$department_id));
	}
