        ...
        <ul class="breadcrumb">
          <li><a href="/backoffice/admin">Home</a></li>
           <li>Reports</li>
            ...
            <li>Entity Summary Report Level 2</li>
          </ul>
...
<div class="form-part bussiness-det">                          
        ...
        <div class="form-row">
		 <?php $entity = Yii::app()->db->createCommand("SELECT cd.company_name as entity_name from  bo_new_application_submission as nas left join bo_company_details cd on cd.reg_no = nas.entity_name where nas.entity_name = '".$reg_no."'")->queryRow(); ?>
          ...
          From Date: <strong><?= $from_date ?></strong> | To Date: <strong><?= $to_date ?></strong>  | Entity Name: <strong><?= $entity['entity_name']; ?></strong>
        
        <?php 


            $pdfurl = "/backoffice/misreports/entity/level2pdf?from_date=$from_date&to_date=$to_date&regno=$reg_no";
...
if(count($records)>0){
  ...
  <tr class="ticket-row tableinside" id="<?php echo $key; ?>">
    ...
    <td class="text-center">
      <p><a href="/backoffice/misreports/entity/level3/type/service/otd/reports?regno=<?= $reg_no;?>&from_date=<?php echo $from_date;?>&to_date=<?php echo $to_date; ?>&service_id=<?= $value['service_id'] ?>" style='color:blue;'><?= $value['service_name'] ?></a></p>
    </td>
     <td class="text-center">
