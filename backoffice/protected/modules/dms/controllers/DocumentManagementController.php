...
class DocumentManagementController extends Controller
{
	...
	public function actionCheckDuplicateDoc(){
		...
		if($iuid>0){
			$sql1="SELECT * FROM cdn_dms_documents WHERE docchk_id='$doc_id' AND iuid='$iuid' AND is_document_active='R'";
			$connection=Yii::app()->db; 
			$command=$connection->createCommand($sql1);
			...
			if(count($doc_ver)>0){
				...
				if(count($doc_ver)>0){
					...
					if($mv == 'Y'){
						...
						$return_msg['response']='FAILED';
						$return_msg['response_msg']='Document already uploaded in your documents list.222';
					}
