...
class DocumentManagement2Controller extends Controller
{
	...
	public function actionMyDocuments(){
		...
		// Get all uploaded list of documents by Investors which are active and ready to use for departments
			...
			$command=$connection->createCommand($sql);
			$documents_array = $command->queryAll();
                        foreach($documents_array as $key=>$doc_array){
                        if(!empty($doc_array)){
                            $doc_id = $doc_array['documents_id'];
                            
                        $sql="SELECT * FROM  bo_application_dms_documents_mapping dm  LEFT JOIN 
                                bo_application_dms_documents_mapping_logs dl ON dm.mapping_id= dl.mapping_id WHERE dm.documents_id = $doc_id order by dl.created_time desc limit 1";
			$connection=Yii::app()->db; 
			$command=$connection->createCommand($sql);
			$documents_array_draft = $command->queryAll();
                        if(!empty($documents_array_draft)){
                        $documents_array[$key]['is_draft'] = $documents_array_draft[0]['is_draft'];}else $documents_array[$key]['is_draft'] = 0;
                       ...
                       /* if(!empty($documents_array)){
                        ...
                        $sql="SELECT * FROM  bo_application_dms_documents_mapping dm  LEFT JOIN 
                                bo_application_dms_documents_mapping_logs dl ON dm.mapping_id= dl.mapping_id WHERE dm.documents_id =$documents_array[0]['documents_id'] order by dl.created_time desc limit 1";
			$connection=Yii::app()->db; 
			$command=$connection->createCommand($sql);
			$documents_array_1 = $command->queryAll();
                        }*/
			//$documents_array = 1;
		// END
		
		$this->render('dms_listing',array('documents_data'=>$documents_array,'draft_array'=>$documents_array_draft));
	}
	...
	public function actionGetDocumentLogs____OLD(){
		@extract($_POST);
		if($document_id){
...
";
	...
	public function actionGetDocumentLogs(){
		@extract($_REQUEST);
		if($document_id){
			$sql="SELECT *,dms_map.status as c_status,dms_map_log.verifier_name as verified_by FROM bo_application_dms_documents_mapping as dms_map
LEFT JOIN bo_application_dms_documents_mapping_logs as dms_map_log ON dms_map_log.mapping_id=dms_map.mapping_id
INNER JOIN bo_departments as d ON d.dept_id=dms_map.dept_id
INNER JOIN bo_sp_applications as sp_ap ON sp_ap.sno=dms_map.sno
WHERE dms_map.documents_id='$document_id'
";
			$connection=Yii::app()->db; 
			$command=$connection->createCommand($sql);
			$doc_ver=$command->queryAll();
                      //  echo "<!--<pre>";                        print_r($doc_ver); echo "<pre>-->";
			if(count($doc_ver)>0){
				$dcccc_value['N'] = "Pending";
				$dcccc_value['V'] = "Verified";
				$dcccc_value['R'] = "Rejected";
				
				$sqldccc = "SELECT * FROM cdn_dms_documents WHERE documents_id='$document_id'";
				$connection=Yii::app()->db; 
				$command=$connection->createCommand($sqldccc);
				$dcccc=$command->queryRow();
				$dcccc_value_key = @$dcccc['vbi'];
				
				echo '<p style="font-weight:bold; text-align:left;">Current version of document has been verified by the issuer: '.$dcccc_value[$dcccc_value_key].'</p>';
				echo '<style>.table > caption + thead > tr:first-child > td, .table > caption + thead > tr:first-child > th, .table > colgroup + thead > tr:first-child > td, .table > colgroup + thead > tr:first-child > th, .table > thead:first-child > tr:first-child > td, .table > thead:first-child > tr:first-child > th {
   vertical-align:middle; !important}</style><table class="table table-striped table-bordered" width="100%"><thead><th>S.No</th><th>Application ID</th><th>Service Name</th><th>Department Name</th><th>CAF ID</th><th>Unit Name</th><th>Used Date</th><th>Status</th><th>Verified By</th><th>Verified On</th><th>Verifier Comment</th></thead>';
				$i=1;
				foreach($doc_ver as $doc_arr){
					if($doc_arr['c_status'] == 'V' && $doc_arr['verifier_comments']==''){
						$comments = "Verified";
					...
					?>
                                        <?php $dstatus=@$doc_arr['c_status'];$listOfDocument=array('V'=>'Verified','U'=>'Unverified','R'=>'Rejected','M'=>'Mismatch'); ?>
					<tr>
                                            <td style="text-align:center;"> <?php echo $i; ?></td>
						<td><?php echo $doc_arr['app_id']; ?></td>
						<td><?php echo $doc_arr['app_name']; ?></td>
						<td><?php echo $doc_arr['department_name']; ?></td>
                                                <td><?php if($doc_arr['caf_id']==0){echo "NA";}else{echo $doc_arr['caf_id'];}; ?></td>
						<td><?php echo $doc_arr['unit_name']; ?></td>
						<td><?php echo $doc_arr['created_on']; ?></td>
						<td><?php echo @$listOfDocument[$dstatus]; ?></td>
						<td><?php echo $doc_arr['verified_by']; ?></td>
						<td><?php echo $doc_arr['created_time']; ?></td>
						<td><?php echo $comments; ?></td>
					</tr>
					<?php
