...
class LandrequesterConnectController extends Controller {
    ...
    public function actionView($id) {
        $sql = "SELECT * FROM bo_landowner_requester_connect  WHERE id=$id ";
    ...
    public function actionCreate() {
        @session_start();
        $model = new LandrequesterConnect;
      //  echo "hi..";die;
        // Uncomment the following line if AJAX validation is needed
        $this->performAjaxValidation($model);
        $guestUserUrl = "";

        if (isset($_POST['LandrequesterConnect'])) {
            $model->attributes = $_POST['LandrequesterConnect'];
			
			/* Author:Pankaj Kumar Tiwari
			   Date:16Feb2018  */
			   
			$model->village = !empty($_POST['LandrequesterConnect']['village'])?$_POST['LandrequesterConnect']['village']:'';
            ...
            $model->ip            = $ip;
            if ($model->save(false)) {
                $url = "/backoffice/iloc/landrequesterContact/create/landID/" . base64_encode($model->id) . $guestUserUrl;
                $this->redirect($url);
            }
        }
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['LandrequesterConnect'])) {
            ...
            //  echo "<pre/>";
              //print_r($model);die;
                $requestedProperty = LandownerConnectEXT::isLandRequesterContactAvailable(base64_encode($id));
                $urlPostFix = "/backoffice/iloc/landrequesterContact/create/landID/" . base64_encode($id) . $guestUserUrl;
                if (!empty($requestedProperty)) {
                    $urlPostFix = "/backoffice/iloc/landrequesterContact/update/id/" . base64_encode($id) . "/landID/" . base64_encode($id) . $guestUserUrl;
                }
    ...
    public function actionIndex() {
        ...
        if (isset($_SESSION['RESPONSE']['land_user_id']) && !empty($_SESSION['land_user_id'])) {
            $application = LandownerConnectEXT::getLandownerConnectListbyLandUserID();
        }
         // Added By : Rahul [30012018]
     ...
     * @return LandrequesterConnect the loaded model
     * @throws CHttpException
     */
     // jitendra singh
    public function loadModel($id) {
        $model = LandrequesterConnect::model()->findByPk($id);
        if ($model === null)
     ...
     * @param LandrequesterConnect $model the model to be validated
     */
    ...
    public function actionLocationData() {
        ...
        $allActiveProperties = $connection->createCommand("UPDATE bo_landowner_requester_connect SET latlong='$latlong' WHERE id=$landID")->execute();
 ...
 //End of adding 16012018
            ...
            $updatedProperties = $connection->createCommand("UPDATE bo_landowner_requester_connect SET status='$propertyAvailable',share_contact='$ShareContact' WHERE id=$landID")->execute();
