...
/**
 * This is the model class for table "bo_landowner_connect_log".
 *
 * The followings are the available columns in table 'bo_landowner_connect_log':
 * @property integer $id
 ...
 */
class LandownerConnectLog extends CActiveRecord
{
	/**
	...
	{
		return 'bo_landowner_connect_log';
	}
	...
	public function rules()
	{
		...
		return array(
			array('land_title, district_id, address, sub_district_id, keshra_khatian, type_of_land, distance_highway, name_highway, distance_airport, name_airport, distance_railway, name_railway, area_sqmt, existing_loan, comment, created_date', 'required'),
			array('is_sale, is_lease, district_id, sub_district_id, dept_user_id, land_id', 'numerical', 'integerOnly'=>true),
			array('user_id', 'length', 'max'=>10),
			array('land_title, area_type, latlong, user_agent', 'length', 'max'=>255),
			array('existing_loan, status, share_contact', 'length', 'max'=>1),
			array('modified_date', 'safe'),
			array('remote_ip', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, land_id, land_title, is_sale, is_lease, district_id, address, sub_district_id, village, keshra_khatian, type_of_land, distance_highway, name_highway, distance_airport, name_airport, distance_railway, name_railway, area_sqmt, area_type, existing_loan, comment, status, share_contact, latlong, dept_user_id, created_date, modified_date', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return LandownerConnectLog the static model class
	 */
