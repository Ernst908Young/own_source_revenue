                        ...
                        <li>
                            <span class="pull-left"><a href="/backoffice/frontuser/home/investorWalkthrough" title="Go to Dashboard : Level 1" class="fa fa-home homeredirect" ></a><b> <?php 
																if(!empty(@$_SESSION['RESPONSE']['user_id']) || (isset($_GET['panel']) && $_GET['panel'] = 'investor')) {
																	echo "Apply For Sectoral Clearances(Beta)";
																}
...
if(!empty($preReques)){
    ...
    foreach($allServiceRequiredBeforApply as $particularService){
       ...
       //$docShouldBe.=",".$preRequesCertificateDocument['doc_checklist_id'];
		...
		if(!empty($preRequesCertificateDocument['doc_checklist_id']))
				...
				<th>Action</th>
							...
							<td width="10%">
								...
								if(!empty($dms_datas)){
									...
									<input name="FileUpload[YII_CSRF_TOKEN]" type="hidden" value="<?php echo Yii::app()->request->csrfToken; ?>">
		...
		<tbody>
			<?php 
					...
					$disabled_btn=false;		
							...
							<?php if(($disabled_btn == false && $caf_id>0) || 1==1){ 
