...
class DocumentManagementController extends Controller
{
		...
		if(isset($_POST['FileUpload']['YII_CSRF_TOKEN'])){
			...
			if (isset($_FILES) && !empty($_FILES['dms_doc_uploads']['name'])){
				...
				if($file_size>25000){
					...
					$doc_date_of_issuance 	= @$_POST['FileUpload']['doc_date_of_issuance'];
					$my_doc_status='R';
					...
					if($doc_chk_arr == false){
						...
						$model->doc_date_of_issuance= @$doc_date_of_issuance;
	...
	public function actionCheckDuplicateDoc(){
		...
		if($iuid>0){
                    // Mismatch Case -- 22Feb2019 ;// Rahul 
                    $sql2="SELECT documents_id,docchk_id,doc_status FROM cdn_dms_documents WHERE iuid='$iuid' AND  docchk_id='$doc_id' ORDER BY documents_id DESC  LIMIT 1";
                   echo $sql2; die;
                    $connection=Yii::app()->db; 
			$command=$connection->createCommand($sql2);
			$doc_verN=$command->queryRow();
                        if($doc_verN['doc_status']=="M"){
                            $mv="Y";
                        }
                       // End of Mismatch Case -- 22Feb2019 ;// 
