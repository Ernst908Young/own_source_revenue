...
class ExistingUnitController extends Controller
    ...
    */
    ...
    private function submitApplication($app_sub_id,$status='P',$updated_date=false,$landrigion_id=null){
        ...
        if($model->save()){
            ...
            $sql="SELECT iw.chklist_id as code,iw.name as document_name,dms.document_name as file_name,dms.doc_status as document_status,iwdt.name as document_type,iwdt.abbr as document_type_code

								FROM cdn_dms_documents as dms

								INNER JOIN bo_infowizard_documentchklist as iw ON iw.docchk_id = dms.docchk_id

								INNER JOIN bo_infowizard_docunenttype_master as iwdt ON iwdt.doc_id=dms.doc_type_id

								WHERE dms.user_id=1049 AND dms.iuid=52796497 AND dms.is_document_active='Y' AND (dms.doc_status='U' OR dms.doc_status='V')";
    ...
    public function actionUploadInvestorDocs(){
      ...
      if($this->isInvestorLoggedIn()){
        ...
        if(!isset($_FILES['caf_applications_uploads']['tmp_name']) || empty($_FILES['caf_applications_uploads']['tmp_name'])){
          ...
          $this->redirect(Yii::app()->createAbsoluteUrl('frontuser/existingUnit/cafForm'));
        ...
        if($post['selected_doc_type']=='image/jpeg'){
          ...
          if(!in_array($_FILES['caf_applications_uploads']['type'], $jpgArray)){
            ...
            $this->redirect(Yii::app()->createAbsoluteUrl('frontuser/existingUnit/cafForm/documentUpload/1'));
    ...
    public function actionSaveCAFPartially(){
       ...
       if($this->isInvestorLoggedIn()){
          ...
          if($applicationTableFlag==1){
              ...
              $modelSpApplications->app_name="Existing CAF";            

              $modelSpApplications->app_status="I";

              $modelSpApplications->app_comments="Existing CAF Application Submitted Partialy";

              $modelSpApplications->app_distt=1;   //$caf->landrigion_id        

              $modelSpApplications->user_id=$_SESSION['RESPONSE']['user_id'];
			  
			  $modelSpApplications->reverted_call_back_url="/backoffice/frontuser/existingUnit/cafForm";
...
//              }
              ...
              //  $next_role_id=$this->getNextRoleId(json_decode($caf->field_value));
                ...
                if($revertedBack){
                   ...
                   // print_r($verification);  echo "===============||";  print_r($caf);die;
					
                    if($verification->save()){
                  
                          $serviceApplicationUpdateQuery="UPDATE bo_sp_applications SET app_comments='Existing CAF Application Submited',app_status='P' WHERE app_id=$caf->submission_id; AND sp_tag='DOI@908#123'";
                             ...
                             foreach ($_FILES['caf_applications_uploads']['tmp_name'] as $key => $file_content) {
                                 ...
                                 if (isset($_POST['selected_doc_type'][$key]) && empty($_POST['selected_doc_type'][$key])) {
                                     ...
                                     $this->redirect(array(
                                         ...
                                         '/frontuser/existingUnit/cafForm'
                                 ...
                                 if ($file_size < $doc_min_size || $file_size > $doc_max_size) {
                                     ...
                                     $this->redirect(array('/frontuser/existingUnit/cafForm'));
...
//session_start();
      //  session_start();
// Change it when making for any other Service (Service ID)
        $serviceExisting = "484.0";

                                        $serviceID = explode(".", $serviceExisting);
$userID=$_SESSION['RESPONSE']['user_id'];
                                         $sql1 = "SELECT document_checklist_creation FROM bo_information_wizard_service_parameters  WHERE service_id=$serviceID[0] AND  servicetype_additionalsubservice=$serviceID[1]  AND is_active='Y' ORDER BY created DESC";
										//echo "<hr>";
										...
										//echo $sql2."<hr>"; die;
										$allFechedData=Yii::app()->db->createCommand($sql2)->queryRow();
										if(!empty($allFechedData)){
                                        $docsInfo[] = $allFechedData;
										}
	...
	$totalMappedDocument=0;
                                     ...
                                     // echo $userID;  print_r($docsInfo);die;
...
// print_r($docsInfo);die;
  ...
  where bo_information_wizard_service_parameters.servicetype_additionalsubservice and bo_information_wizard_service_master.issuerby_id=:Depname";
