...
//Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M')
			...
			if($this->page == 1){
				$this->Cell(0, 0, 'Form 13', 0, 1, 'R', 0, '', 0);
			}
  ...
  <td width="20%"> </td>
			...
			if(isset($_GET['is_vpd'])){
				...
				$data.='<hr /><table class="tbl">
			            ...
			            <tr>
			            <th >SRN Number Of Name Reservation Form: '.$this->reg_no.'</th></tr>
			          </table>'; 
			}
...
class Utility22_0{
	...
	public static function generatePdfApp($content,$contentform3=null,$contentform6=null,$contentform19=null,$name,$app_log,$signatory_details,$fieldValues2){ 
		...
		$reg_no = 'NA';
		if($fieldValues2){
			if(isset($fieldValues2['UK-FCL-00360_0'])){
				if($fieldValues2['UK-FCL-00360_0']!=''){
					$reg_no = $fieldValues2['UK-FCL-00360_0'];
				}				
			}
		}
		...
		if(!empty($contentform3)){
			$pdf->AddPage('','NEXT-ODD','','','','','','','','','','','','','',-1,-1,-1,-1);
			$pdf->writeHTML($contentform3, true, 0, 0, 0);
		}
		if(!empty($contentform6)){
			$pdf->AddPage('','NEXT-ODD','','','','','','','','','','','','','',-1,-1,-1,-1);
			$pdf->writeHTML($contentform6, true, 0, 0, 0);
		}
		if(!empty($contentform19)){
			$pdf->AddPage('','NEXT-ODD','','','','','','','','','','','','','',-1,-1,-1,-1);
			$pdf->writeHTML($contentform19, true, 0, 0, 0);
		}
