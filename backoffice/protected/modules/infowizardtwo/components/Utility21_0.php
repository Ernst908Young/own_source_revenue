...
//Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M')
			...
			if($this->page == 1){
				$this->Cell(0, 0, 'Form 17', 0, 1, 'R', 0, '', 0);
			}
  ...
  <td width="20%"> </td>
			...
			if(isset($_GET['is_vpd'])){
				...
				$data.='<hr /><table class="tbl">
			            ...
			            <tr>
			            <th >SRN Number Of Name Reservation: '.$this->reg_no.'</th></tr>
			          </table>'; 
			}
...
class Utility21_0{
	...
	public static function generatePdfApp($content,$contentform4,$contentform9,$name,$app_log,$fieldValues2){ 
		...
		$reg_no = 'NA';
		if($fieldValues2){
			if(isset($fieldValues2['UK-FCL-00331_0'])){
				if($fieldValues2['UK-FCL-00331_0']!=''){
					$reg_no = $fieldValues2['UK-FCL-00331_0'];
				}				
			}
		}
		...
		$pdf->writeHTML($content, true, 0, 0, 0);
          ...
          //echo $content;echo $contentform4;
		//echo $contentform9;die;
		$pdf->AddPage();
		$pdf->writeHTML($contentform4, true, 0, 0, 0);
		//$pdf->Cell(0, 10, $pdf->writeHTML($contentform4), 0, 1, 'L');

		$pdf->AddPage();
		$pdf->writeHTML($contentform9, true, 0, 0, 0);
		//$pdf->Cell(0, 10, $pdf->writeHTML($contentform9), 0, 1, 'L');
