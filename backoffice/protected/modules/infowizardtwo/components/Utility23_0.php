  ...
  <td width="20%"> </td>
			...
			if(isset($_GET['is_vpd'])){
				...
				$data.='<hr /><table class="tbl">
			            ...
			            <tr>
			            <th >Society Number: '.$this->reg_no.'</th></tr>
			          </table>'; 
			}
...
class Utility23_0{
	...
	public static function generatePdfApp($content,$contentform3, $contentform6, $contentform19,$name, $app_log,$fieldValues2){ 
		...
		$reg_no = 'NA';
		if($fieldValues2){
			if(isset($fieldValues2['UK-FCL-00290_0'])){
				if($fieldValues2['UK-FCL-00290_0']!=''){
					
					$reg_no = $fieldValues2['UK-FCL-00290_0'];
				}				
			}
		}
		...
		$pdf->Image($image_file2, 17.5, 4.7, 2, '', '', '', 'T', false, 150, '', false, false, 0, false, false, false);
		$pdf->SetY(3);
		}else{
		...
		if($contentform3!=NULL){
			$pdf->AddPage();
			$pdf->writeHTML($contentform3);
		}
		if($contentform6!=NULL){
			$pdf->AddPage();
			$pdf->writeHTML($contentform6);
		}
		if($contentform19!=NULL){
			$pdf->AddPage();
			$pdf->writeHTML($contentform19);
		}
