...
class SubFormRegistrationofExternalCompaniesController extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationofExternalCompanies/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$maxid = $cdmaxid;
 ...
 $country1 = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00347_0'])->queryRow(); 

       $state = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00385_0'])->queryRow(); 

      $fulladdress = $farr['UK-FCL-00340_0'].' '.$farr['UK-FCL-00341_0'].' '.$farr['UK-FCL-00344_0'].' '.$farr['UK-FCL-00346_0'].' '. $state['lr_name'].' '.$country1['lr_name'];   
        ...
        $companyDetail->address = $fulladdress;
         ...
         $companyDetail->state_parish_id = $farr['UK-FCL-00385_0'];
         $companyDetail->postal_code_id = '';
         $companyDetail->state_parish = @$state['lr_name'];
         $companyDetail->postal_code = $farr['UK-FCL-00346_0'];
         $companyDetail->country = @$country1['lr_name'];
        $companyDetail->save();
...
/*Yii::app()->db->createCommand("INSERT INTO bo_company_details (srn_no, name_related_srn, service_id, company_type, reg_no,company_name,user_id,approved_by,address)
                VALUES ($asrn, $name_related_srn, $aser_id, 'EC', '".$maxid."', '".$c_name."', $auser_id, $loginuserid,'".$fulladdress."')")->execute()*/
                // end of aamir code

                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '8.0') {
                    $data = '';
                    $path = '';
                    $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate8_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility8_0c";
...
// End Code of service provider             
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationofExternalCompanies/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormRegistrationofExternalCompanies/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        $content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
         $contentform_30pdf = $this->renderPartial('form_30pdf', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        ...
        //echo $content; die;
        Utility8_0::generatePdfApp($content,$contentform_30pdf , $name, $app_log);
         exit;
        }else{
...
public function actionGetCompanyNameBySrnNo(){
     $srn_no = DefaultUtility::dataSenetize($_GET['srn_no']);
      ...
      /* echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>'IT Test', 'timeremain'=>20,'msg'=>'success'));
       exit;*/
            ...
            if($chk_srn_is_used){
                ...
                if($name_res){
                    ...
                    if($company_detail){
                        ...
                        if($checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name reservation for new Company" && $checkformtype['UK-FCL-00031_0']=='1'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Name Reservation for External Company can only be used.'));
                        }                                   
                    }else{         
