...
class SubFormApplicationforExemptionForm34Controller extends Controller {
        ...
        */
        ...
        if (isset($allRes) && !empty($allRes)) {
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormApplicationforExemptionForm34/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
//End Entity Code     
            ...
            if ($model->save()) {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormApplicationforExemptionForm34/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
                $print_app_call_back_url = "/backoffice/infowizardtwo/subFormApplicationforExemptionForm34/downloadNewApp/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
...
// jitendra singh
   ...
   public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
       ...
       //echo $content; die;
       Utility31_0::generatePdfApp($content,$name,$app_log,$fieldValues2);
       exit;
        }else{
    ...
    public function actionGetCompanyNameBySrnNo() {
        ...
        $user_id = @$_SESSION['RESPONSE']['user_id'];
        //$user_id = 11;
        $comp_detal = Yii::app()->db->createCommand("SELECT * FROM bo_company_details WHERE user_id = $user_id AND is_active=1 AND company_type IN ('IC') AND reg_no='" . $reg_no . "' AND service_id IN('4.0') ")->queryRow();
        if ($user_id) {
            if ($comp_detal)
