...
class FormFieldMasterOtherServiceController extends Controller
{
	/**
    ...
    public function actionGetFormHtml(){		
		...
		if(!empty($_GET['app_Sub_id']) && !empty($_GET['service_id']) && !empty($_GET['form_id']))
		{
			...
			if($_GET['table']=='Department'){
				/*  $sql = "SELECT a.*,b.department_name,c.role_name  FROM 				bo_infowiz_formbuilder_application_forward_level a 
					JOIN bo_departments b ON b.dept_id = a.forwarded_dept_id
					JOIN bo_roles c ON c.role_id = a.next_role_id 
					WHERE  appr_lvl_id=$idds"; */
					$sql = "SELECT a.*,b.department_name,c.role_name,bo_new_application_submission.application_status   FROM bo_infowiz_formbuilder_application_forward_level a 
					JOIN bo_departments b ON b.dept_id = a.forwarded_dept_id
					JOIN bo_roles c ON c.role_id = a.next_role_id 
					...
					$command = $connection->createCommand($sql);
			 		...
			 		if($data['post_info'] && $data['approv_status'] !='V'  ||  $app_status=='PF'){
						...
						$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_other_service', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}		 		
						...
						$fieldValues2 = (array)json_decode($fieldValues['field_value']);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_other_service', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$fieldValues2,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}
				...
				$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
	 			...
	 			$this->renderPartial('/formBuilder/subformloggerview_other_service', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'no','selectedDept'=>$selectedDept));
	 		}
			else{
