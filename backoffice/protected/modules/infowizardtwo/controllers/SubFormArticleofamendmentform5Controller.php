...
class SubFormArticleofamendmentform5Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticleofamendmentform5/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                $c_name =  $_POST['UK-FCL-00089_0'];
                $maxid =  $_POST['UK-FCL-00403_0'];
                ...
                $c_type = 'AF5'; // Amendment Form 5 type
...
if($farr['UK-FCL-00346_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00340_0'].' '.@$farr['UK-FCL-00341_0'].' '.@$farr['UK-FCL-00344_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00347_0'] ; */
        ...
        $companyDetail->company_type = 'AF5';
...
/*Yii::app()->db->createCommand("INSERT INTO bo_company_details (srn_no, name_related_srn, service_id, company_type, reg_no,company_name,user_id,approved_by,address)
                ...
                // end of aamir code
	// if($farr['UK-FCL-00502_0']=='Yes'){
 //        $sql = "SELECT * FROM bo_company_details WHERE reg_no='".$farr['UK-FCL-00403_0']."'";
 //        $updateCompany =  BoCompanyDetails::model()->findBySql($sql);
 //        $updateCompany->company_name = $farr['UK-FCL-00507_0'];
 //        $updateCompany->save();
	// }
                ...
                $d_url = "";
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate20_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility20_0c";
...
// Code By Aamir for Service provider
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticleofamendmentform5/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormArticleofamendmentform5/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
			...
			$content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'getApprovedName'=>$getApprovedName, 'application_created_date' => $fieldValues['application_created_date']), true);
       
			$name = "CAIPO-Application_Form_" . time() . ".pdf";

			Utility20_0::generatePdfApp($content, $name, $app_log,$fieldValues2);
            exit;
        }else{
   ...
   public function actionGetcompanyNameByregno(){
		...
		$c_detail = Yii::app()->db->createCommand("SELECT A.*,B.* from bo_company_details A
            ...
            WHERE A.company_type IN ('IC','NPC') AND A.user_id = $user_id  AND A.reg_no='".$reg_no."'  AND A.service_id IN('4.0','5.0') ")->queryRow();
        ...
        if($c_detail){  
				...
				if($check_already_exist){
                   ...
                   // print_r($check_already_exist['service_id']);die;
