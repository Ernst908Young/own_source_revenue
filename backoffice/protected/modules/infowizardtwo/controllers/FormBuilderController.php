        ...
        if (!empty($_GET['service_id'])) {
			...
			if(isset($user_id) && !empty($user_id))
			{
				...
				if(!empty($resultData) && (($resultData['application_status']=='I') || ($resultData['application_status']=='DP'))){
					$this->redirect("/backoffice/infowizardtwo/subForm/updateSubForm/service_id/$_GET[service_id]/pageID/$_GET[pageID]/subID/$resultData[submission_id]/formCodeID/$_GET[formCodeID]");
				}
				...
				if(!empty($resultData) && $resultData['application_status']=='PD'){
					...
					$this->redirect("/backoffice/infowizardtwo/payment/paymentRedirect/sno/$sno");
				}
			}
	...
	public function actionSubform() {
        ...
        if (!empty($_GET['service_id'])) {
			...
			if(isset($user_id) && !empty($user_id))
			{
				...
				$sql = "SELECT * FROM bo_new_application_submission where service_id=:service_id  AND application_status IN ('I','PD','DP') AND user_id=:user_id";
				$connection = Yii::app()->db;
				$command = $connection->createCommand($sql);
				$command->bindParam(':service_id', $service_id, PDO::PARAM_STR);
				$command->bindParam(':user_id', $user_id, PDO::PARAM_STR);
				$resultData = $command->queryRow();
                // echo "<pre>";print_r($resultData);die;
					...
					if(!empty($resultData) && (($resultData['application_status']=='I') || ($resultData['application_status']=='DP'))){
						...
						$this->redirect("/panchayatiraj/backoffice/infowizardtwo/$ser_controller_action_path/updateSubForm/service_id/$_GET[service_id]/pageID/$_GET[pageID]/subID/$resultData[submission_id]/formCodeID/$_GET[formCodeID]");
					}			
					/* else if(!empty($resultData) && $resultData['application_status']=='PD')
					{						
						$chechpaymentoffline = Yii::app()->db->createCommand("SELECT * FROM tbl_payment where service_id=$_GET[service_id] AND payment_submit_by=$user_id AND payment_mode=3 AND submission_id = $resultData[submission_id] AND payment_status = 'pending' order by id DESC")->queryAll();	
						if($chechpaymentoffline)
						{
							$this->redirect('/backoffice/investor/home/investorWalkthroughLevel2/type/SERVICES/financial_year/ALL');
						}else{		
								$this->redirect("/backoffice/investor/services/payment/srn_no/$resultData[submission_id]"); 
							}
					} */
			} 
			...
			extract($_GET);
			$formData_new=$this->alignInSequence($service_id,$formCodeID);
			$appStatus = "";
			$field_value = "";
        }
		//echo "<pre>";
		//print_r($formData_new);
        //die(); 
  
            $default_form_data = @$_SESSION['applicant_data_from_pdf'][$_GET['service_id']];
       

        $this->render('subform', array('aap' => $allActivePages, 'formData' => $formData_new,'serviceID'=>$_GET['service_id'],'issuer_id'=>@$_POST['issuer_id'],'field_value'=>$field_value,'postedData'=>@$_POST,'default_form_data'=>$default_form_data));
    }
...
//        $formData = $command->queryAll();
     ...
     * @return infowizardtwoFormvariableMaster the loaded model
    ...
    public function actionResetData(){
        if(isset($_GET['submission_id'])){
            $submission_id = base64_decode($_GET['submission_id']);
             ...
             $allow_reset_status_arr = ['DP','SP','PD','H'];

             if(in_array($model->application_status, $allow_reset_status_arr)){
                    $data = json_decode($model->field_value);
                     $maindata = [];
                     $seledata = [];

                     if($model->service_id=='2.0'){                
                         foreach ($data as $key => $value) {
                            $matchword = substr($key,0,6);
                            if($key != 'UK-FCL-00044_0' && ($matchword=='UK-FCL' || $matchword=='checkb')){
                                $seledata[$key] = "";
                            }else{
                                 $maindata[$key] = $value;
                            }
                         }            
                         $finalarray = json_encode(array_merge($maindata, $seledata));
             ...
             $this->redirect($model->reverted_call_back_url);
            
        }
    }

    /*public function actionDeleterowonupdate(){
        if(isset($_POST['submission_id'])){
             ...
             $model = NewApplicationSubmission::model()->findBySql($sql);
              $data = json_decode($model->field_value);
             $i =  $_POST['index'];
             foreach ($_POST['fieldscode'] as $key => $value) {      
                $inarray = $data->$value;
                 $data->$value = array();
                foreach ($inarray as $k => $val) {                   
                    if($k==$i){                      
                    }else{
                        $data->$value[] = $val;
                    }
