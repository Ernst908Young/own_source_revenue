<?php
class SubFormManagementProxyCirculareForm11Controller extends Controller
{
        ...
        */
        ...
        if (isset($allRes) && !empty($allRes))
        {
            ...
            if ($_POST['app_status'] == 'H')
            {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormManagementProxyCirculareForm11/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
    ...
    public function actionSaveData()
    {
            ...
            ->isAjaxRequest)
        {
            ...
            if ($model->save())
            {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/SubFormManagementProxyCirculareForm11/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
                $print_app_call_back_url = "/backoffice/infowizardtwo/SubFormManagementProxyCirculareForm11/downloadNewApp/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
    ...
    public function actionDownloadNewApp()
    {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
            ...
            $fieldValues = Yii::app()
                ...
                ->createCommand("SELECT * FROM bo_new_application_submission where submission_id = $subID")->queryRow();
            $fieldValues2 = (array)json_decode($fieldValues['field_value']);
            $sig_detail = Yii::app()->db->createCommand("SELECT * FROM bo_signature_metadata where submission_id = $subID AND is_active=1")->queryAll();
        

        $content = $this->renderPartial('company_name_pdf_view', array(
            'aap' => $allActivePages,
            'formData' => $formData,
            'fieldValues' => $fieldValues2,
        ...
        //echo $content; die;
        Utility27_0::generatePdfApp($content, $name, $app_log,$fieldValues2);
         exit;
        }else{
    ...
    //     }
    // }
    public function actionGetNameByregno()
    {
        ...
        $c_detail = Yii::app()
            ->db
            ->createCommand("SELECT * from bo_company_details WHERE company_type IN ('IC') AND user_id = $user_id  AND reg_no='" . $reg_no . "'")->queryRow();
        if ($c_detail)
        {
        ...
        else
        {
            echo CJavaScript::jsonEncode(array(
                'status' => false,
                'msg' => 'Please enter a correct and valid Company Number'
            ));
        }
    ...
    public function actionGetCompanyNameBySrnNo(){
       ...
       if($user_id){
        if($subid==NULL){
            ...
            if($chk_srn_is_used){
                 echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'SRN entered is already used against an application, please enter a new and valid SRN'));
            }else{
                ...
                if($name_res){
                    ...
                    if($company_detail){
                      ...
                      if($checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name change of existing company" && ($checkformtype['UK-FCL-00492_0']=='2' || $checkformtype['UK-FCL-00492_0']=='3') && $checkformtype['UK-FCL-00032_0']=='3'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 33, filed for Name reservation of existing company'));
                        }                                   
                    }else{         
                        echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'The status of Form 33 for the mentioned SRN is not approved.'));
                        }
                }else{
                     echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'This SRN does not belongs to Name Reservation'));
                }
