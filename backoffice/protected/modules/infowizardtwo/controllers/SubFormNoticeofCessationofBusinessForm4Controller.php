...
class SubFormNoticeofCessationofBusinessForm4Controller extends Controller {
                ...
                */
                ...
                if($model){
                  $responce = Processapplication::Matchstatus($model,$_POST['app_status']);
                    if($responce==false){
                        Yii::app()->user->setFlash('error', "Some one can performed the action on this application");
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormNoticeofCessationofBusinessForm4/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                $c_type = 'NCB4'; // Notice of Cessation of Business(Form 4)
...
if($farr['UK-FCL-00346_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00340_0'].' '.@$farr['UK-FCL-00341_0'].' '.@$farr['UK-FCL-00344_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00347_0'] ; */
        ...
        $companyDetail->company_type = 'NCB4';
  ...
  /* if($farr['UK-FCL-00502_0']=='Yes'){
        ...
        $updateCompany->save();
                ...
                $d_url = "";
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate43_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility43_0c";
            ...
            if ($_POST['app_status'] == 'FA') {
             ...
             $nocPath = "";
				...
				if($model->application_status=='A'){
					...
					Closeentity::parentfunction($model);
				}
...
// End Code of service provider 
            //Entity Code By Aamir
                        $model->entity_name = $_POST['UK-FCL-00415_0'];
            //End Entity Code
            ...
            {               
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormNoticeofCessationofBusinessForm4/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
                $print_app_call_back_url ="/backoffice/infowizardtwo/subFormNoticeofCessationofBusinessForm4/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
    ...
    public function actionUpdateSubForm() {
        ...
        if (!empty($_GET['service_id'])) {
			...
			$date = date('d/m/Y'); 
			// print_r($date);die;
			// print_r($fieldValues['field_value']);die;
    ...
    public function actionDownloadNewApp()
    {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility43_0::generatePdfApp($content, $name, $app_log,$fieldValues2,$submission_id=null,$dept_id=null);
        exit;
        }else{
	...
	public function actionGetCompanyNameByregno(){
        ...
        // $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type='BUS' AND reg_no='$reg_no'")->queryRow();
		...
		$c_detail = Yii::app()->db->createCommand("SELECT A.*,B.* from bo_company_details A
            ...
            WHERE A.company_type IN ('BUS')   AND A.reg_no='".$reg_no."' AND A.service_id IN('2.0') ")->queryRow();
		// print_r($c_detail);die;
        if($c_detail){         
				$fieldValues = json_decode($c_detail['field_value'], true);
				
				// echo $fieldValues['UK-FCL-00328_0'];die();
				
                echo CJavaScript::jsonEncode(
                    array('status'=>true,
                            'cname'=>$c_detail['company_name'],
							'regfirstname'=>$fieldValues['UK-FCL-00002_0'],
							'regmiddlename'=>$fieldValues['UK-FCL-00003_0'],
							'reglastname'=>$fieldValues['UK-FCL-00260_0'],
							'regaddress1'=>$fieldValues['UK-FCL-00011_0'],
							'regaddress2'=>$fieldValues['UK-FCL-00327_0'],
							
							'regparish'=>InfowizardQuestionMasterExt::getCountryStateNameByID($fieldValues['UK-FCL-00008_0']),
							'regpostalcode'=>$fieldValues['UK-FCL-00328_0'],
							
							'regcity'=>$fieldValues['UK-FCL-00010_0'],
							'regcountry'=>InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00007_0']),
							'regdate'=>$c_detail['created_on'],
							'datedthis'=>$c_detail['application_updated_date_time'],
                            'msg'=>'success'
                    )
                );
                      
        }else{
            echo CJavaScript::jsonEncode(array('status'=>false,'msg'=>'Please enter the valid business number issued by CAIPO'));
        }
     ...
     public function actionGetCompanyNameBySrnNo(){
        ...
        if($user_id){
            ...
            WHERE user_id=$user_id AND service_id='43.0'
            $checkexistingappid  
            ...
            if(!$chk_srn_is_used){
                 echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 7, filed for Intent to liquidate and dissolve.'));
            }elseif($isRevoked['UK-FCL-00618_0'] == 'Intends to liquidate and dissolve') {
                echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 7, filed for Intent to liquidate and dissolve.'));
            }else{ 
                if($chk_srn_is_used['application_status']!='A'){
                    echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'The status of Form 7 for the mentioned SRN is not approved.'));
                }
                else{
                    echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>json_decode($chk_srn_is_used['field_value'],true)['UK-FCL-00197_0'], 'timeremain'=>90,'msg'=>'success'));
                }
