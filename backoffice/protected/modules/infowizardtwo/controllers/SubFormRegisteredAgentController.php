...
class SubFormRegisteredAgentController extends Controller {
...
*/
        ...
        if (isset($allRes) && !empty($allRes)) {
			...
			{
				$templateID = DefaultUtility::dataSenetize($_POST['UK-FCL-00035_0']);
          ...
          /*  echo "<pre>";
            ...
            if (isset($dept) && !empty($dept)) {
                if (is_array($dept)) {
                    $deptStr = DefaultUtility::dataSenetize(implode(",", $dept));
                } else {
                    $dept2[0] = $dept;
                    $deptStr = DefaultUtility::dataSenetize(implode(",", $dept2));
                    $dept = array();
                    $dept[0] = $deptStr;
                }
            ...
            $uid = DefaultUtility::dataSenetize($_SESSION['uid']);
            ...
            if ($_POST['app_status'] == 'F') {
                foreach ($dept as $k => $v) {
                    ...
                    $msg = "Application has been forwarded to $dpt[$v]";
					$msglog = DefaultUtility::dataSenetize($msg);
                    //Update Data IN bo_sp_application for log
                    $currentDate = DefaultUtility::dataSenetize(date('Y-m-d H:i:s'));
                    $serviceNameArr = ApplicationV2Ext::getInfowizServiceDetails($_POST['service_id']);
                    $serviceApplicationUpdateQuery = "UPDATE bo_sp_applications SET app_comments='$msglog ',app_status='" . DefaultUtility::dataSenetize($_POST['app_status']) . "',updated_on='$currentDate' WHERE app_id=$app_Sub_id AND sp_tag='$sptagData[service_provider_tag]'";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegisteredAgent/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A') {
                ...
                $updateNewApplication = Yii::app()->db->createCommand("UPDATE bo_new_application_submission set application_status='$app_status',application_updated_date_time='$currentDate' where submission_id=$app_Sub_id")->execute();
                
                $d_url = "";
                /*if ($_POST['service_id'] == '16.0') {
                    $data = '';
                    $path = '';
            ...
            if ($_POST['app_status'] != 'F') {
                ...
                if ($_POST['app_status'] == 'H') {
                    ...
                    $app_status = 'P';
                    $update_date = DefaultUtility::dataSenetize(date('Y-m-d H:i:s'));
                    $sql_update = "UPDATE bo_new_application_submission SET application_status='P',application_updated_date_time='$update_date' where submission_id='$app_Sub_id'";
                    $updateNewApp = Yii::app()->db->createCommand($sql_update)->execute();
                } else {
...
//End Entity Code        
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/SubFormRegisteredAgent/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/SubFormRegisteredAgent/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
    ...
    function getExistingCafDocuments($sno = null, $service_id = null, $dept_id = null, $user_id = null) {
        $serviceExisting = DefaultUtility::dataSenetize($service_id);
        $serviceID = DefaultUtility::dataSenetize(explode(".", $serviceExisting));
        $docsInfo = array();
        $userID = DefaultUtility::dataSenetize($user_id);
        ...
        if (isset($docs['document_checklist_creation']) && !empty($docs['document_checklist_creation'])) {
            ...
            foreach ($allDocumnetsForExistingCaf as $doces) {
                $do = DefaultUtility::dataSenetize($doces->doc_id);
        ...
        foreach ($docsInfo as $docData) {
            if (isset($docData['documents_id'])) {
                $documentsID = DefaultUtility::dataSenetize($docData['documents_id']);
...
//		$res_caf = $command->queryAll();
        ...
        if (isset($_GET['id']) && $_GET['id'] > 0) {
            $id = DefaultUtility::dataSenetize($_GET['id']);
   ...
   static function alignInSequence($serviceID = null, $formCodeID = null) {
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        $content1 = $this->renderPartial('company_name_pdf_view_1', array('aap' => $allActivePages, 'formData' => $formData, 'signatoryDetails'=>$signatoryDetails, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        ...
        //echo $content; die;
        Utility16_0::generatePdfApp($content,$content1, $name, $app_log,$fieldValues2);
        exit;
        }else{
	...
	function actionSaveAllDocuments(){
		...
		$app_id = DefaultUtility::dataSenetize($app_id);
    ...
    public static function SavePDF($url, $path) {
     ...
     public function actionGetCompanyNameBySrnNo(){
       ...
       if($user_id){
        ...
        if($company_detail){
             $checkSrnStatus_newname = Yii::app()->db->createCommand("SELECT * from bo_recomended_name WHERE submission_id= '".$srn_no."' AND role_id IN(84, 83) AND  assign_new_name_status = '1' ORDER BY role_id DESC LIMIT 1")->queryRow();
             if($checkSrnStatus_newname){
                 echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>$checkSrnStatus_newname['assign_new_name'],'msg'=>'success'));
             }else{
                $checkSrnStatus = Yii::app()->db->createCommand("SELECT * from bo_recomended_name WHERE submission_id= '".$srn_no."' AND role_id IN(84, 83) AND  withdrawal_status = '0' ORDER BY role_id DESC LIMIT 1")->queryRow(); 
                     if($checkSrnStatus){
                         echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>$checkSrnStatus['recomended_value'],'msg'=>'success'));
                    }else{
                          echo CJavaScript::jsonEncode(
                            array('status'=>true, 'app_status'=>'invalid',                        // 'app_status'=>$company_detail['application_status'],
            ...
            /* echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>'Demo Society','msg'=>'success'));*/

            echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN'));
        }
       }else{
	...
	public function actionGetSocityNameByregno(){
        $reg_no = DefaultUtility::dataSenetize($_GET['reg_no']);
         ...
         $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE user_id = $user_id AND is_active=1 AND  company_type = 'S' AND reg_no='".$reg_no."'")->queryRow();
         if($c_detail){
            $check_already_exist =  Yii::app()->db->createCommand("SELECT * FROM entity_application_latest_data where entity_no='" . $reg_no  . "' AND service_id='9.0'")->queryRow();
                if($check_already_exist){
                    $field_value = (array) json_decode($check_already_exist['field_value']);
                    $parish = 'NA';
                    if(@$field_value['UK-FCL-00405_0']){
                        $parish = LandregionExt::getLandRegionNameViaId(@$field_value['UK-FCL-00405_0']);
                    }

                    $society_address = @$field_value['UK-FCL-00308_0'].' '.@$field_value['UK-FCL-00309_0'].' '.strtoupper($parish).' BARBADOS '.@$field_value['UK-FCL-00094_0'];

                    echo CJavaScript::jsonEncode(
                        array('status'=>true,
                            'app_status'=>'valid',
                                'name'=>$c_detail['company_name'],
                    ...
                    $society_address = $c_detail['address_line1'].' '.$c_detail['address_line2'].' '.$c_detail['city'].' '.$c_detail['state_parish'].' '.$c_detail['country'].' '.$c_detail['postal_code'];

                     echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>$c_detail['company_name'],'msg'=>'success','address'=>$society_address));
