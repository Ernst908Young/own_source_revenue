<?php
class SubFormArticlesOfAmendmentForm4Controller extends Controller
{
    ...
    bo_new_application_submission.processing_level from bo_sp_applications
        ...
        $result = Yii::app()
            ->db
            ->createCommand($sqlspapp)->queryRow();
        if (isset($result)) return $result[$val];
        else return false;
    }
    public function actionTestData(){
        $cdmaxid = BoCompanyDetails::getreg_no('S');
        echo "<pre>";print_r($cdmaxid);
        //$maxid = "BBA".date('Y').(str_pad($cdmaxid['max'], 6, '0', STR_PAD_LEFT));
        $maxid = (str_pad($cdmaxid, 7, '0', STR_PAD_LEFT));
        echo "<br>".$maxid;die;
    }
...
*/
        ...
        $allRes = Yii::app()
            ->db
            ->createCommand("SELECT * FROM bo_infowiz_form_builder_config_values where (service_id=$serviceID OR service_id=0) AND is_active='Y' AND table_name='bo_infowiz_formbuilder_application_forward_level' order by id desc")->queryAll();

        $allData = Yii::app()
            ->db
            ->createCommand("SELECT * FROM bo_infowiz_form_builder_configuration where service_id='$serviceID' AND current_role_id='$current_role_id' AND processing_level = (Select bo_new_application_submission.processing_level from bo_new_application_submission where bo_new_application_submission.submission_id='$app_Sub_id')")->queryRow();
        ...
        $sptagData = Yii::app()
            ->db
            ->createCommand("SELECT service_provider_tag FROM bo_infowizard_issuerby_master where issuerby_id IN (select dept_id from bo_new_application_submission where service_id='$serviceID' )")->queryRow();
        ...
        if (isset($allRes) && !empty($allRes))
        {
            ...
            if (isset($_POST['UK-FCL-00034_0']) && $_POST['UK-FCL-00034_0'] == 'Select From Template' && isset($_POST['UK-FCL-00035_0']) && !empty($_POST['UK-FCL-00035_0']))
            {
                ...
                $templateArr = Yii::app()
                    ->db
                    ->createCommand("SELECT template FROM bo_templates where id='$templateID'")->queryRow();
            ...
            if (isset($dept) && !empty($dept))
            {
                ...
                $deptNameArr = Yii::app()
                    ->db
                    ->createCommand("SELECT dept_id,department_name FROM `bo_departments` WHERE dept_id IN($deptStr)")->queryAll();
            ...
            if ($_POST['app_status'] == 'F')
            {
                foreach ($dept as $k => $v)
                {
                    ...
                    $serviceDetail = Yii::app()
                        ->db
                        ->createCommand($serviceApplicationUpdateQuery)->execute();

                    $serviceApplication = "Select * FROM  bo_sp_applications WHERE app_id=$app_Sub_id AND sp_tag='$sptagData[service_provider_tag]'";
                    $applicationDetail = Yii::app()
                        ->db
                        ->createCommand($serviceApplication)->queryRow();
            ...
            if (isset($flag) && $flag == 1)
            {
               ...
               $supportive_document ="";
                ...
                $res = Yii::app()
                    ->db
                    ->createCommand("Select appr_lvl_id from bo_infowiz_formbuilder_application_forward_level where app_Sub_id=$app_Sub_id AND next_role_id=$current_role_id AND forwarded_dept_id=$dept_id AND approv_status='P' order by appr_lvl_id DESC")->queryRow();
                $department_log_id = $res['appr_lvl_id'];

                $updateVerifierLevel = Yii::app()
                    ->db
                    ->createCommand("update bo_infowiz_formbuilder_application_forward_level set verifier_user_id=$uid,approv_status='V',verifier_user_comment='" . $comment . "',support_document='$supportive_document',comment_date='$comment_date',updated_date_time='$comment_date',post_info='$postData',form_id=$form_id where app_Sub_id=$app_Sub_id AND next_role_id=$current_role_id AND forwarded_dept_id=$dept_id")->execute();



                if ($allData['can_revert_to_investor'] == 'Y' && $app_status != 'V')
                {
                    $updateNewApplication = Yii::app()
                        ->db
                        ->createCommand("update bo_new_application_submission set application_status='$app_status' where submission_id=$app_Sub_id")->execute();
                }
            }
            if ($_POST['app_status'] == 'F')
            {
                Yii::app()
                    ->user
                    ->setFlash('success', "Application Id : $app_Sub_id has been forwarded to $deptName");
            }
            if ($_POST['app_status'] == 'H')
            {
                Yii::app()
                    ->user
                    ->setFlash('success', "Application Id : $app_Sub_id has been reverted to applicant.");
                $msglog = "Application Id : $app_Sub_id has been reverted.";
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationOfSocieties/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A')
            {
                Yii::app()
                    ->user
                    ->setFlash('success', "Application Id : $app_Sub_id has been approved successfully.");
                ...
                // get previous id record to get address 
                $reg_no=$_POST['UK-FCL-00290_0'];
                $fetchsrnRecords = Yii::app()
                    ->db
                    ->createCommand("
                            SELECT sub.* 
                            FROM bo_new_application_submission sub
                            INNER JOIN bo_company_details com
                            on sub.submission_id=com.srn_no
                            where com.reg_no='$reg_no'
                        ")->queryRow();

                $farr = (array)json_decode($fetchsrnRecords['field_value']);
               // echo "<pre>";print_r($_POST);print_r($farr);die;
                $name_related_srn = $_POST['UK-FCL-00360_0'];
                $c_name = $_POST['UK-FCL-00503_0'];
                $maxid = $_POST['UK-FCL-00290_0'];
                $asrn = $_POST['app_Sub_id'];
                $aser_id = $_POST['service_id'];
                $auser_id = $fetchsrnRecords['user_id'];
                $loginuserid = $_SESSION['uid'];

                $c_type = 'AA4'; //Article of Amendment form 4

                $cdmaxid = BoCompanyDetails::getreg_no($c_type);
                //$maxid = "BBA".date('Y').(str_pad($cdmaxid['max'], 6, 'Sahil 0', STR_PAD_LEFT));
                // $maxid = (str_pad($cdmaxid, 7, '0', STR_PAD_LEFT));
                $societyAddress=null;

                $parish1 = Yii::app()
                    ->db
                    ->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id='" . @$farr['UK-FCL-00405_0']."'")->queryRow();
                if (isset($farr['UK-FCL-00094_0']))
                {
                    $prases2 = Yii::app()
                        ->db
                        ->createCommand("SELECT * from bo_postalcode_in_barbados where id='" . @$farr['UK-FCL-00094_0']."'")->queryRow();
                    $pc1 = $prases2['code'];
                }
                else
                {
                 ...
                 $fulladdress =     @$farr['UK-FCL-00308_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
                ...
                $companyDetail->company_type ='AA4';
                ...
                $companyDetail->address_line1 = @$farr['UK-FCL-00308_0'];
                $companyDetail->address_line2 = @$farr['UK-FCL-00309_0'];
                $companyDetail->city = @$farr['UK-FCL-00310_0'];
                $companyDetail->state_parish_id = @$farr['UK-FCL-00405_0'];
                $companyDetail->postal_code_id = @$farr['UK-FCL-00094_0'];
                $companyDetail->state_parish = @$parish1['lr_name'];
                $companyDetail->postal_code = @$pc1;
                $companyDetail->country = @$farr['UK-FCL-00096_0'];
                $companyDetail->save();
                // end of aamir code
                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '19.0')
                {
                    $data = '';
                    $path = '';
                    ...
                    $download_certificate_call_back_url1 = CURL_URL . "/backoffice/infowizardtwo/subFormPdf/saveNewApprovalCertificate19_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id) . "/utility/Utility19_0c";
                    // echo $download_certificate_call_back_url1;die;
                ...
                $serviceApplicationUpdateQuery = "UPDATE bo_new_application_submission SET download_certificate_call_back_url = '$d_url', application_updated_date_time='$currentDate', entity_name='" . $companyDetail->reg_no . "', name_related_srn='".$name_related_srn."' WHERE submission_id='$app_Sub_id'";
                //echo $serviceApplicationUpdateQuery;die;
                $serviceDetail = Yii::app()
                    ->db
                    ->createCommand($serviceApplicationUpdateQuery)->execute();
            ...
            if ($_POST['app_status'] == 'R')
            {
                ...
                Yii::app()
                    ->user
                    ->setFlash('success', "Application Id : $app_Sub_id has been rejected.");
            ...
            if ($_POST['app_status'] == 'V')
            {
                ...
                $total_data = Yii::app()
                    ->db
                    ->createCommand("SELECT count(*) as total FROM bo_infowiz_formbuilder_application_forward_level where app_Sub_id=$app_Sub_id AND approv_status='P' AND next_role_id=$current_role_id")->queryRow();
                ...
                if ($total_data['total'] == 0)
                {
                    ...
                    if (isset($canRevToInves) && $canRevToInves == 'Y')
                    {
                        ...
                        $updateNewApp = Yii::app()
                            ->db
                            ->createCommand($sql_update)->execute();
                    }
                    $AppData = Yii::app()
                        ->db
                        ->createCommand("SELECT * FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                Yii::app()
                    ->user
                    ->setFlash('success', "Application Id : $app_Sub_id has been processed.");
            ...
            if ($_POST['app_status'] == 'FA')
            {
              ...
              $supportive_document ="";
                ...
                $approverData = Yii::app()
                    ->db
                    ->createCommand("SELECT approver_id FROM bo_infowiz_form_builder_configuration where service_id=$serviceID AND processing_level = (Select bo_new_application_submission.processing_level from bo_new_application_submission where bo_new_application_submission.submission_id='$app_Sub_id')")->queryRow();
                ...
                if ($ForwardLevelmodel->save())
                {
                    Yii::app()
                        ->user
                        ->setFlash('success', "Application Id : $app_Sub_id has been forwarded to Approver");
           ...
           $nocPath = "";
            ...
            if($model->application_status=='A'){
                Updatecompanymasterdata_for_society::parentfunction($model);
            }
    ...
    public function actionSaveData()
    {
       // print_r($_POST);die;
        ...
        $saved_caf_id = NULL;
        if (!empty($_POST) && !Yii::app()
            ->request
            ->isAjaxRequest)
        {
            $approvel_id = 0;
            $serviceID = DefaultUtility::sanatizeParams($_POST['service_id']);
            $issuer_id = DefaultUtility::sanatizeParams($_POST['issuer_id']);
            $approval_id = DefaultUtility::sanatizeParams(@$_POST['approval_id']);
            ...
            $allRes = Yii::app()
                ->db
                ->createCommand("SELECT * FROM bo_infowiz_form_builder_config_values where table_name='bo_new_application_submission' AND is_active='Y'")
                ->queryAll();
            ...
            if (isset($model->unit_name) && !empty($model->unit_name)) $unitName = $model->unit_name;
            ...
            //Get Land Region Id
            $landRegionArr = Yii::app()
                ->db
                ->createCommand("SELECT id,formvar_code FROM bo_infowiz_form_builder_config_values where use_for='landrigion_id' and table_name='bo_new_application_submission' and is_active='Y' and (service_id='0' OR service_id='$serviceID') order by service_id DESC")->queryRow();
            ...
            //Entity Code By Aamir
                $model->entity_name = $_POST['UK-FCL-00290_0'];
            ...
            if ($model->save())
            {
                ...
                {
                    $application_id = Yii::app()
                        ->db
                        ->getLastInsertId();
                }

                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfAmendmentForm4/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
                $print_app_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfAmendmentForm4/downloadNewApp/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";

                Yii::app()
                    ->db
                    ->createCommand("update bo_new_application_submission set reverted_call_back_url='$reverted_call_back_url',print_app_call_back_url='$print_app_call_back_url' where submission_id=$application_id")->execute();
                ...
                Yii::app()
                    ->user
                    ->setFlash('success', "Application details saved, please upload documents");
            ...
            else
            {
                Yii::app()
                    ->user
                    ->setFlash('error', "Form Not Submitted Please Try Again!");
    ...
    public function actionForwardToApprover()
    {
        $allData = Yii::app()
            ->db
            ->createCommand("SELECT id,service_id,current_role_id,next_role_id FROM bo_infowiz_form_builder_configuration where service_id=$serviceID AND current_role_id=0")->queryRow();
        $ForwardLevelmodel = new FormbuilderApplicationForwardLevel();
        ...
        if ($ForwardLevelmodel->save())
        {
            Yii::app()
                ->user
                ->setFlash('message', 'Application Forwarded Successfully');
    ...
    function getExistingCafDocuments($sno = null, $service_id = null, $dept_id = null, $user_id = null)
    {
        ...
        foreach ($docsInfo as $docData)
        {
            if (isset($docData['documents_id']))
            {
                ...
                //echo $sql22;
                $UploadedDocs = Yii::app()
                    ->db
                    ->createCommand($sql22)->queryAll();
    ...
    function actionSubFormListing()
    {
        ...
        $this->render("subformlisting", array(
            'res_d' => $res_d,
            'res_caf' => $res_caf,
            'res_s' => $res_s,
            'id' => $id
    ...
    public function actionGetFormCodeAjax()
    {
        ...
        else if ($page_results)
        {
            ...
            $result_data['data'] = array(
                "form_code" => $page_results['form_code'],
                'status' => true
            );
        }
        else
    ...
    static function alignInSequence($serviceID = null, $formCodeID = null)
    {
        ...
        $allActivePagesASC = Yii::app()
            ->db
            ->createCommand("SELECT page_name,id FROM bo_infowiz_page_master where service_id=$serviceID AND is_active='Y' $additionalCondition order by prefrence ASC")->queryAll();
        ...
        if (!empty($allActivePagesASC))
        {
            foreach ($allActivePagesASC as $pageAsPerPreferenceASC)
            {
                ...
                $allActiveCategoriesASC = Yii::app()
                    ->db
                    ->createCommand("SELECT category_id FROM bo_page_category_mapping where page_id=$pageAsPerPreferenceASC[id]  AND is_active='Y' order by prefrence ASC")->queryAll();
                if (!empty($allActiveCategoriesASC))
                {
                    foreach ($allActiveCategoriesASC as $categoryAsPerPreferenceASC)
                    {
                        ...
                        $allActiveMappedFormFieldASC = Yii::app()
                            ->db
                            ->createCommand("SELECT * FROM bo_information_wizard_form_builder where service_id=$serviceID AND category_id=$categoryAsPerPreferenceASC[category_id] $additionalCondition AND is_active='Y' order by preference ASC")->queryAll();
    ...
    public function actionSubFormView()
    {
        if (!empty($_GET['service_id']))
        {
            ...
            $allActivePages = Yii::app()
                ->db
                ->createCommand("SELECT page_name,id FROM bo_infowiz_page_master where service_id=$serviceID AND is_active='Y' AND form_id = $formCodeID order by prefrence ASC")->queryAll();
            extract($_GET);
            ...
            $fieldValues = Yii::app()
                ->db
                ->createCommand("SELECT * FROM bo_new_application_submission where submission_id = $subID")->queryRow();
            $fieldValues2 = (array)json_decode($fieldValues['field_value']);
        }
        $this->render('subformview', array(
            'aap' => $allActivePages,
            'formData' => $formData,
    ...
    public function actionDepartmentFormView()
    {
        ...
        if (!empty($_GET['service_id']))
        {
            ...
            $allActivePages = Yii::app()
                ->db
                ->createCommand("SELECT page_name,id FROM bo_infowiz_page_master where service_id=$serviceID AND is_active='Y' AND form_id = $applicantFormId order by prefrence ASC")->queryAll();

            $formData = $this->alignInSequence($_GET['service_id'], $applicantFormId);
            extract($_GET);

            $allProcessingFormFieldsArr = Yii::app()
                ->db
                ->createCommand("SELECT page_name,id FROM bo_infowiz_page_master where service_id=$serviceID AND is_active='Y' AND form_id = $formCodeID order by prefrence ASC")->queryAll();
        ...
        $this->render('departmentview', array(
            'aap' => $allActivePages,
            'formData' => $formData,
            'fieldValues' => $fieldValues2,
    ...
    public function actionUpdateSubForm()
    {
        ...
        $this->render('updatesubform', array(
            'aap' => $allActivePages,
            'formData' => $formData,
            'fieldValues' => $fieldValues2,
            'serviceID' => $_GET['service_id'],
    ...
    public function actionSaveUpdateSubForm()
    {
        ...
        if (isset($_POST) && !empty($_POST))
        {
            ...
            if ($modellog->save())
            {
                ...
                Yii::app()
                    ->user
                    ->setFlash('success', "Your application has been updated successfully.");
    ...
    public function actionDownloadNewApp()
    {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        $content = $this->renderPartial('amendment_society_pdf_view', array(
            'aap' => $allActivePages,
            'formData' => $formData,
            'signatoryDetails'=>$signatoryDetails,
            'fieldValues' => $fieldValues2,
            'app_id' => $fieldValues['submission_id'],
            'app_status' => $fieldValues['application_status'],
            'application_created_date' => $fieldValues['application_created_date']
        ) , true);
        ...
        //echo $content; die;
        Utility19_0::generatePdfApp($content, $name, $app_log,$fieldValues2, $submission_id=null,$dept_id=null);
         exit;
        }else{
    ...
    public function actionGenerateCertificate()
    {
        ...
        if (!empty($_POST))
        {
            ...
            $getContentExist = Yii::app()
                ->db
                ->createCommand("SELECT count(*) as tot FROM bo_infowiz_form_builder_certificate where service_id='$service_id' AND dept_id='$dept_id'")->queryRow();
            ...
            if ($getContentExist['tot'] > 0)
            {
                ...
                $updateNewApplication = Yii::app()
                    ->db
                    ->createCommand("update bo_infowiz_form_builder_certificate set content='$content',updated='$updated' where service_id='$service_id' and dept_id='$dept_id'")->execute();
                Yii::app()
                    ->user
                    ->setFlash('success', "Your content update successfully.");
            }
            else
            {
                ...
                if ($model->save())
                {
                    Yii::app()
                        ->user
                        ->setFlash('success', "Your content saved successfully.");
                }
                else
                {
                    die(var_dump($model->getErrors()));
        ...
        $getContentArr = Yii::app()
            ->db
            ->createCommand("SELECT * FROM bo_infowiz_form_builder_certificate where service_id='$service_id' AND dept_id='$dept_id'")->queryRow();

        $this->render('generate_certificate', array(
            'getContentArr' => $getContentArr
        ));
    }
    ...
    function actionSaveAllDocuments()
    {
        ...
        $sqlCheckFeeAplicable = Yii::app()
            ->db
            ->createCommand("SELECT id,fee_detail FROM bo_information_wizard_service_fee WHERE CONCAT(service_id,'.',servicetype_additionalsubservice) = $service_id")->queryRow();
        $user_id = $_SESSION['RESPONSE']['user_id'];
        //echo "<pre>"; print_r($sqlCheckFeeAplicable);die;
        $sqlCheckPaymentPaidorNot = Yii::app()
            ->db
            ->createCommand("SELECT count(*) as total FROM bo_unified_payment_logs WHERE service_id = '$service_id' AND submission_id = '$app_id' AND user_id= '$user_id'")->queryRow();
        ...
        if ($pdsatus == 1)
        {
            ...
            $sptagData = Yii::app()
                ->db
                ->createCommand("SELECT * FROM bo_infowizard_issuerby_master where issuerby_id=$dept_id")->queryRow();
            $allData = Yii::app()
                ->db
                ->createCommand("SELECT landrigion_id FROM bo_new_application_submission where submission_id=$app_id")->queryRow();
    ...
    public function ForwardToDepartment($submission_id = null, $sno = null, $service_id = null, $processLevel)
    {
        ...
        $allData = Yii::app()
            ->db
            ->createCommand("SELECT id,service_id,current_role_id,next_role_id FROM bo_infowiz_form_builder_configuration where service_id=$service_id AND current_role_id=0 AND processing_level='$processLevel'")->queryRow();
    ...
    //  public function actionCheckCompanyNameBySrnNo(){
    //     $srn_no = $_GET['srn_no'];
    //     $user_id=@$_SESSION['RESPONSE']['user_id'];
    //     $company_detail = Yii::app()->db->createCommand("SELECT * from bo_new_application_submission WHERE user_id=$user_id AND submission_id='".$srn_no."'")->queryRow();
    //     if($company_detail){
    //         $checkSrnStatus = Yii::app()->db->createCommand("SELECT * from bo_recomended_name WHERE submission_id= '".$srn_no."' AND role_id IN(84, 83) AND withdrawal_status = '0' ORDER BY role_id DESC LIMIT 1")->queryRow();
    //         if($checkSrnStatus){
    //              echo CJavaScript::jsonEncode(array('status'=>true,'recomended_value'=>$checkSrnStatus['recomended_value'],'records'=>$checkSrnStatus,'msg'=>'success'));
    //         }else{
    //               echo CJavaScript::jsonEncode(
    //                 array('status'=>'invalid',
    //                     // 'app_status'=>$company_detail['application_status'],
    //                     'msg'=>'SRN number is not approved',
    //                     'msg1'=>'1) Please file Form 15 before filing this Form for name reservation of the Company. Further, approval of Form 15 is a pre-requisite for approval of this Form 1.',
    //                     'msg2' => '2) The status of Form 15 for SRN filled in above field no. 1 is not approved. Please insert a valid SRN.'
    //                 )
    //             );
    //         }
    //     }else{
    //         echo CJavaScript::jsonEncode(array('status'=>false,'msg'=>'Please enter a valid SRN'));
    //     }
    // }
    public function actionGetsocietyNameByregno()
    {
        ...
        $c_detail = Yii::app()
            ->db
            ->createCommand(
                       "SELECT sub.*,com.* FROM bo_new_application_submission sub INNER JOIN 
                        bo_company_details com
                        ON com.srn_no=sub.submission_id
                        WHERE sub.user_id = $user_id AND is_active=1 AND company_type IN ('S')  AND reg_no='" . $reg_no . "' AND sub.service_id IN('9.0') ")->queryRow();
			// print_r($c_detail);die;
        if ($c_detail)
        {
             $check_already_exist =  Yii::app()->db->createCommand("SELECT * FROM entity_application_latest_data where entity_no='" . $reg_no  . "' AND service_id='".$c_detail['service_id']."'")->queryRow();
			// print_r($check_already_exist);die;
            if($check_already_exist['service_id']=='9.0'){
                 $fieldValue = (array) json_decode($check_already_exist['field_value']);
				// print_r($fieldValue);die;
               
                 echo CJavaScript::jsonEncode(
                     array('status'=>true,
                             'cname'=>$fieldValue['UK-FCL-00197_0'],
                             'totalCount'=>@$fieldValue['UK-FCL-00365_0'],
                             'managerDetails'=>$this->renderPartial('autoPopulateManger',["fieldValue"=>$fieldValue],true),
							 'noclasssahre' => @$fieldValue['UK-FCL-00365_0'],
                             'carryon'=>$fieldValue['UK-FCL-00232_0'],
                             'otherprovisions'=>@$fieldValue['UK-FCL-00126_0'],                 
                             'msg'=>'success'
                     )
                   );
				...
				$fieldValue=json_decode($c_detail['field_value'],true);
        ...
        else
        {
            echo CJavaScript::jsonEncode(array(
                'status' => false,
                'msg' => 'Please enter a correct and valid Society Number'
            ));
        }
    ...
    public function actionGetCompanyNameBySrnNo()
    {
        ...
        if ($user_id)
        {
            ...
            $chk_srn_is_used = Yii::app()
                ->db
                ->createCommand("SELECT * from bo_new_application_submission 
            WHERE application_status IN ('A','PD','F','I','P','Z','FA','AB','DP','W')
            ...
            if ($chk_srn_is_used)
            {
                echo CJavaScript::jsonEncode(array(
                    'status' => false,
                    'user' => $user_id, 
                    'msg' => 'Please enter a valid SRN, it must relate to Form 15, filed for Name reservation of same Society'
                ));
            }
            else
            {
                ...
                if ($name_res)
                {
                    ...
                    if ($company_detail)
                    {
                        ...
                        if($checkformtype['UK-FCL-00044_0']==1){
                              ...
                              if($banned_words){
                                  ...
                                  if($timeRemaining>0){
                                   echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>$banned_words['banned_words_name'], 'timeremain'=>$timeRemaining,'msg'=>'success'));
                                      ...
                                      echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, the SRN entered is expired, i.e. the time period of 90 days has been lapsed'));
                                  }
                              }else{
                               ...
                               echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 15, field for Name reservation of existing society'));
                          }    
                    }
                    else{         
                        echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'The status of Form 15 for the mentioned SRN is not approved.'));
                        }
                }else{
                     echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 15, filed for Name reservation of existing Society'));
                }
