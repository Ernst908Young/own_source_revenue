...
class SubFormAnnualReturnForm31Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormAnnualReturnForm31/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A') {
                ...
                $c_type = 'AR31'; // Annual Return of External Company (Form 31)
...
if($farr['UK-FCL-00346_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00340_0'].' '.@$farr['UK-FCL-00341_0'].' '.@$farr['UK-FCL-00344_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00347_0'] ; */
        ...
        $companyDetail->company_type = 'AR31';
...
/*Yii::app()->db->createCommand("INSERT INTO bo_company_details (srn_no, name_related_srn, service_id, company_type, reg_no,company_name,user_id,approved_by,address)
                ...
                if ($_POST['service_id'] == '30.0') {
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate30_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility30_0c";
...
//END CODE  BY:- Aamir Date :- 19-08-2021  
    ...
    public function actionSaveData() {
        ...
        if (!empty($_POST) && !Yii::app()->request->isAjaxRequest) {
            ...
            $submission_id = DefaultUtility::sanatizeParams(@$_POST['submission_id']);
            ...
            if (isset($_POST['submission_id'])) {
                $sql = "SELECT * FROM bo_new_application_submission where submission_id='" . $submission_id . "'";
                $model = NewApplicationSubmission::model()->findBySql($sql);
            ...
            $serviceNameArr = ApplicationV2Ext::getInfowizServiceDetails($serviceID);
            if (isset($allRes) && !empty($allRes)) {
            ...
            $model->landrigion_id = DefaultUtility::sanatizeParams(@$_POST[$landRegionArr['formvar_code']]);
            $model->field_value = json_encode($_POST);
            $model->dept_id = $getDepartMentId['issuerby_id'];
            $model->application_status = 'DP';
            $model->form_id = 1;
            $model->service_id = DefaultUtility::sanatizeParams($_POST['service_id']);
...
//Entity Code By Aamir
            $model->entity_name = DefaultUtility::sanatizeParams($_POST['UK-FCL-00403_0']);
...
//End Entity Code     
            ...
            if ($model->save()) {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormAnnualReturnForm31/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
                $print_app_call_back_url = "/backoffice/infowizardtwo/subFormAnnualReturnForm31/downloadNewApp/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
...
// jitendra singh
    public function actionSaveAjax() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $service_id = DefaultUtility::dataSenetize($_POST['service_id']);
            $prefrence = DefaultUtility::dataSenetize($_POST['prefrence']);
            $form_code = DefaultUtility::dataSenetize($_POST['form_code']);
            $page_name = DefaultUtility::dataSenetize($_POST['page_name']);
   ...
   public function actionSaveUpdateSubForm() {
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        $content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, "signatoryDetails"=>$signatoryDetails,'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        ...
        //echo $content; die;
        Utility30_0::generatePdfApp($content,$name,$app_log,$fieldValues2, $submission_id=null,$dept_id=null);
        exit;
        }else{
    ...
    public function actionGetCompanyNameBySrnNo() {
        ...
        $user_id = @$_SESSION['RESPONSE']['user_id'];
        //$user_id = 11;
        $comp_detal = Yii::app()->db->createCommand("SELECT * FROM bo_company_details WHERE user_id = $user_id AND is_active=1 AND company_type IN ('IC') AND reg_no='" . $reg_no . "'")->queryRow();
        if ($user_id) {
            if ($comp_detal)
                echo CJavaScript::jsonEncode(array('status' => true, 'app_status' => 'valid', 'name' => $comp_detal['company_name'], 'msg' => 'success', 'address' => $comp_detal['address']));
            else
                echo CJavaScript::jsonEncode(array('status' => false, 'user' => true, 'msg' => 'Please enter a correct and valid Company Number'));
        }else {
            echo CJavaScript::jsonEncode(array('status' => false, 'user' => false, 'msg' => 'login problem please login first'));
        }
    }
    ...
    public function actionGetcompanyNameByregno(){
        ...
        $user_id = $_SESSION['RESPONSE']['user_id'];
       // $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('EC') AND user_id = $user_id  AND reg_no='".$reg_no."'")->queryRow();
		
		$c_detail = Yii::app()->db->createCommand("SELECT A.*,B.* from bo_company_details A
            INNER JOIN bo_new_application_submission B ON A.srn_no = B.submission_id
            WHERE A.company_type IN ('EC') AND A.user_id = $user_id  AND A.reg_no='".$reg_no."' AND A.service_id IN('8.0') ")->queryRow();
        ...
        if($c_detail){   
				$check_already_exist =  Yii::app()->db->createCommand("SELECT * FROM entity_application_latest_data where entity_no='" . $reg_no  . "' AND service_id='".$c_detail['service_id']."'  ")->queryRow();
				 // print_r($check_already_exist['field_value']);die;
				$sharecapital_table = $director_table= NULL;
				if($check_already_exist){
					// echo $check_already_exist['field_value'];die;
                    $fieldValues =  (array) json_decode($check_already_exist['field_value']);
					
                    if($check_already_exist['service_id']=='8.0'){
						
						if(isset($fieldValues['UK-FCL-00248_0'])){
							if(is_array($fieldValues['UK-FCL-00248_0'])){
								$sharecapital_table='<div class="col-md-12 mb-3 test" id="add_more_186" style=""><div style="overflow:auto;"><hr><table class="table table-striped table-bordered table-hover responsive-table" i="" id="tbl_669"><thead><tr class="add_more_186"><th><b class="ukfcl" style="display:none;" title="115">UK-FCL-00248_0: </b>Class of shares </th><th><b class="ukfcl" style="display:none;" title="950">UK-FCL-00249_0: </b>Number of shares issued</th><th><b class="ukfcl" style="display:none;" title="951">UK-FCL-00250_0: </b>Authorised Capital</th><th><b class="ukfcl" style="display:none;" title="975">UK-FCL-00256_0: </b>Purchased by Company In last financial period </th><th><b class="ukfcl" style="display:none;" title="666">UK-FCL-00257_0: </b>Purchased by Company Cumulative Total</th><th><b class="ukfcl" style="display:none;" title="143">UK-FCL-00258_0: </b>Redeemed by Company In last financial period</th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00259_0: </b>Redeemed by Company Cumulative Total</th></tr></thead><tbody>';
								 foreach ($fieldValues['UK-FCL-00248_0'] as $key => $value) {  
									$sharecapital_table.='<tr class="add_more_186"> <td><input type="text" name="UK-FCL-00248_0[]" value="'.$value.'" class="form-control" title="'.$value.'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00249_0[]" value="'.$fieldValues['UK-FCL-00249_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00249_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00250_0[]" value="'.$fieldValues['UK-FCL-00250_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00250_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00256_0[]" value="'.$fieldValues['UK-FCL-00256_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00256_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00257_0[]" value="'.$fieldValues['UK-FCL-00257_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00257_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00258_0[]" value="'.$fieldValues['UK-FCL-00258_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00258_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00259_0[]" value="'.$fieldValues['UK-FCL-00259_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00259_0'][$key].'" readonly="" aria-invalid="false"></td>  </tr>';
								 }
								 $sharecapital_table.='</tbody></table><hr></div></div>';
							}
						}
							...
							if(isset($fieldValues['UK-FCL-00132_0'])){
								if(is_array($fieldValues['UK-FCL-00132_0'])){
									$director_table='<div class="col-md-12 mb-3 director" id="add_more_138" style=""><div style="overflow:auto;"><hr><table class="table table-striped table-bordered table-hover responsive-table" i="" id="tbl_669"><thead><tr class="add_more_138"><th><b class="ukfcl" style="display:none;" title="950">UK-FCL-00132_0: </b>First Name</th><th><b class="ukfcl" style="display:none;" title="951">UK-FCL-00133_0: </b>Middle Name</th><th><b class="ukfcl" style="display:none;" title="975">UK-FCL-00134_0: </b>Surname</th><th><b class="ukfcl" style="display:none;" title="666">UK-FCL-00093_0: </b>Address Line 1</th><th><b class="ukfcl" style="display:none;" title="143">UK-FCL-00309_0: </b>Address Line 2</th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00096_0: </b>Country </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00372_0: </b>State/Parish  </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00310_0: </b>City </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00094_0: </b>Postal Code </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00137_0: </b>Occupation  </th><th><b class="ukfcl" style="display:none;" title="2270">UK-FCL-00495_0: </b>Holder of a prominent public office</th><th><b class="ukfcl" style="display:none;" title="2271">UK-FCL-00496_0: </b>State name of office</th></tr></thead><tbody>';
									 foreach ($fieldValues['UK-FCL-00132_0'] as $key => $value) {  
										$director_table.='<tr class="add_more_138"> <td><input type="text" name="UK-FCL-00150_0[]" value="'.$value.'" class="form-control" title="'.$value.'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00133_0[]" value="'.$fieldValues['UK-FCL-00133_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00133_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00134_0[]" value="'.$fieldValues['UK-FCL-00134_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00134_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00169_0[]" value="'.$fieldValues['UK-FCL-00093_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00093_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00335_0[]" value="'.$fieldValues['UK-FCL-00309_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00309_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00295_0[]" value="'.InfowizardQuestionMasterExt::getCountryStateNameByID($fieldValues['UK-FCL-00096_0'][$key]).'" class="form-control" title="'.InfowizardQuestionMasterExt::getCountryStateNameByID($fieldValues['UK-FCL-00096_0'][$key]).'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00372_0[]" value="'.$fieldValues['UK-FCL-00372_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00372_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00354_0[]" value="'.$fieldValues['UK-FCL-00310_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00310_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00356_0[]" value="'.$fieldValues['UK-FCL-00094_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00094_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00137_0[]" value="'.$fieldValues['UK-FCL-00137_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00137_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00495_0[]" value="'.$fieldValues['UK-FCL-00495_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00495_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00496_0[]" value="'.$fieldValues['UK-FCL-00496_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00496_0'][$key].'" readonly="" aria-invalid="false"></td>  </tr>';
									 }
									 $director_table.='</tbody></table><hr></div></div>';
								}
							}
							
							$cname = $fieldValues['UK-FCL-00211_0'];
							$regaddress1 = @$fieldValues['UK-FCL-00340_0'];
							$regaddress2 = @$fieldValues['UK-FCL-00341_0'];
							$officecountry =(@$fieldValues['UK-FCL-00347_0']); 
								
							$regstate = (@$fieldValues['UK-FCL-00385_0']);
							$regcity = @$fieldValues['UK-FCL-00344_0'];
							$regpostal = @$fieldValues['UK-FCL-00346_0'];
							
							$priaddress1 = @$fieldValues['UK-FCL-00352_0'];
							$priaddress2 = @$fieldValues['UK-FCL-00353_0'];
							$pricity = @$fieldValues['UK-FCL-00354_0'];
							$pristate = InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00355_0']);
							$pripostal = @$fieldValues['UK-FCL-00356_0'];
							$pricountry = @$fieldValues['UK-FCL-00357_0']; 
							$pridate = @$fieldValues['UK-FCL-00218_0'];
							
							$firstname = @$fieldValues['UK-FCL-00150_0']; 
							$middlename = @$fieldValues['UK-FCL-00105_0']; 
							$lastname = @$fieldValues['UK-FCL-00106_0'];
							$address1 = @$fieldValues['UK-FCL-00107_0']; 
							$address2 = @$fieldValues['UK-FCL-00335_0']; 
							$attrcountry = (@$fieldValues['UK-FCL-00402_0']);
							$attstate = (@$fieldValues['UK-FCL-00400_0']);
							$attcity = @$fieldValues['UK-FCL-00399_0'];
							$attpostal = @$fieldValues['UK-FCL-00401_0'];
							$fieldValues = @$fieldValues;
							$sharecapital_table = $sharecapital_table;
                    ...
                    echo CJavaScript::jsonEncode(
                        array('status'=>true,
							...
							'priaddress2'=>@$fieldValues['UK-FCL-00353_0'],
							'pricity'=>@$fieldValues['UK-FCL-00354_0'],
							'pristate'=>(@$fieldValues['UK-FCL-00355_0']),
							'pripostal'=>@$fieldValues['UK-FCL-00356_0'],
							'pricountry'=>@$fieldValues['UK-FCL-00357_0'], 
							'pridate'=>@$fieldValues['UK-FCL-00218_0'],  
					...
					$fieldValues = (array) json_decode($c_detail['field_value']);
						if(isset($fieldValues['UK-FCL-00248_0'])){
							if(is_array($fieldValues['UK-FCL-00248_0'])){
								$sharecapital_table='<div class="col-md-12 mb-3 test" id="add_more_186" style=""><div style="overflow:auto;"><hr><table class="table table-striped table-bordered table-hover responsive-table" i="" id="tbl_669"><thead><tr class="add_more_186"><th><b class="ukfcl" style="display:none;" title="115">UK-FCL-00248_0: </b>Class of shares </th><th><b class="ukfcl" style="display:none;" title="950">UK-FCL-00249_0: </b>Number of shares issued</th><th><b class="ukfcl" style="display:none;" title="951">UK-FCL-00250_0: </b>Authorised Capital</th><th><b class="ukfcl" style="display:none;" title="975">UK-FCL-00256_0: </b>Purchased by Company In last financial period </th><th><b class="ukfcl" style="display:none;" title="666">UK-FCL-00257_0: </b>Purchased by Company Cumulative Total</th><th><b class="ukfcl" style="display:none;" title="143">UK-FCL-00258_0: </b>Redeemed by Company In last financial period</th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00259_0: </b>Redeemed by Company Cumulative Total</th></tr></thead><tbody>';
								 foreach ($fieldValues['UK-FCL-00248_0'] as $key => $value) {  
						...
						if(isset($fieldValues['UK-FCL-00132_0'])){
							if(is_array($fieldValues['UK-FCL-00132_0'])){
								$director_table='<div class="col-md-12 mb-3 director" id="add_more_138" style=""><div style="overflow:auto;"><hr><table class="table table-striped table-bordered table-hover responsive-table" i="" id="tbl_669"><thead><tr class="add_more_138"><th><b class="ukfcl" style="display:none;" title="950">UK-FCL-00132_0: </b>First Name</th><th><b class="ukfcl" style="display:none;" title="951">UK-FCL-00133_0: </b>Middle Name</th><th><b class="ukfcl" style="display:none;" title="975">UK-FCL-00134_0: </b>Surname</th><th><b class="ukfcl" style="display:none;" title="666">UK-FCL-00093_0: </b>Address Line 1</th><th><b class="ukfcl" style="display:none;" title="143">UK-FCL-00309_0: </b>Address Line 2</th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00096_0: </b>Country </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00372_0: </b>State/Parish  </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00310_0: </b>City </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00094_0: </b>Postal Code </th><th><b class="ukfcl" style="display:none;" title="744">UK-FCL-00137_0: </b>Occupation  </th><th><b class="ukfcl" style="display:none;" title="2270">UK-FCL-00495_0: </b>Holder of a prominent public office</th><th><b class="ukfcl" style="display:none;" title="2271">UK-FCL-00496_0: </b>State name of office</th></tr></thead><tbody>';
								 foreach ($fieldValues['UK-FCL-00132_0'] as $key => $value) {  
									$director_table.='<tr class="add_more_138"> <td><input type="text" name="UK-FCL-00150_0[]" value="'.$value.'" class="form-control" title="'.$value.'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00133_0[]" value="'.$fieldValues['UK-FCL-00133_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00133_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00134_0[]" value="'.$fieldValues['UK-FCL-00134_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00134_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00169_0[]" value="'.$fieldValues['UK-FCL-00093_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00093_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00335_0[]" value="'.$fieldValues['UK-FCL-00309_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00309_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00295_0[]" value="'.$fieldValues['UK-FCL-00096_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00096_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00372_0[]" value="'.$fieldValues['UK-FCL-00372_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00372_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00354_0[]" value="'.$fieldValues['UK-FCL-00310_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00310_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00356_0[]" value="'.$fieldValues['UK-FCL-00094_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00094_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00137_0[]" value="'.$fieldValues['UK-FCL-00137_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00137_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00495_0[]" value="'.$fieldValues['UK-FCL-00495_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00495_0'][$key].'" readonly="" aria-invalid="false"></td><td><input type="text" name="UK-FCL-00496_0[]" value="'.$fieldValues['UK-FCL-00496_0'][$key].'" class="form-control" title="'.$fieldValues['UK-FCL-00496_0'][$key].'" readonly="" aria-invalid="false"></td>  </tr>';
								 }
								 $director_table.='</tbody></table><hr></div></div>';
							}
						}
                    ...
                    array('status'=>true,
							...
							'regaddress1'=>$fieldValues['UK-FCL-00340_0'],
							'regaddress2'=>$fieldValues['UK-FCL-00341_0'],
							'officecountry'=>InfowizardQuestionMasterExt::getCountryStateNameByID($fieldValues['UK-FCL-00347_0']), 
							'regstate'=>InfowizardQuestionMasterExt::getCountryStateNameByID($fieldValues['UK-FCL-00385_0']),
							'regcity'=>$fieldValues['UK-FCL-00344_0'],
							'regpostal'=>$fieldValues['UK-FCL-00346_0'],
							...
							'address1'=>$fieldValues['UK-FCL-00107_0'], 
							'address2'=>$fieldValues['UK-FCL-00335_0'], 
							'attrcountry'=>InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00402_0']), 
							'attstate'=>InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00400_0']),
							'attcity'=>@$fieldValues['UK-FCL-00399_0'],
							'attpostal'=>@$fieldValues['UK-FCL-00401_0'],
							'fieldValues'=>@$fieldValues,
                            'sharecapital_table'=>$sharecapital_table,
