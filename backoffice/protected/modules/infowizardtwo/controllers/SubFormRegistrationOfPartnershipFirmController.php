...
class SubFormRegistrationOfPartnershipFirmController extends Controller {
...
*/
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationOfPartnershipFirm/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
        ...
        /* $desireName = $farr['UK-FCL-00268_0'];
          Yii::app()->db->createCommand("INSERT INTO  bo_banned_words (banned_words_name, banned_words_type, banned_word_match_type, status, process_from, app_id) VALUES ('".$desireName."', 'CHARITY_NAME', 'FULL', 'Y', 'SYSTEM', $app_Sub_id)")->execute();*/

          $farr = (array) json_decode($fetchsrnRecords['field_value']);
                $name_related_srn = 0;
                $c_name = $farr['UK-FCL-00268_0'];
...
$parish1 = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00404_0'])->queryRow(); 
...
if($farr['UK-FCL-00094_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00107_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
        ...
        $companyDetail->address_line1 = @$farr['UK-FCL-00107_0'];
         $companyDetail->address_line2 = @$farr['UK-FCL-00309_0'];
         $companyDetail->city = @$farr['UK-FCL-00310_0'];
         $companyDetail->state_parish_id = $farr['UK-FCL-00404_0'];
        ...
        $companyDetail->save();
                ...
                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '10.0') {
                    $data = '';
                    $path = '';
                    $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate10_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility10_0c";
...
// End Code of service provider             
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationOfPartnershipFirm/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormRegistrationOfPartnershipFirm/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
   ...
   static function alignInSequence($serviceID = null, $formCodeID = null) {
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility10_0::generatePdfApp($content,$name,$app_log);
        exit;
        }else{
