          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'F') {
                ...
                foreach ($dept as $k => $v) {
                    ...
                    $msglog = "Application has been forwarded to Registrar";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subForm/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A') {
                ...
                if ($_POST['service_id'] == '47.0') {
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate47_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility47_0c";
...
//END CODE  BY:- Aamir Date :- 19-08-2021  
            ...
            if($model->application_status=='A'){
                  ...
                  Yii::app()->db->createCommand("INSERT INTO entity_application_latest_data (entity_no, service_id, srn_no, user_id, field_value, created_on, updated_on, latest_entity_name, changed_from_service_id, changed_from_srn_no)
                                    ...
                                    WHERE a.submission_id = ".$model->submission_id)->execute();
    ...
    public function actionSaveData() {
        ...
        if (!empty($_POST) && !Yii::app()->request->isAjaxRequest) {
            ...
            $model->name_related_srn = DefaultUtility::dataSenetize(@$_POST['UK-FCL-00088_0']);
...
// End Code of service provider 
            ...
            {               
                ...
                if($model->service_id=='49.0'){
                    $name_place_id = $_POST['UK-FCL-00839_0'];
                }

                if($model->service_id=='52.0'){
                    $name_place_id = $_POST['UK-FCL-00849_0'];
                }

                
                if(isset($_SESSION['booking']['from_date'.$name_place_id])){
                   
                    $from_date = $_SESSION['booking']['from_date'.$name_place_id];
                    $to_date = $_SESSION['booking']['to_date'.$name_place_id];
                    $total_booking_amt = $_SESSION['booking']['total_booking_amt'.$name_place_id];

                    Yii::app()->db->createCommand("INSERT INTO bo_service_booking_schedule (service_id, submission_id, from_date, to_date, bo_booking_sub_category_master_id, total_amount, created_on) VALUES('".$model->service_id."','".$model->submission_id."','".date('Y-m-d',strtotime($from_date))."','".date('Y-m-d',strtotime($to_date))."','".$name_place_id."','".$total_booking_amt."','".date('Y-m-d H:i:s')."') ")->execute();

                        unset($_SESSION['booking']);
                 }

                 if($model->service_id=='50.0'){
                    $amount = $_POST['UK-FCL-00864_0'];
                    Yii::app()->db->createCommand("INSERT INTO bo_service_booking_schedule (service_id, submission_id, total_amount, created_on) VALUES('".$model->service_id."','".$model->submission_id."','".$amount."','".date('Y-m-d H:i:s')."') ")->execute();
                }

                
                   if($model->service_id=='51.0'){
                    $amount = $_POST['UK-FCL-00798_0'];
                    Yii::app()->db->createCommand("INSERT INTO bo_service_booking_schedule (service_id, submission_id, total_amount, created_on) VALUES('".$model->service_id."','".$model->submission_id."','".$amount."','".date('Y-m-d H:i:s')."') ")->execute();
                } 

                $reverted_call_back_url = "/panchayatiraj/backoffice/infowizardtwo/subForm/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
                $print_app_call_back_url ="/panchayatiraj/backoffice/infowizardtwo/subForm/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
               ...
               // Yii::app()->user->setFlash('success', "Your application for $service_Name has been submitted successfully . Your application id  is : $application_id");
                 ...
                 if(isset($_SESSION['applicant_data_from_pdf'][@$_POST['service_id']])){
                    unset($_SESSION['applicant_data_from_pdf'][@$_POST['service_id']]);
                 }
           ...
           Sendmailforservice::senttofo_draftmode($model); 
                ...
                if(isset($_SESSION['RESPONSE']["user_id"]) && !empty($_SESSION['RESPONSE']["user_id"])) 
                {
                    ...
                    $this->redirect("/panchayatiraj/backoffice/investor/ApplyService/DocumentsChecklist/is/no/type/POS?service_id=$seriveArr[0]&sub_service_id=$seriveArr[1]&department_id=$getDepartMentId[issuerby_id]&swcs_department_id=$department_id&swcs_service_id=$swcs_service_id&$servicePlus&caf_id=$application_id&type=Integrated+With+SWCS&ptype=Integrated+With+SWCS&approval_id=@$approval_id");
                }
            } else {
...
public function actionSaveUpdateSubForm() {
    if(isset($_POST) && !empty($_POST)) {
            ...
            $name_related_srn = DefaultUtility::dataSenetize(@$_POST['UK-FCL-00088_0']);
            $s4 = "update bo_new_application_submission set field_value='$postData',application_status='$stat',app_comments='$msglog',application_updated_date_time='$currentDate',name_related_srn='$name_related_srn' where submission_id=:submission_id";
            $connection = Yii::app()->db;
  ...
  if(IS_NOTIFICATION_ON==1){
    ...
    public function actionDownloadNewApp() {
        ...
        $content = $this->renderPartial('application_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);

        $name = "EJ-Application_Form_" . time() . ".pdf";
        
        //echo $content; die;
      //  UtilityFilmShooting::generatePdfApp($content, $name,'application_pdf');
        Utility_subform::generatePdfApp($content, $name);
        exit;
    ...
    public function actionProcessedApplication() {
        /*$app_Sub_id = 2814;
         $data = '';
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate47_0/service_id/" . base64_encode('47.0') . "/subID/" . base64_encode($app_Sub_id) . "/dept_id/" . base64_encode(1)."/utility/Utility47_0c";
                ...
                $serviceApplicationUpdateQuery = "UPDATE bo_new_application_submission SET download_certificate_call_back_url = '$d_url' WHERE submission_id='$app_Sub_id'";
                $serviceDetail = Yii::app()->db->createCommand($serviceApplicationUpdateQuery)->execute();*/
    ...
    public static function SavePDF($url, $path) {
     ...
     public function actionGetCompanyNameBySrnNo(){
       ...
       // echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>'Test', 'timeremain'=>20,'msg'=>'success'));
       // exit;
            ...
            if($chk_srn_is_used){
                ...
                if($name_res){
                    ...
                    if($company_detail){
                        ...
                        if($checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name reservation for new Company" && $checkformtype['UK-FCL-00031_0']=='4'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Name Reservation for new company can only be used.'));
                        }                                   
                    }else{         
    ...
    public function actionRequesttocorrectfieldvalue(){
        if(isset($_POST['service_id']) && isset($_POST['submission_id']) && isset($_POST['field_code'])){
            $service_id = $_POST['service_id'];
            $submission_id = $_POST['submission_id'];
            $field_code = $_POST['field_code'];
            $action_taken = $_POST['action_taken'];
            if($action_taken=='checked'){
                $is_active = 'Y';
            }else{
                $is_active = 'N';
            }
            ...
            $check_existing = Yii::app()->db->createCommand("SELECT * FROM bo_field_value_change_to_correct WHERE service_id='$service_id' AND submission_id=$submission_id AND field_code='$field_code' ")->queryRow();

            if($check_existing){
                Yii::app()->db->createCommand("UPDATE bo_field_value_change_to_correct SET is_active= '$is_active' WHERE id= ".$check_existing['id'])->execute();
            }else{
                Yii::app()->db->createCommand("INSERT INTO bo_field_value_change_to_correct (service_id, submission_id,field_code, created_on) VALUES ('$service_id', $submission_id, '$field_code', '$current_date')")->execute();
            }
            echo CJavaScript::jsonEncode(['status'=>true]);

        }
    }

    public function actionCheckforwardedlevelbtn(){
         if(isset($_POST['service_id']) && isset($_POST['submission_id'])){
             $service_id = $_POST['service_id'];
            $submission_id = $_POST['submission_id'];
            $check_field_change_reject_doc = 0;

            $check_existing = Yii::app()->db->createCommand("SELECT c.*, ffm.name, app.field_value 
                FROM bo_field_value_change_to_correct c
                INNER JOIN bo_infowizard_formvariable_master ffm on c.field_code=ffm.formchk_id 
                INNER JOIN bo_new_application_submission app on c.submission_id=app.submission_id                
                WHERE c.service_id='$service_id' AND c.submission_id=$submission_id AND c.is_active='Y' ")->queryAll();

            
            $data_table =[];
            if($check_existing){
                 $check_field_change_reject_doc = $check_field_change_reject_doc+ sizeof($check_existing);
                 
                 $fieldValues = (array) json_decode($check_existing[0]['field_value']);
                 foreach ($check_existing as $key => $value) {
                    $field_value = '';
                    if(array_key_exists($value['field_code'], $fieldValues)){
                        $field_value = $fieldValues[$value['field_code']];
                    }
                    $data_table[] = ['sr_no'=>$key+1,'field_name'=>$value['name'],'field_value'=>$field_value];
                 }               
            }

            $doc_array=Yii::app()->db->createCommand("SELECT dm.* , dc.name
                FROM bo_application_dms_documents_mapping dm 
                INNER JOIN bo_infowizard_documentchklist dc on dm.doc_id=dc.docchk_id
                where dm.sno=$submission_id AND dm.status='R'")->queryAll();

            if($doc_array){
                $check_field_change_reject_doc = $check_field_change_reject_doc+ sizeof($doc_array);
                foreach ($doc_array as $key => $value) {
                   
                    $data_table[] = ['sr_no'=>(sizeof($data_table)+1),'field_name'=>$value['name'],'field_value'=>$value['comments']];
                 }   
            }
            ...
            if(empty($data_table)){
                $data_table = [['sr_no'=>'No Data Found','field_name'=>'']];
            }

             echo CJavaScript::jsonEncode([
                'status'=>true,
                'check_field_change_reject_doc'=>$check_field_change_reject_doc,
 ...
 public function actionGetnameplaces() {
       
        $causes = "<option value=''>select record</options>";
        $district_code = $_GET['district_code'];
        $block_code = $_GET['block_code'];
        $gram_panchayat_code = $_GET['gram_panchayat_code'];
        $service_id = $_GET['service_id'];
        $all = Yii::app()->db->createCommand("SELECT * FROM bo_booking_sub_category_master WHERE is_active='Y' AND district_code=$district_code AND block_code=$block_code AND village_panchayat_code=$gram_panchayat_code AND service_id='".$service_id."'")->queryAll();
    ...
    public function actionChangecalendar(){
        if(isset($_POST['act']) && isset($_POST['month']) && isset($_POST['year'])){
            $action_for = $_POST['act'];
            $month = $_POST['month'];
            $year = $_POST['year'];
            $name_place_id = $_POST['name_place_id'];
            $month_year = $month.'-'.$year;
            $date = "$year-$month-01";

             if($action_for=='current'){                
                $new_date = date('Y-m-d');         
            }else{
                if($action_for=='next'){                
             ...
             $calender_content = $this->renderPartial('/formBuilder/subform/booking_calender',['month'=>$month,'year'=>$year,'name_place_id'=>$name_place_id],true);
            echo CJavaScript::jsonEncode(array('status'=>true,'calender_content'=>$calender_content));
        }
    }

    public function actionBookdates(){
        if(isset($_POST['name_place_id']) && isset($_POST['month']) && isset($_POST['year'])){
            $date = $_POST['date'];
            $month = $_POST['month'];
            $year = $_POST['year'];
            $name_place_id = $_POST['name_place_id'];
            ...
            $new_date = date('Y-m-d',strtotime($date));       
             ...
             if(isset($_SESSION['booking']['from_date'.$name_place_id])){
                $old_date = $_SESSION['booking']['from_date'.$name_place_id];

                if(strtotime($old_date)< strtotime($new_date)){
                    $_SESSION['booking']['from_date'.$name_place_id] = $old_date;
                    $_SESSION['booking']['to_date'.$name_place_id] = $new_date;
                }else{
                     if(strtotime($old_date) > strtotime($new_date)){
                        $_SESSION['booking']['from_date'.$name_place_id] = $new_date;
                        $_SESSION['booking']['to_date'.$name_place_id] = $old_date;
                    }else{
                         $_SESSION['booking']['from_date'.$name_place_id] = $new_date;
                         $_SESSION['booking']['to_date'.$name_place_id] = $new_date;
                    }
             ...
             $date1=date_create(@$_SESSION['booking']['from_date'.$name_place_id]);
            $date2=date_create(@$_SESSION['booking']['to_date'.$name_place_id]);
            $diff=date_diff($date1,$date2);
            if(($diff->days+1)>0){
                 $total_days = $diff->days+1;

                $name_place_details = Yii::app()->db->createCommand("SELECT * FROM bo_booking_sub_category_master WHERE id=$name_place_id")->queryRow();

                $service_fees = Yii::app()->db->createCommand("SELECT * FROM tbl_service_fee WHERE service_id='".$name_place_details['service_id']."'")->queryRow();

                $total_fees = $service_fees['fee_amount']*$total_days;
            }else{
                $total_fees = 0;
            }
            
            $_SESSION['booking']['total_booking_amt'.$name_place_id] = $total_fees;

             $calender_content = $this->renderPartial('/formBuilder/subform/booking_calender',['month'=>$month,'year'=>$year,'name_place_id'=>$name_place_id,'total_fees'=> $total_fees],true);
	...
	public function actionDonations() {
        $all = Yii::app()->db->createCommand("SELECT sso_users.mobile_no,sso_users.email,sso_profiles.first_name,sso_profiles.last_name,sso_profiles.surname,tbl_payment.* FROM tbl_payment 
		left join sso_users ON sso_users.user_id = tbl_payment.payment_submit_by
		left join sso_profiles ON sso_profiles.user_id = sso_users.user_id
		WHERE payment_for='donation'")->queryAll();
        $this->render('donation_application',array('all'=>$all));
    }
