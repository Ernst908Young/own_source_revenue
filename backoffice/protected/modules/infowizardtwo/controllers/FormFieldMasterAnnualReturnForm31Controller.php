...
class FormFieldMasterAnnualReturnForm31Controller extends Controller
{
	/**
		...
		if(!empty($_GET['app_Sub_id']) && !empty($_GET['service_id']) && !empty($_GET['form_id'])){
			...
			if($_GET['table']=='Department'){
					...
					$command = $connection->createCommand($sql);
			 		...
			 		if($data['post_info'] && $data['approv_status'] !='V'  ||  $app_status=='PF'){
						...
						$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_annualreturnform31', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}		 		
						...
						$fieldValues2 = (array)json_decode($fieldValues['field_value']);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_annualreturnform31', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$fieldValues2,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}
				...
				$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
	 			...
	 			$this->renderPartial('/formBuilder/subformloggerview_annualreturnform31', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'no','selectedDept'=>$selectedDept));
	 		}
			else{
