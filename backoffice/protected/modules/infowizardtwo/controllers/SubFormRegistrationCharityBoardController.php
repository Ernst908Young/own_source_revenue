...
class SubFormRegistrationCharityBoardController extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationCharityBoard/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A') {
                ...
                $updateNewApplication = Yii::app()->db->createCommand("update bo_new_application_submission set application_status='$app_status' where submission_id=$app_Sub_id")->execute();
                //Aamir code to inser word in banned_word table
...
$parish1 = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00129_0'])->queryRow(); 

if($farr['UK-FCL-00094_0']){
       $prases = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where id=".$farr['UK-FCL-00094_0'])->queryRow(); 
       $pc1 = $prases['code'];
      }else{
        $pc1 = "";
      }
  ...
  $fulladdress =     @$farr['UK-FCL-00308_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
        ...
        $companyDetail->address = $fulladdress;
         ...
         $companyDetail->state_parish_id = $farr['UK-FCL-00129_0'];
...
/*Yii::app()->db->createCommand("INSERT INTO  bo_banned_words (banned_words_name, banned_words_type, banned_word_match_type, status, process_from, app_id) VALUES ('".$c_name."', 'CHARITY_NAME', 'FULL', 'Y', 'SYSTEM', $app_Sub_id)")->execute();*/
//end code

                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '7.0') {
                    $data = '';
                    $path = '';
                    $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
                    $path = Yii::app()->basePath . "/caipo_certificate/" . $name;
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate7_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility7_0c";
...
// End Code of service provider             
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationCharityBoard/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormRegistrationCharityBoard/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
die();*/
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $app_log; die;
        Utility7_0::generatePdfApp($content, $name, $app_log, $fieldValues2); 
          exit;
        }else{
    ...
    public static function SavePDF($url, $path) {
     ...
     public function actionGetCharityNameByregno(){
        ...
        $user_id = $_SESSION['RESPONSE']['user_id'];
        $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type='CH' AND user_id = $user_id  AND reg_no='".$reg_no."'")->queryRow();
        if($c_detail){         
            ...
            echo CJavaScript::jsonEncode(array('status'=>false,'msg'=>'Please enter the valid charity registration number'));
        }
    ...
    public function actionGetSocityNameByregno(){
        $reg_no = $_GET['reg_no'];
        $user_id = $_SESSION['RESPONSE']['user_id'];
        $s_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type='S' AND user_id = $user_id  AND reg_no='".$reg_no."'")->queryRow();
        if($s_detail){         
                  echo CJavaScript::jsonEncode(
                    array('status'=>true,
                     		'sname'=>$s_detail['company_name'],
                     		'rdate'=>$s_detail['created_on'] ? date('d/m/Y',strtotime($s_detail['created_on'])) : NULL,
                     		'msg'=>'success'
                    )
                );
                      
        }else{
            echo CJavaScript::jsonEncode(array('status'=>false,'msg'=>'Please enter the valid society registration number'));
        }
