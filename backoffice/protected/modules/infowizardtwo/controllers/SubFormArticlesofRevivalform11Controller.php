...
class SubFormArticlesofRevivalform11Controller extends Controller {
...
*/
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesofRevivalform11/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
        ...
        $c_type = 'AR11'; //  Articles of Revival (Form 11)
        ...
        $companyDetail->company_type = 'AR11';
        ...
        $path = Yii::app()->basePath . "/caipo_certificate/" . $name;
                    ...
                    $download_certificate_call_back_url1 = CURL_URL . "/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate23_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id) . "/utility/Utility23_0c";
                ...
                $serviceApplicationUpdateQuery = "UPDATE bo_new_application_submission SET download_certificate_call_back_url = '$d_url', application_updated_date_time='$currentDate', application_status='$app_status' WHERE submission_id='$app_Sub_id'";
...
// End Code of service provider 
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesofRevivalform11/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormArticlesofRevivalform11/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
       ...
       $signatoryDetails= Yii::app()->db->createCommand("SELECT * FROM bo_signature_metadata where submission_id = $subID and is_active=1")->queryAll();
        ...
        $content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2,'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
         ...
         $contentform3 =  $contentform6 =  $contentform19 = NULL;
        ...
        //echo $content; die;
        Utility23_0::generatePdfApp($content, $contentform3, $contentform6, $contentform19, $name, $app_log,$fieldValues2);
        exit;
        }else{
   ...
   public function actionGetsocietyNameByregno(){
        ...
        $user_id = $_SESSION['RESPONSE']['user_id'];
        $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('S') AND user_id = $user_id  AND reg_no='".$reg_no."' AND service_id IN('9.0') ")->queryRow();
        if($c_detail){         
	...
	/* public function actionGetCompanyNameBySrnNo(){
        ...
        if($user_id){
            ...
            WHERE user_id=$user_id AND service_id='2.0'
            $checkexistingappid  
            AND submission_id='".$srn_no."'")->queryRow();
			// print_r($chk_srn_is_used);die;
            $isRevoked = json_decode($chk_srn_is_used['field_value'], true);
			// print_r($isRevoked);die;
            if(!$chk_srn_is_used){
                 echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid and correct SRN of Form 15 which shall be pertaining to Revival of the Society.'));
            }else{ 
                if($chk_srn_is_used['application_status']!='A'){
                    echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid and correct SRN of Form 15 which shall be pertaining to Revival of the Society.'));
                }
                else{
					// echo"aa";die;
                    echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>json_decode($chk_srn_is_used['field_value'],true)['UK-FCL-00046_0'],'name_last'=>json_decode($chk_srn_is_used['field_value'],true)['UK-FCL-00475_0'], 'timeremain'=>90,'msg'=>'success'));
           ...
           /*  echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>'Test', 'timeremain'=>20,'msg'=>'success'));
			...
			if($user_id){
                ...
                if($chk_srn_is_used){
                    ...
                    $name_res = Yii::app()->db->createCommand("SELECT submission_id from bo_new_application_submission where service_id = '2.0'  AND submission_id='".$srn_no."'")->queryRow();
                    if($name_res){

                    $company_detail = Yii::app()->db->createCommand("SELECT * from bo_new_application_submission  WHERE user_id=$user_id 
                        AND service_id = '2.0'  AND application_status = 'A' AND submission_id='".$srn_no."'")->queryRow();
                        if($company_detail){
                            ...
                            if($checkformtype['UK-FCL-00044_0']==1 ){
                                ...
                                $banned_words = Yii::app()->db->createCommand("SELECT * from bo_banned_words WHERE status='Y'
