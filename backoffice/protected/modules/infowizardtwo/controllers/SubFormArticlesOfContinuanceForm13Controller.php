...
class SubFormArticlesOfContinuanceForm13Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfContinuanceForm13/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                $name_related_srn = $farr['UK-FCL-00360_0'];
                $c_name = $farr['UK-FCL-00197_0'];
                ...
                $c_type = 'AC13'; // Article of Continuance Form 13

$cdmaxid = BoCompanyDetails::getreg_no($c_type);
//$maxid = "BBA".date('Y').(str_pad($cdmaxid['max'], 6, '0', STR_PAD_LEFT));
$maxid =  $farr['UK-FCL-00360_0'];

$parish1 = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00228_0'])->queryRow(); 

// if($farr['UK-FCL-00094_0']){
//        $prases1 = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where id=".$farr['UK-FCL-00094_0'])->queryRow(); 
//        $pc1 = $prases1['code'];
//       }else{
//         $pc1 = "";
//       }

  $fulladdress =     @$farr['UK-FCL-00308_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$farr['UK-FCL-00094_0'].' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
        ...
        $companyDetail->company_type =  'AC13';
        ...
        $companyDetail->address = $fulladdress;
         $companyDetail->address_line1 = @$farr['UK-FCL-00104_0'];
         $companyDetail->address_line2 = @$farr['UK-FCL-00335_0'];
         $companyDetail->city = @$farr['UK-FCL-00336_0'];
         $companyDetail->state_parish_id = $farr['UK-FCL-00228_0'];
         $companyDetail->postal_code_id = @$farr['UK-FCL-00401_0'];
         $companyDetail->state_parish = @$parish1['lr_name'];
         $companyDetail->postal_code = @$farr['UK-FCL-00401_0'];
         $companyDetail->country = @$farr['UK-FCL-00465_0'];
        $companyDetail->save();
                // end of aamir code

                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '22.0') {
                    $data = '';
                    $path = '';
                    $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
                    $path = Yii::app()->basePath . "/caipo_certificate/" . $name;

                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate22_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility22_0c";
...
//END CODE  BY:- Aamir Date :- 19-08-2021  
            ...
            if (isset($_POST['app_Sub_id'])) {
                $sql = "SELECT * FROM bo_new_application_submission where submission_id='" . $_POST['app_Sub_id']. "'";
                $model = NewApplicationSubmission::model()->findBySql($sql);
                $submission_id = $_POST['app_Sub_id'];              
            } 
    ...
    public function actionSaveData() {
        ...
        if (!empty($_POST) && !Yii::app()->request->isAjaxRequest) {
            ...
            if(isset($_POST['UK-FCL-00360_0']))
			$model->name_related_srn = DefaultUtility::dataSenetize($_POST['UK-FCL-00360_0']);
...
// End Code of service provider        
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfContinuanceForm13/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormArticlesOfContinuanceForm13/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
    ...
    public function actionDownloadNewApp() {
      ...
      //  $content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
       ...
       // $content = false;
        ...
        $contentform3 = $this->renderPartial('form3_pdf', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        $contentform6 = $this->renderPartial('form6_pdf', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
         $contentform19="";
       if(isset($fieldValues2['UK-FCL-00362_0']) && $fieldValues2['UK-FCL-00362_0']=='Yes')
        $contentform19 = $this->renderPartial('form19_pdf', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'signatoryDetails'=>$signatoryDetails, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        ...
        //echo $content; die;
        Utility22_0::generatePdfApp($content,$contentform3,$contentform6,$contentform19, $name, $app_log,$signatory_details,$fieldValues2);
        exit;
        }else{
            ...
            if($chk_srn_is_used){
                ...
                if($name_res){
                    ...
                    if($company_detail){
                        ...
                        if($checkformtype['UK-FCL-00044_0']==1 && $checkformtype['UK-FCL-00053_0']== '1'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 15, filed for Name reservation of new Society.'));
                        }                                   
                    }else{         
