...
class SubFormRestatedArticlesForm13Controller extends Controller {
...
*/
        ...
        if (isset($allRes) && !empty($allRes)) {
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRestatedArticlesForm13/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
        $c_type = 'RAI13'; //COMPANIES Restated Articles of Incorporation (Form 13)

        $companyDetail = new BoCompanyDetails;
        $companyDetail->srn_no = $_POST['app_Sub_id'];
        $companyDetail->name_related_srn = NULL;
        $companyDetail->service_id = $_POST['service_id'];
        $companyDetail->company_type = 'RAI13';
        $companyDetail->reg_no = NULL;
        $companyDetail->company_name = NULL;
        $companyDetail->user_id = $fetchsrnRecords['user_id'];
        ...
        $companyDetail->save();
                    ...
                    $download_certificate_call_back_url1 = CURL_URL . "/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate33_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id) . "/utility/Utility33_0c";
...
//End Entity Code  
            ...
            if ($model->save()) {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRestatedArticlesForm13/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
                $print_app_call_back_url = "/backoffice/infowizardtwo/subFormRestatedArticlesForm13/downloadNewApp/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $application_id . "/formCodeID/1";
   ...
   public function actionSaveUpdateSubForm() {
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
            ...
            /* echo "<pre>";
              print_r($formData);die(); */
            $fieldValues = Yii::app()->db->createCommand("SELECT * FROM      where submission_id = $subID")->queryRow();
            $fieldValues2 = (array) json_decode($fieldValues['field_value']);
        ...
        // echo $content; die;
        Utility33_0::generatePdfApp($content,$name,$app_log,$fieldValues2);
         exit;
        }else{
	...
	public function actionGetcompanyNameByregno(){
        $reg_no = DefaultUtility::dataSenetize($_GET['reg_no']);
		$typeOfEntity = $_POST['type_of_entity'];
		// print_r($typeOfEntity);die;
			if(empty($reg_no)){
		...
		if($typeOfEntity == 'Society'){
			$c_detail = Yii::app()->db->createCommand("SELECT A.*,B.* from bo_company_details A INNER JOIN bo_new_application_submission B ON A.srn_no = B.submission_id WHERE A.company_type IN ('S') AND A.user_id = $user_id  AND A.reg_no='".$reg_no."' AND A.service_id IN('9.0') ")->queryRow();
			...
			if($c_detail){    
                 $check_already_exist =  Yii::app()->db->createCommand("SELECT * FROM entity_application_latest_data where entity_no='" . $reg_no  . "' AND service_id='".$c_detail['service_id']."'")->queryRow();
                     $fieldValues = json_decode($c_detail['field_value'], true); 
                ...
                if($check_already_exist['service_id']=='9.0'){       
                    ...
                    echo CJavaScript::jsonEncode(
                      array('status'=>true,
                            ...
                            'purposesociety'=>$fieldValues['UK-FCL-00199_0'],                            
                            'durationsociety'=>@$fieldValues['UK-FCL-00361_0'],

                            'regadd1'=>@$fieldValues['UK-FCL-00308_0'],
                            'regadd2'=>@$fieldValues['UK-FCL-00309_0'],
                            'regcity'=>@$fieldValues['UK-FCL-00310_0'],
                            'regstate'=>InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00405_0']),
                            'regpostal'=>@$fieldValues['UK-FCL-00094_0'],
                            'regcountry'=>@$fieldValues['UK-FCL-00096_0'],

                            'regagent'=>@$fieldValues['UK-FCL-00362_0'],

                            'agentfirst'=>@$fieldValues['UK-FCL-00301_0'],
                            'agentmiddle'=>@$fieldValues['UK-FCL-00105_0'],
                            'agentsurname'=>@$fieldValues['UK-FCL-00324_0'],
                            'agnetadd1'=>@$fieldValues['UK-FCL-00107_0'],
                            'agentadd2'=>@$fieldValues['UK-FCL-00457_0'],
                            'agentcity'=>@$fieldValues['UK-FCL-00463_0'],
                            'agentstate'=>InfowizardQuestionMasterExt::getCountryStateNameByID(@$fieldValues['UK-FCL-00404_0']),
                            'agnetpostal'=>@$fieldValues['UK-FCL-00455_0'],
                            'agentcountry'=>@$fieldValues['UK-FCL-00320_0'],
							
							                      

                            'businesscarryon'=>@$fieldValues['UK-FCL-00232_0'],
                            'otherprovision'=>@$fieldValues['UK-FCL-00126_0'],
			    ...
			    echo CJavaScript::jsonEncode(array('status'=>false,'msg'=>'Please enter a correct and valid Society Number'));
		     }
		}else{
			$c_detail = Yii::app()->db->createCommand("SELECT A.*,B.* from bo_company_details A INNER JOIN bo_new_application_submission B ON A.srn_no = B.submission_id WHERE A.company_type IN ('IC') AND A.user_id = $user_id  AND A.reg_no='".$reg_no."' AND A.service_id IN('4.0') ")->queryRow();
                ...
                if($check_already_exist['service_id']=='4.0'){										
                    echo CJavaScript::jsonEncode(
                      array('status'=>true,
                            ...
                            'shareDetails'=>$this->renderPartial('autoPopulateshareholder',["fieldValue"=>$fieldValue],true),

                            'fixedrange'=>@$fieldValues['UK-FCL-00240_0'],
                            'minrange'=>@$fieldValues['UK-FCL-00119_0'],
                            'maxrange'=>@$fieldValues['UK-FCL-00120_0'],
                            'noofdirector'=>@$fieldValues['UK-FCL-00241_0'], 

                            'businesscarryon'=>@$fieldValues['UK-FCL-00121_0'],
                            'otherprovision'=>@$fieldValues['UK-FCL-00126_0'],                            

                            'publicprivate'=>@$fieldValues['UK-FCL-00123_0'],
                            'provisionincluded'=>@$fieldValues['UK-FCL-00124_0'],

                           
							
							//'managerDetails'=>'NA',
                            //'sharecapital_table'=>$sharecapital_table,
                            'company_detail'=>'comp_detal',
                            'society_detail'=>'NA',                 
		...
		// print_r($c_detail['service_id']);die;
