...
class SubFormRegistrationOfSocietiesController extends Controller {
...
$maxid = $cdmaxid;
$parish1 = Yii::app()->db->createCommand("SELECT lr_id,lr_name from bo_landregion where lr_id=".$farr['UK-FCL-00405_0'])->queryRow(); 
...
if($farr['UK-FCL-00094_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00308_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
        ...
        $companyDetail->company_type =  $c_type;
        ...
        $companyDetail->save();
                ...
                $currentDate = date('Y-m-d H:i:s');
                $d_url = "";
                if ($_POST['service_id'] == '9.0') {
                    $data = '';
                    $path = '';
                    $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
                    $path = Yii::app()->basePath . "/caipo_certificate/" . $name;

                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate9_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility9_0c";
...
// End Code of service provider             
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationOfSocieties/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormRegistrationOfSocieties/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
   ...
   static function alignInSequence($serviceID = null, $formCodeID = null) {
	...
	public function actionSaveUpdateSubForm() {
        ...
        $name_related_srn = DefaultUtility::dataSenetize($_POST['UK-FCL-00360_0']);
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility9_0::generatePdfApp($content, $contentform3, $contentform6, $name, $app_log);
         exit;
        }else{
                    ...
                    if($company_detail){
                        ...
                        if($checkformtype['UK-FCL-00044_0']==1 && $checkformtype['UK-FCL-00053_0']== '1'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Name Reservation for Societies can only be used.'));
                        }                                   
                    }else{         
