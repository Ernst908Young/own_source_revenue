...
class SubFormArticlesOfAmalgamationForm15Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfAmalgamationForm15/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                $c_type = 'AAF15'; // Articles Of Amalgamation (Form 15)
...
if($farr['UK-FCL-00346_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00340_0'].' '.@$farr['UK-FCL-00341_0'].' '.@$farr['UK-FCL-00344_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00347_0'] ; */
        ...
        $companyDetail->company_type = 'AAF15';
...
/*Yii::app()->db->createCommand("INSERT INTO bo_company_details (srn_no, name_related_srn, service_id, company_type, reg_no,company_name,user_id,approved_by,address)
                ...
                if ($_POST['service_id'] == '37.0') {
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate24_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility37_0c";
...
// End Code of service provider 
            //Entity Code By Aamir
                        $model->entity_name = $_POST['UK-FCL-00711_0'];
            //End Entity Code
            ...
            {               
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormArticlesOfAmalgamationForm15/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
                $print_app_call_back_url ="/backoffice/infowizardtwo/subFormArticlesOfAmalgamationForm15/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
    ...
    public function actionDownloadNewApp() {
      ...
      //  $content = $this->renderPartial('company_name_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
       ...
       // echo "<pre>";print_r($fieldValues2);die;
        ...
        //echo $content; die;
        Utility37_0::generatePdfApp($content,$contentform4,$contentform9, $name, $app_log,$signatory_details,$fieldValues2);
         exit;
        }else{
   ...
   public function actionGetcompanyNameByregno(){
		...
		$old_data = DefaultUtility::dataSenetize($_POST['old_data']);
		$latest_data = DefaultUtility::dataSenetize($_POST['latest_data']);
        $user_id = $_SESSION['RESPONSE']['user_id'];
		
		if($latest_data == ''){
			$c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('IC','NPC') AND user_id = $user_id  AND reg_no='".$reg_no."' AND service_id IN('4.0','5.0')")->queryRow();
		}else{
			$c1_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('IC','NPC') AND user_id = $user_id  AND reg_no='".$old_data."' AND service_id IN('4.0','5.0')")->queryRow();
		
			$old_data = $c1_detail['service_id'];
			
			if($old_data == '4.0'){

			$c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('IC') AND user_id = $user_id  AND reg_no='".$latest_data."' AND service_id IN('4.0')")->queryRow();
			
			}else if($old_data == '5.0'){
				$c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('NPC') AND user_id = $user_id  AND reg_no='".$latest_data."' AND service_id IN('5.0')")->queryRow();
        ...
        if($subid==NULL){
            ...
            if($chk_srn_is_used){
                ...
                $name_res = Yii::app()->db->createCommand("SELECT submission_id 
                    ...
                    where service_id = '2.0' AND user_id=$user_id            
                ...
                if($name_res){
                  ...
                  --  AND application_status = 'A'
                    ...
                    if($company_detail){
                        ...
                        if($company_detail['application_status']=='A' && $checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name change of existing company" && $checkformtype['UK-FCL-00032_0']=='1'){
                        ...
                        }else if($company_detail['application_status']!='A' && $checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name change of existing company" && $checkformtype['UK-FCL-00032_0']=='1'){ 
                            ...
                            echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 33, filed for Name change of existing company due to amalgamation.'));
                        }                                 
                    }else{ 
                        echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 33, filed for Name change of existing company due to amalgamation.'));
