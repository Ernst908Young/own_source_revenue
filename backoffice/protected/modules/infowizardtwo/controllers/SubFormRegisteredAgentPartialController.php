<?php
class SubFormRegisteredAgentPartialController extends Controller {
	...
	public function actionSaveDataPartial() {
        if(!empty($_POST)) 
		{
			...
			if(isset($_POST['submission_id']) && !empty($_POST['submission_id'])){				
				$sql = "SELECT * FROM bo_new_application_submission where submission_id='" . DefaultUtility::dataSenetize($_POST['submission_id'])."'";        
...
// End Code of service provider 			
	...
	function getExistingCafDocuments($sno = null,$service_id = null,$dept_id=null,$user_id=null) {       
        ...
        $serviceID = explode(".", DefaultUtility::dataSenetize($serviceExisting));
