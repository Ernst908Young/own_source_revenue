        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationCharity/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            ...
            if ($_POST['app_status'] == 'A') {
                ...
                $updateNewApplication = Yii::app()->db->createCommand("update bo_new_application_submission set application_status='$app_status', application_updated_date_time='$up_dt' where submission_id=$app_Sub_id")->execute();
...
if($farr['UK-FCL-00401_0']){
       $prases1 = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where id=".$farr['UK-FCL-00401_0'])->queryRow(); 
       $pc1 = $prases1['code'];
      }else{
        $pc1 = "";
      }

  $fulladdress =     @$farr['UK-FCL-00093_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00320_0'] ; 
        ...
        $companyDetail->address = $fulladdress;
         ...
         $companyDetail->country = @$farr['UK-FCL-00320_0'];
        $companyDetail->save();
...
/*Yii::app()->db->createCommand("INSERT INTO bo_company_details (srn_no, name_related_srn, service_id, company_type, reg_no,company_name,user_id,approved_by,address)
                VALUES ($asrn, $name_related_srn, $aser_id, 'CH', '".$maxid."', '".$c_name."', $auser_id, $loginuserid, '".$fulladdress."')")->execute();*/
                // end of aamir code

    
               //Insert banned name

            /* $desireName = $farr['UK-FCL-00111_0'];
            Yii::app()->db->createCommand("INSERT INTO  bo_banned_words (banned_words_name, banned_words_type, banned_word_match_type, status, process_from, app_id) VALUES ('".$desireName."', 'CHARITY_NAME', 'FULL', 'Y', 'SYSTEM', $app_Sub_id)")->execute();*/
               //End 
                ...
                if ($_POST['service_id'] == '6.0') {
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate6_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility6_0c";
...
// End Code of service provider             
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormRegistrationCharity/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormRegistrationCharity/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
 ...
 if(IS_NOTIFICATION_ON==1){
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility6_0::generatePdfApp($content, $name, $app_log);
          exit;
        }else{
