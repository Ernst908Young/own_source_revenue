...
class SubFormStatementOfIntentToDissolveRevocationOfIntentToDissolveForm25Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormStatementOfIntentToDissolveRevocationOfIntentToDissolveForm25/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
$fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();
                ...
                $c_type = 'SIDR25'; // Statement Of Intent To Dissolve/Revocation Of Intent To Dissolve (Form 25)
...
if($farr['UK-FCL-00346_0']){
  ...
  $fulladdress =     @$farr['UK-FCL-00340_0'].' '.@$farr['UK-FCL-00341_0'].' '.@$farr['UK-FCL-00344_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00347_0'] ; */
        ...
        $companyDetail->company_type = 'SIDR25';
  ...
  /* if($farr['UK-FCL-00502_0']=='Yes'){
        ...
        $updateCompany->save();
                ...
                $d_url = "";
                    ...
                    $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate39_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility39_0c";
...
// End Code of service provider
    ...
    //End Entity Code 
            ...
            {               
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormStatementOfIntentToDissolveRevocationOfIntentToDissolveForm25/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
                $print_app_call_back_url ="/backoffice/infowizardtwo/subFormStatementOfIntentToDissolveRevocationOfIntentToDissolveForm25/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
    ...
    public function actionDownloadNewApp()
    {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility39_0::generatePdfApp($content, $name, $app_log,$fieldValues2, $submission_id=null,$dept_id=null);
         exit;
        }else{
   ...
   public function actionGetcompanyNameByregno(){
        ...
        $user_id = $_SESSION['RESPONSE']['user_id'];
        $c_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE company_type IN ('IC') AND user_id = $user_id  AND reg_no='".$reg_no."' AND service_id IN('4.0') ")->queryRow();
     ...
     public function actionGetCompanyNameBySrnNo(){
        ...
        if($user_id){
            ...
            WHERE user_id=$user_id AND service_id='39.0'
            $checkexistingappid  
            ...
            if(!$chk_srn_is_used){
                 echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 25, filed for Intent to liquidate and dissolve.'));
            }elseif($isRevoked['UK-FCL-00613_0'] == 'Revokes its intent to dissolve') {
                echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Please enter a valid SRN, it must relate to Form 25, filed for Intent to liquidate and dissolve.'));
            }else{ 
                if($chk_srn_is_used['application_status']!='A'){
                    echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'The status of Form 25 for the mentioned SRN is not approved.'));
                }
                else{
                    echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','name'=>json_decode($chk_srn_is_used['field_value'],true)['UK-FCL-00089_0'], 'timeremain'=>90,'msg'=>'success'));
                }
