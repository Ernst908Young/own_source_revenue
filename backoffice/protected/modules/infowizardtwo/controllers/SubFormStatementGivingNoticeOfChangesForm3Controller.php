...
class SubFormStatementGivingNoticeOfChangesForm3Controller extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormStatementGivingNoticeOfChangesForm3/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
...
//END CODE  BY:- Aamir Date :- 19-08-2021  	
    ...
    public function actionGetCompanyNameByregno(){
     ...
     $comp_detal = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE user_id = $user_id AND company_type='BUS' AND is_active=1 AND reg_no='$reg_no'")->queryRow();
            ...
            if($comp_detal){
                 if(isset($_SESSION['RESPONSE']["user_type"]) && !empty($_SESSION['RESPONSE']["user_type"])) {
                    if($_SESSION['RESPONSE']["user_type"]==2 || $_SESSION['RESPONSE']["user_type"]==3){
                       if(@$_SESSION['individualuser_company_id']==$comp_detal['id']){
                           echo CJavaScript::jsonEncode(array('status'=>true,'app_status'=>'valid','cname'=>$comp_detal['company_name'],'msg'=>'success','address'=>$comp_detal['address']));
                       }else{
                         echo CJavaScript::jsonEncode(array('status'=>false,'user'=>true,'msg'=>'You are not allowed to perform this action'));
                       }
                    }else{
              ...
              echo CJavaScript::jsonEncode(array('status'=>false,'user'=>true,'msg'=>'Please enter a correct and valid Individual or firm Number'));
            }
       }else{
...
//End Entity Code   
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormStatementGivingNoticeOfChangesForm3/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormStatementGivingNoticeOfChangesForm3/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
   ...
   static function alignInSequence($serviceID = null, $formCodeID = null) {
    ...
    public function actionDownloadNewApp() {
      ...
      if (!empty($_GET['service_id']) && ((isset($_SESSION['RESPONSE']['user_id']) && $_SESSION['RESPONSE']['user_id']==$fieldValues['user_id']) || isset($_SESSION['role_id']) || isset($_POST['is_api_user']))) {
        ...
        //echo $content; die;
        Utility17_0::generatePdfApp($content, $name, $app_log, $fieldValues2);
        exit;
        }else{
