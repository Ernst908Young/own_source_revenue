		...
		if(!in_array($valued['button_id'],$arrofIn))
		{
			...
			{	
				...
				{ 
					...
					?>
					<?php if(isset($allData23[$datag]) && is_array($allData23[$datag])){ 
						if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) {
						 $ShowTableflg=1; $btnCoin=1; 
						} 
					}else{ 
						if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { 
							$ShowTableflg=1; $btnCoin=1; } 
				...
				<?php if($btnCoin==1){ ?>
					...
					if(isset($allDataMappedWithButton[$btnID]) && !empty($allDataMappedWithButton[$btnID])) 
						{  
						  foreach($allDataMappedWithButton[$btnID] as $key1=>$datag){ 
							...
							if(isset($allData23[$datag]) && is_array($allData23[$datag])){
							 if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { 
							 	$ShowTableflg=1; $gho=@$allData23[$datag][$k];
							 } 
							}else{
							 if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { 
							 	$ShowTableflg=1; $gho=@$allData23[$datag];
							 } 
							} 
						...
						?>
							...
							<td>
								<input class="form-control" name="<?php echo @$datag;?>[]" type="text" value="<?php if(isset($allData23[$datag]) && is_array($allData23[$datag])){ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag][$k];} }else{ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag];} } ?>" readonly title="<?php  if(isset($allData23[$datag]) && is_array($allData23[$datag])){ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag][$k];} }else{ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag];} } ?>"> 
							</td>

				<?php 

					$fieldsfordelete[] =  @$datag;
