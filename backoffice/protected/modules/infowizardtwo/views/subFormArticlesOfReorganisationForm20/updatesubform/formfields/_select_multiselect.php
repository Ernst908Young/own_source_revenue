 ...
 WHERE bo.formfield_id=$fd[id] AND bo.master_table_id!=0 AND bo.is_active='Y' ORDER BY bo.id DESC")->queryRow();
	...
	<?php } else {
		...
		<div class="col-md-12" id="input_<?php echo $tablefieldname;?>">
			...
			<?php //print_r($fieldValues[$tablefieldname]);die; ?>
				<option value="">Please Select </option>
			<?php foreach ($options as $option) { ?>
					<option value="<?php echo $option['options']; ?>" <?php if ($inputType == "multipleselect") { if(is_array(@$fieldValues[$tablefieldname]) AND(in_array($option['options'],@$fieldValues[$tablefieldname]))){ echo "selected";} }else{
		...
		<?php }else{ ?>
			...
			<?php 
					...
					if(@$fieldValues['UK-FCL-00351_0']!='' && $tablefieldname == 'UK-FCL-00349_0'){
			     	...
			     	if(@$fieldValues['UK-FCL-00295_0']!='' && $tablefieldname == 'UK-FCL-00400_0'){

			     		//$c_id = implode(', ', $fieldValues['UK-FCL-00295_0']);

			     		
                        
			     		$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id 
			     			IN ('".implode("','",$fieldValues['UK-FCL-00295_0'])."') 
			     			and lr_type='state' AND is_lr_active='Y'")->queryAll();        
			     	        foreach ($options as $k => $v) {				           
				...
				/*if($tablefieldname == 'UK-FCL-00400_0'){
					$c_id = $fieldValues['UK-FCL-00096_0'];
     				...
     				/*if($tablefieldname == 'UK-FCL-00438_0'){
					$c_id = $fieldValues['UK-FCL-00437_0'];
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					           
					            $select = $fieldValues['UK-FCL-00438_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
     				}*/
     				
     				/*if($tablefieldname == 'UK-FCL-00447_0'){
					$c_id = $fieldValues['UK-FCL-00446_0'];
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					           
					            $select = $fieldValues['UK-FCL-00447_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
     				}*/
