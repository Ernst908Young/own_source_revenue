 ...
 WHERE bo.formfield_id=$fd[id] AND bo.master_table_id!=0 AND bo.is_active='Y' ORDER BY bo.id DESC")->queryRow();
	...
	if (isset($options) && !empty($options)) {
		...
		<div class="col-md-12" id="input_<?php echo $tablefieldname;?>">
			<select name="<?php echo $tablefieldname; if ($inputType != "select") {   echo "[]"; } ?>" 	<?php if ($inputType == "multipleselect") {		echo " multiple='multiple'";}?> placeholder="<?php echo $formName; ?>" class="<?php echo $val_cls; ?> select2-me"	<?php if($fd['is_required'] == 'Y' && $val_cls == '') {	echo "required"; }?> id="<?php echo $tablefieldname; ?>" labelname="<?php echo $ln; ?>" style="<?php echo $classEditOrNot;?>">
	...
	<?php } else {
		...
		<div class="col-md-12" id="input_<?php echo $tablefieldname;?>">
			...
			<?php foreach ($options as $option) { ?>
					<option value="<?php echo $option['options']; ?>" 
						<?php if ($inputType == "multipleselect") { 
							if(@$fieldValues[$tablefieldname]){
