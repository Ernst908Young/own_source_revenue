 ...
 WHERE bo.formfield_id=$fd[id] AND bo.master_table_id!=0 AND bo.is_active='Y' ORDER BY bo.id DESC")->queryRow();
	...
	<?php } else {
		...
		<?php }else{ ?>
			...
			<?php 
// below code is for form 2 auto select dropdown select
			if($fieldValues['UK-FCL-00345_0'] !='' && $tablefieldname == 'UK-FCL-00346_0'){
     		...
     		/* if($fieldValues['UK-FCL-00531_0'] !='' && $tablefieldname == 'UK-FCL-00532_0'){
				 $p_id = $fieldValues['UK-FCL-00531_0'];			
		         $postalcode = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where p_id=$p_id")->queryAll();        
		         if($postalcode){		        
		            foreach ($postalcode as $key => $value) {
		            	$select = $fieldValues['UK-FCL-00532_0']==$value['id'] ? 'selected' : '';
		             echo "<option value='".$value['id']."' $select>".$value['district'].' - '.$value['code']."</option>";
		            }
		         }else{
		            echo "<option>-</option>";
...
// below code is for form 3 auto select dropdown select
			if(@$fieldValues['UK-FCL-00096_0']!="" && $tablefieldname == 'UK-FCL-00129_0'){
					$c_id = $fieldValues['UK-FCL-00096_0'];
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					           
					            $select = $fieldValues['UK-FCL-00129_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
     				}
     		if(@$fieldValues['UK-FCL-00384_0']!="" && $tablefieldname == 'UK-FCL-00534_0'){
					$c_id = $fieldValues['UK-FCL-00384_0'];
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					           
					            $select = $fieldValues['UK-FCL-00534_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
     				}
     		if(@$fieldValues['UK-FCL-00096_0']!='' && $tablefieldname == 'UK-FCL-00372_0' && !is_array($fieldValues['UK-FCL-00096_0'])){
					$c_id = $fieldValues['UK-FCL-00096_0'];
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					           
					            $select = $fieldValues['UK-FCL-00372_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
     				}
// form 4 dropdown code
	if($fieldValues['UK-FCL-00096_0'] !='' && $tablefieldname == 'UK-FCL-00372_0' && !is_array($fieldValues['UK-FCL-00096_0'])){
				 $p_id = $fieldValues['UK-FCL-00096_0'];			
		         $postalcode = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where p_id=$p_id")->queryAll();        
		         if($postalcode){		        
		            foreach ($postalcode as $key => $value) {
		            	$select = $fieldValues['UK-FCL-00372_0']==$value['id'] ? 'selected' : '';
		             echo "<option value='".$value['id']."' $select>".$value['district'].' - '.$value['code']."</option>";
		            }
		         }else{
		            echo "<option>-</option>";
