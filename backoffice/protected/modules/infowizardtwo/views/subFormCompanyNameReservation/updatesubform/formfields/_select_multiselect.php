...
$ispayment = Yii::app()->db->createCommand("SELECT * FROM tbl_payment WHERE submission_id=$app_id AND payment_status='success'")->queryRow();
if($ispayment){
	$ispayment_refund = Yii::app()->db->createCommand("SELECT * FROM tbl_payment WHERE submission_id=$app_id AND is_fee_refunded=1")->queryRow();
	if($ispayment_refund){
		$showdropdown = true;
	}else{
		$showdropdown = false;
...
$options = Yii::app()->db->createCommand("SELECT bo.*, bm.master_table_name,bm.condition, bm.key_id, bm.field_value, bm.is_active_field,
 ...
 WHERE bo.formfield_id=$fd[id] AND bo.master_table_id!=0 AND bo.is_active='Y' ORDER BY bo.id DESC")->queryRow();
	...
	if (isset($options) && !empty($options)) {
		...
		?>
		<div class="col-md-12">
			<?php if($tablefieldname=='UK-FCL-00044_0'){ 
					if($showdropdown){ ?>
				...
				<?php	}else{ ?>
						...
						<input type="hidden" name="<?php echo $tablefieldname ?>" id="<?php echo $tablefieldname ?>" value="<?= $fieldValues[$tablefieldname] ?>" />
						<textarea class="form-control" readonly="true" row="2" ><?php echo $allList[$fieldValues[$tablefieldname]] ?></textarea>

						<span style="color: orange;">In case you do not want to continue with this option, you can file for refund and apply for a new application.</span>
				<?php	}
				 }else{ ?>
	...
	<?php } else {
		...
		<?php }else{ ?>
			...
			/*if($tablefieldname == 'UK-FCL-00060_0'){
					$c_id = @$fieldValues['UK-FCL-00059_0'];
					if($c_id){
						$options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
					        foreach ($options as $k => $v) {
					            ...
					            $select = @$fieldValues['UK-FCL-00060_0']==$v['lr_id'] ? 'selected' : '';
							echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
					        }
					   }
     				}*/

			/*if($tablefieldname == 'UK-FCL-00061_0'){
				$p_id = @$fieldValues['UK-FCL-00060_0'];	
				if($p_id){
					$postalcode = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where p_id=$p_id")->queryAll();        
			         if($postalcode){		        
			            foreach ($postalcode as $key => $value) {
			            	$select = @$fieldValues['UK-FCL-00061_0']==$value['id'] ? 'selected' : '';
