                        ...
                        if (isset($options) && !empty($options)) {
                            ...
                            <div class="col-md-12">
                                ...
                                ?>" 
                                    ...
                                    <option value="">Please Select </option>
                                     <?php 
               if(@$fieldValues['UK-FCL-00295_0']!='' && $tablefieldname == 'UK-FCL-00129_0'){
                    if(isset($fieldValues['UK-FCL-00295_0'])){
                        ...
                        $options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
                            ...
                            echo "<option value='".$v['lr_id']."' $select>".$v['lr_name']."</option>";
                                ...
                                <?php }else{  ?>
                                    <?php 
                                    if($tablefieldname=='UK-FCL-00416_0'){
                                            $comsepstrg = implode(',', $fieldValues[$tablefieldname]);
                                            echo '<input type="hidden" value="'.$comsepstrg.'" id="UK-FCL-00416_0">';
                                        }
                                    foreach ($fieldValues[$tablefieldname] as $key => $value) {
                                        echo $value.'<br>';

                                    } ?>
                                    <!-- <div class="hidden">
                                     <select name="<1?php echo $tablefieldname; ?>"  multiple='true'
                                         placeholder='<1?php echo $formName; ?>' class="form-control disabled_dropdown"  <1?php if ($fd['is_required'] == 'Y') echo "required"; ?> id="<1?php echo $tablefieldname; ?>">

                                    <option value="">Please Select </option>
                                    <1?php foreach ($options as $option) { ?>
                                        <option value="<1?php echo $option['options']; ?>" <1?php
                                        if ($inputType == "multipleselect") {
                                            if (in_array($option['options'], @$fieldValues[$tablefieldname])) {
                                                echo "selected";
                                            }
                                        ...
                                        ?>><1?php echo $option['options']; ?></option>
                                            <1?php } ?>
                                </select>  
                            </div> -->
                                <?php } ?>  
