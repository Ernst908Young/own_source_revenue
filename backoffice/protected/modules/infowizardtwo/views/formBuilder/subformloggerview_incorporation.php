...
<div class="reservation-form p-0" style="text-align: left;"> 
    ...
    //Aamir code for pre-select each checkbox
    $change_to_checked = Yii::app()->db->createCommand("SELECT * FROM bo_field_value_change_to_correct WHERE service_id='$service_id' AND submission_id=$sub_id AND is_active='Y'")->queryAll();
    $ctc_array = [];
    foreach ($change_to_checked as $key => $value) {
        $ctc_array[] = $value['field_code'];
    }
   /* echo '<pre>';
    print_r($ctc_array);*/
    $sfArray = array();
        ...
        if (!in_array($fd['id'], $sfArray)) {
            ...
            if (!in_array($fd['id'], $btnArray)) {  //IF IN ARRAY START
                ...
                if ($inputType == "text" || $inputType == "number" || $inputType == "password") {
                    ...
                    @$tablefieldname = InfowizardQuestionMasterExt::getMasterName('bo_infowizard_formvariable_master', $fd['form_field_id'], 'formchk_id', 'formvar_id');
                    if(in_array(@$tablefieldname, $ctc_array)){
                        $checked = 'checked';
                    }else{
                        $checked = '';
                    }
                    ...
                    <div class="form-group col-md-6" id="div_<?php echo $tablefieldname; ?>">
                        <label class="col-md-12 control-label text-left" for=""  id="label_<?php echo $tablefieldname; ?>">
                            ...
                            <input type="checkbox" onchange="requesttochange('CHG_<?= $tablefieldname ?>','<?= $service_id ?>', '<?= $sub_id ?>','<?= $tablefieldname ?>')" id="CHG_<?= $tablefieldname; ?>" <?= $checked ?> /> 
                        </label>
                        ...
                        if (isset($options) && !empty($options)) {
                            ...
                            <div class="col-md-12">
                                ...
                                ?>" 
                                    ...
                                    <option value="">Please Select </option>
                                    <?php 
                                             if(@$fieldValues['UK-FCL-00345_0'] !='' && $tablefieldname == 'UK-FCL-00346_0'){
                if(isset($fieldValues['UK-FCL-00345_0'])){
                    $p_id = $fieldValues['UK-FCL-00345_0'];         
                 ...
                 if($postalcode){               
                    foreach ($postalcode as $key => $value) {
                        $select = $fieldValues['UK-FCL-00346_0']==$value['id'] ? 'selected' : '';
                ...
                if(@$fieldValues['UK-FCL-00351_0']!= '' && $tablefieldname == 'UK-FCL-00349_0'){
                    if(isset($fieldValues['UK-FCL-00351_0'])){
                        $c_id = $fieldValues['UK-FCL-00351_0'];
                        $options =  Yii::app()->db->createCommand("SELECT lr_id,lr_name FROM bo_landregion where parent_id='$c_id' and lr_type='state' AND is_lr_active='Y'")->queryAll();        
                            foreach ($options as $k => $v) {
                                ...
                                $select = $fieldValues['UK-FCL-00349_0']==$v['lr_id'] ? 'selected' : '';
    ...
    if (isset($result)) {
      ...
      <div class="form-part bussiness-det">
                ...
                <?php if (isset($VeriFyData) && !empty($VeriFyData)) { ?>           
                 <!--inline-block;-->   <div style="float: right;display: none; padding: 12px 0 8px;"><a href="/panchayatiraj/backoffice/dms/DepartmentDMSNew/view/sno/<?php echo base64_encode($result['sno']); ?>/user/<?php echo base64_encode($VeriFyData['user_id']) ?>" style="color:#fff;" class="btn btn-primary" target="_blank">Verify Documents</a></div>
                <?php } ?>                
                    <div class="form-row row">
                        <div class="col-lg-12">
                               ...
                               // if (!in_array($val_arr['chklist_id'], $checkDuplicate)) {
                                 //   $checkDuplicate[] = $val_arr['chklist_id'];
                                    ...
                                    <tr>
                                        ...
                                        <td class="td_center">
                                            <a target="_blank" href="/panchayatiraj/backoffice/doc/mydoc?view=<?php echo DefaultUtility::getDockey($val_arr['document_file_name']); ?>"style="color:blue;">
...
<!-- <div class="form-part bussiness-det">
    <div id="title_<1?php echo $tablefieldname;?>">                
        <h4 class="form-heading">
            Signature
        </h4>
    </div>
...
<div id="title_field_to_be_change">                
    <h4 class="form-heading">
        List of Fields to be change
    </h4>
</div>                

<div class="form-row row">
    <div class="form-group col-md-12">
        <table class="table table-striped table-bordered table-hover responsive-table" id="loftbc_table" style="">
            <thead>
            <tr>
                <th>Sr. No.</th>
                <th>Field Name</th>
                 <th>Field Value/Document Comment</th>
            </tr>
            </thead>
            <tbody id="loftbc_body">
...
?>
                    ...
                    if ($formName == 'Approve') {
                        $status = "A";
                        $cls = "btn btn-primary forwd_approved_btn";
                         $showFlag = 1;
...
<script type="text/javascript">
    ...
    function openPopup(doc_chk_id, dm, sid) {
        $('#md_cont').load('/panchayatiraj/backoffice/dms/DepartmentDMS/GetDCP/doc_chk_id/' + doc_chk_id + '/dm/' + dm + '/sid/' + sid);
    }
           ...
           success: function (data) {
               if (data.status) {
                 ...
                 $('#cmt_' + comid).val("");
                   ...
                   checkforwardedlevelbtn(data.service_id, data.submission_id);
    ...
    function requesttochange(id, service_id, submission_id, field_code) {
        var remember = document.getElementById(id);
          if (remember.checked) {
            $.ajax({
                type: "POST",
               
                url: "/panchayatiraj/backoffice/infowizardtwo/subForm/requesttocorrectfieldvalue",
                data : {service_id:service_id, submission_id:submission_id, field_code:field_code, action_taken:'checked'},
                 success : function (data) {
                     checkforwardedlevelbtn(service_id, submission_id);     
                 }
            });
            ...
            $.ajax({
                ...
                url: "/panchayatiraj/backoffice/infowizardtwo/subForm/requesttocorrectfieldvalue",
                data : {service_id:service_id, submission_id:submission_id, field_code:field_code, action_taken:'unchecked'},
                success : function (data) {
                     checkforwardedlevelbtn(service_id, submission_id);     
                 } 
            });
    ...
    function checkforwardedlevelbtn(service_id, submission_id){
        $.ajax({
                type: "POST",
                dataType:'json',
               
                url: "/panchayatiraj/backoffice/infowizardtwo/subForm/checkforwardedlevelbtn",
                data : {service_id:service_id, submission_id:submission_id},
                 success: function (data) {                 
                             
                    if(data.check_field_change_reject_doc<=0){
                         console.log('forwarded button show');
                        $(".forwd_approved_btn").attr('type','button');
                    }else{
                        $(".forwd_approved_btn").attr('type','hidden');
                    }
                    var table_data = data.data_table;
                    $("#loftbc_body").html("");
                    if(table_data){
                        $.each(table_data, function (key, value) {
                            $('#loftbc_body').append("<tr>\
                                        <td>"+value.sr_no+"</td>\
