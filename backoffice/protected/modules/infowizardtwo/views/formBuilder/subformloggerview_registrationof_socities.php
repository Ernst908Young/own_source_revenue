...
<?php
  $basePath = Yii::app()->basePath;
    include_once($basePath . "/modules/infowizardtwo/views/formBuilder/9_0_validation.php");
                    ...
                    <div class="form-group col-md-6" id="div_<?php echo $tablefieldname; ?>">
                        ...
                        <div class="col-md-12">
                            <input type="<?php echo $inputType; ?>" id="<?php echo $tablefieldname; ?>" name="<?php echo $tablefieldname; ?>" placeholder='<?php echo $formName ?>' class="form-control"  
                                   <?php if ($fd['is_required'] == 'Y') echo "required"; ?> value="<?php if (!is_array(@$fieldValues[$tablefieldname])) echo @$fieldValues[$tablefieldname]; ?>" disabled>
                        </div>
                    </div>
                    <?php
                }
...
if($tablefieldname == 'UK-FCL-00094_0'){                        
                if(isset($fieldValues['UK-FCL-00405_0'])){
                 ...
                 if($postalcode){               
                    foreach ($postalcode as $key => $value) {
                        $select = $fieldValues['UK-FCL-00094_0']==$value['id'] ? 'selected' : '';
           ...
           // form 1 agent detail is main fields select yes
            if($tablefieldname == 'UK-FCL-00455_0' && @$fieldValues['UK-FCL-00455_0']){                     
                if(isset($fieldValues['UK-FCL-00404_0'])){
                    $p_id = $fieldValues['UK-FCL-00404_0'];
                    $postalcode = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where p_id=$p_id")->queryAll();        
                 if($postalcode){               
                    foreach ($postalcode as $key => $value) {
                        $select = $fieldValues['UK-FCL-00455_0']==$value['id'] ? 'selected' : '';
                     echo "<option value='".$value['id']."' $select>".$value['district'].' - '.$value['code']."</option>";
                    }
                 }else{
                    echo "<option>-</option>";
            ...
            // form 2 mailling address when mailling address set to nos
            if($tablefieldname == 'UK-FCL-00338_0' && @$fieldValues['UK-FCL-00338_0']){                     
                if(isset($fieldValues['UK-FCL-00228_0'])){
                    $p_id = $fieldValues['UK-FCL-00228_0'];
                    $postalcode = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where p_id=$p_id")->queryAll();        
                 if($postalcode){               
                    foreach ($postalcode as $key => $value) {
                        $select = $fieldValues['UK-FCL-00338_0']==$value['id'] ? 'selected' : '';
                     echo "<option value='".$value['id']."' $select>".$value['district'].' - '.$value['code']."</option>";
                    }
                 }else{
                    echo "<option>-</option>";
...
<div class="form-group">
                ...
                <input disabled  name="<?php echo $tablefieldname;if ($inputType == "checkbox") { echo "[]";}?>" type="<?php echo $inputType; ?>" value="<?php echo $option['options']; ?>" name="<?php echo $fd['id'] ?>" class="chk_<?php echo $tablefieldname ?>"  
                    ...
                    if(isset($fieldValues[$tablefieldname])){
                            if($inputType== "checkbox" && is_array($fieldValues[$tablefieldname])) { 
                        if(in_array($option['options'],$fieldValues[$tablefieldname])){
                         echo "checked";} 
                        }else{
                            if($fieldValues[$tablefieldname]==$option['options']){
                ...
                if ($inputType == "textarea") {
                    ...
                    <div class="form-group col-md-6"  id="div_<?php echo @$tablefieldname; ?>">
                        ...
                        <div class="col-md-12">
                            <textarea name="<?php echo $tablefieldname; ?>" class="form-control" row="2" disabled id="<?php echo $tablefieldname; ?>"><?php if (!is_array(@$fieldValues[$tablefieldname])) echo @$fieldValues[$tablefieldname]; ?>
                            </textarea>
                        </div>
                    </div>
                    <?php
                ...
                if ($inputType == "calender") {
                    ...
                    <div class="form-group col-md-6"  id="div_<?php echo @$tablefieldname; ?>">
                        ...
                        <div class="col-md-12">
                            <input type="text" name="<?php echo $tablefieldname; ?>" class="form-control" value="<?php echo @$fieldValues[$tablefieldname]; ?>" disabled id="<?php echo $tablefieldname; ?>">
                        </div>
                    </div>
                    <?php
                }
