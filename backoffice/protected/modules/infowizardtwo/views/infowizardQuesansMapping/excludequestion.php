...
$stringArr=array(); 
if(!empty($excludee)) { 
foreach($excludee as $rstl)
{
$key=$rstl['queans_mapp_id'];
$stringArr[$key]=$rstl['exclude_question'];
//print_r($stringArr);
}  } 
else { $stringArr=array(); } 
...
<form name="" action="" method="POST">
           <?php
            if(empty($apps)){
             ...
             <tr><td colspan="8">Q<?php echo $apps['question_id']; ?>&nbsp;&nbsp;<?php $question=InfowizardQuesansMappingController::getListofQuestion($apps['question_id']); echo $question['name'];?> 
			 </td></tr>                        
                    </thead>
					<tr><th></th></tr>
      ...
      <?php $rrr=InfowizardQuesansMappingController::getDetailOfQuesAns($apps['question_id']); 
			  ...
			  <input type="hidden" name="question_id[]" value="<?php echo $apps['question_id']; ?>" />
			  <input type="hidden" name="queans_mapp_id[]" value="<?php echo $qmID=$ans['queans_mapp_id']; ?>" />
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="exclude_question[]" name="exclude_question[]" 
			  value="<?php if(!empty($stringArr[$qmID]) && ($qmID==$ans['queans_mapp_id'])) { echo $stringArr[$qmID]; } ?>"  />
