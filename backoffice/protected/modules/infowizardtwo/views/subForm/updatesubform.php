...
$check_app_reverted = Yii::app()->db->createCommand("SELECT action_status FROM bo_infowiz_form_builder_application_log WHERE action_status='H' AND app_Sub_id =".$_GET['subID'])->queryScalar();

$classEditOrNot = "";
$classEditOrNot_radio_checkbox = "";
if($check_app_reverted){
	$sqlEdit="SELECT show_field_editable_or_not FROM bo_infowiz_form_builder_configuration where current_role_id=0 AND service_id=$servID";
	...
	if(isset($rowEditYesorNo['show_field_editable_or_not']) && $rowEditYesorNo['show_field_editable_or_not']==1){
		...
		$classEditOrNot_radio_checkbox = "pointer-events:none;accent-color:#ccc;";
...
{ 
	$this->renderPartial('/formBuilder/'.$serviceIDArray[0].'_'.$serviceIDArray[1].'_validation',['fieldValues'=>$fieldValues]);
...
/*Aamir 4-1-2023 below code is for change fields*/
$fieldschanges = Yii::app()->db->createCommand("SELECT field_code FROM bo_field_value_change_to_correct WHERE is_active='Y' AND service_id = '$servID' AND submission_id =".$_GET['subID'])->queryAll();
$match_fields_for_change_array = [];
foreach ($fieldschanges as $key => $value) {
	$match_fields_for_change_array[] = $value['field_code'];
}
...
$this->renderPartial('updatesubform/section_category',['formData'=>$formData,'ap'=>$ap,'addMoreLineCheck'=>$addMoreLineCheck,'classEditOrNot'=>$classEditOrNot,'classEditOrNot_radio_checkbox'=>$classEditOrNot_radio_checkbox,'fieldValues'=>$fieldValues,'ButtonsArrayNew'=>$ButtonsArrayNew,'get_selected_field'=>$get_selected_field,'allData23'=>$allData23,'allDataMappedWithButton'=>$allDataMappedWithButton,'match_fields_for_change_array'=>$match_fields_for_change_array ]); ?>
...
<?php if($_GET['subID']){
	 ...
	 ?>
		...
		<a href="/panchayatiraj/backoffice/infowizardtwo/formBuilder/resetData/submission_id/<?= base64_encode($app_id) ?>" data-method="post" class="custome-btn reset_btn" onclick="return confirm('This will clear the entire form. Do you want to proceed ?')">
			<span>
...
<!--end: Form Wizard-->
	...
	<?php if(isset($action['form_service_js']) && !empty($action['form_service_js'])){ ?>		
		<script src="/panchayatiraj/backoffice/themes/swcsNewTheme/fb/demo/default/custom/components/forms/wizard/<?php echo @$action['form_service_js'];?>" type="text/javascript"></script>
	<?php }?>
	<!--end::Page Resources -->
		<script src="/panchayatiraj/backoffice/themes/swcsNewTheme/fb/demo/default/custom/components/forms/wizard/mainerrmessage.js" type="text/javascript"></script>
