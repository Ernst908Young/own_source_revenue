  ...
  <h1>Status for <span class = "cblue">Application ID - <?=$_GET['subID']?> </span> <?php if(isset($app_status)){
        ...
        switch ($app_status) {
          case 'DP':
            $app_status_name = 'Draft';
            break;
          case 'A':
            $app_status_name = 'Approved';
            break;
            case 'P':
            $app_status_name = 'Pending for Approval';
            break;
             case 'FA':
            $app_status_name = 'Pending for Approval';
            break;
            case 'R':
            $app_status_name = 'Rejected';
            break;
            case 'H':
            $app_status_name = 'Reverted';
            break;
            
           

          default:
            $app_status_name = $app_status;
            break;
        }
                             echo " is $app_status_name ";
                    ...
                    if(is_array($arrofIn) && !in_array($valued['button_id'],$arrofIn)){
                      ...
                      { 
                        ...
                        <tr>
                          ...
                          { 
                            $flagtbl = 1;
                            $fomFeildMasterArr = array('UK-FCL-01054_0','UK-FCL-01055_0','UK-FCL-00298_0','UK-FCL-00299_0');
                            ...
                            { 
                              ...
                              if(isset($gho) && in_array($datag,$fomFeildMasterArr) && is_numeric($gho))
                              {
                                ...
                                if($datag=='UK-FCL-01054_0'){
                                  $Catg=Yii::app()->db->createCommand("select name from bo_product_category where id=$gho")->queryRow();
                                  $approvalval = $Catg['name'];
                                }
                                if($datag=='UK-FCL-01055_0'){
                                  $SubCat=Yii::app()->db->createCommand("select name from bo_sub_product_category where id=$gho")->queryRow();
                                  $approvalval = $SubCat['name'];
                                }
