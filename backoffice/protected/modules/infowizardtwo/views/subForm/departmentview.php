...
<?php 
	...
	<div class="portlet box green">
		<div class="portlet-title">
			...
			?>
			<a href="/panchayatiraj/backoffice/infowizardtwo/<?php echo $action['form_action_controller'];?>/downloadNewApp/service_id/<?php echo $_GET['service_id'];?>/pageID/1/subID/<?php echo $submittion_id;?>/formCodeID/1" target="blank" class="btn btn-primary" style="height: 33px;">Print Application</a>
			</div>
		</div>
		<div class="portlet-body">
			<table class="table table-striped table-bordered table-hover">						
				<tr>
					<td class="a_cent"><strong style="color: #333;">Applicant Name:</strong> <?php echo $invData['first_name']." ".$invData['last_name']." ".$invData['surname']; ?></td>
		...
		if(!empty($get_logg))
		{
			...
			{                           
						...
						{					
							...
							if(isset($vall['form_id'])){
								if($vall['form_id']==3 && $vall['action_status']=='P')
									$btn_name= 'Reverted to Verifier';
							}
                    ...
                    ?>				
						<tr>
							...
							<!-- <td class="a_cent"><1?php echo $btn_name;?></td> -->
							<td class="a_cent"><?php $comm = $vall['department_comment'];
								
										
										echo nl2br(stripcslashes(stripcslashes(stripcslashes(stripcslashes($comm))))) ;?></td>
			...
			action="<?php echo Yii::app()->createAbsoluteUrl("/infowizardtwo/subForm/processData"); ?>">
    ...
    $(document).ready(function () {
		...
		$('.btn_view').on('click',function(){
			...
			$.ajax({
				...
				url: "<?php echo Yii::app()->request->baseUrl; ?>/infowizardtwo/formFieldMasterIncorporation/getFormHtml",
				success: function(data){
					console.log(data);
                                      //  alert(next_role_id+"===="+co);
					if(next_role_id && co !='V' && co !='F' && table!='Investor'){
						$('#frm').html(data);
						checkforwardedlevelbtn(service_id, app_Sub_id)
					}else{
						$('#frm').hide();
						$('.modal').modal();
						$('.modal-body').html(data);
