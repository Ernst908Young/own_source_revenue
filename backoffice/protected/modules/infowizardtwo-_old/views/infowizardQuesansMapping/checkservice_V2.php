...
.radio,.checkbox { 
    display: -webkit-inline-box !important; 
     display: -moz-inline-box !important; margin-left: 8px;!important; 
}
...
$('input').change(function() 
{
   ...
   for (var i = 0;i<arrayFromPHP.length;i++){
       if(elementItem == arrayFromPHP[i]['val']){    
           var sDisplay = arrayFromPHP[i]["exclude_question"] == null?'block':'none';
           var excludeItem = arrayFromPHP[i]["exclude_question"]==null? arrayFromPHP[i]["include_question"]:arrayFromPHP[i]["exclude_question"];
           var excludeArray = excludeItem.split(',');
           for(var p = 0 ;p<excludeArray.length;p++){
               var hItms = excludeArray[p];               
               var pk  = $('.'+hItms)[0];debugger;
              var pk= document.getElementsByClassName(hItms)[0];
               if(pk){debugger;
               pk.style.display=sDisplay;
