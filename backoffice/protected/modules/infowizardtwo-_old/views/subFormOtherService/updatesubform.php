...
if(!empty($ID)){ 
//$sql="SELECT selected_field_id,MIN(prefrence) asd ,bo_infowiz_subform_addmore_master.* FROM bo_infowiz_subform_addmore_master where is_active='Y' and service_id='$ID' GROUP BY button_id ORDER BY id";
//$sql="SELECT selected_field_id, id, MIN(prefrence) min_pref,is_active,button_id,prefrence
//FROM bo_infowiz_subform_addmore_master where is_active='Y' AND service_id='$ID' 
//GROUP BY button_id ORDER by id";
...
.admr{display:none;}
  ...
  .h4{font-size:14px;color:#9699a2; 
  	margin-top: 15px; }
   /*.h4:before,*/
.h4:after {
...
if(isset($_GET['service_id']) && ($_GET['service_id']=='592.0'))
{     	
	include_once($basePath."/modules/infowizard/views/formBuilder/592_0_validation.php");
}else 
if(isset($_GET['service_id']) && $_GET['service_id']=='596.0')
{     
	include_once($basePath."/modules/infowizard/views/formBuilder/596_0_validation.php");
}else
...
<?php 
	...
	include_once($basePath."/modules/infowizard/views/formBuilder/120_0_validation.php");
		...
		include_once($basePath."/modules/infowizard/views/formBuilder/597_0_validation");
 }
 ?>
...
if (isset($submittion_id) && !empty($submittion_id)) {
	...
	<!--begin: Portlet Body-->
						...
						<?php }else if(isset($_SESSION['uid']) && !empty(isset($_SESSION['uid'])) && in_array($_GET['service_id'],array('119.0','226.0','227.0','228.0'))){ ?>
							<form class="m-form m-form--label-align-left- m-form--state-" id="m_form" method="post" action="<?php echo Yii::app()->createAbsoluteUrl("/infowizard/subForm/saveUpdateSubForm"); ?>">
						<?php }else if(in_array($_GET['service_id'],array('119.0','226.0','227.0','228.0','119.1','119.3','119.5','226.1','226.3','226.5','226.6','227.1','227.3','227.5','227.6','228.1','228.3','228.5','228.6'))){ ?>
							<form class="m-form m-form--label-align-left- m-form--state-" id="other_service_form" method="post" action="<?php echo Yii::app()->createAbsoluteUrl("/infowizard/subFormOtherService/saveUpdateSubForm"); ?>">
						<?php }else{ ?>
							<form class="m-form m-form--label-align-left- m-form--state-" id="m_form" method="post" action="<?php echo Yii::app()->createAbsoluteUrl("/infowizard/subForm/saveUpdateSubForm"); ?>">
						<?php }?>
						...
						<?php  foreach ($aap as $pageKey => $ap) { ?>
							...
							<input type="hidden" name="service_id" value="<?php echo $serviceID; ?>">
										...
										/*  echo "<pre>";
										print_r($allDataMappedWithButton); */
                                            ...
                                            if ($ap['id'] == $fd['page_name']) {
                                                ...
                                                if ($inputType == "text" || $inputType == "number" || $inputType == "password"  || $inputType == "email" || $inputType == "url") 
												{
                                                    ...
                                                    <div class="form-group col-md-6" id="div_<?php echo $tablefieldname;?>">
                                                        ...
                                                        <div class="col-md-12">
															<input type="<?php echo $inputType; ?>" name="<?php echo $tablefieldname; ?>" placeholder='<?php echo $formName ?>' labelname="<?php echo $ln; ?>" class="form-control <?php echo $val_cls; ?>" <?php if ($fd['is_required'] == 'Y' && $val_cls == '') { echo "required"; } ?> id="<?php echo $tablefieldname; ?>" onfocus="this.placeholder = ''" onblur="this.placeholder = '<?php echo $formName ?>'" value="<?php if(!is_array(@$fieldValues[$tablefieldname])) echo  @$fieldValues[$tablefieldname]; ?>" <?php if($maxLength>0){?>maxlength="<?php echo $maxLength; ?>"<?php  } ?> <?php if($minLength>0){?>minlength="<?php echo $minLength; ?>"<?php  } ?>  <?php echo $addAttr; ?>>
                                                        </div>
                                                    </div>
                                                <?php 
												} 
									...
									?>
											...
											$options = Yii::app()->db->createCommand("SELECT bo.*, bm.master_table_name, bm.key_id, bm.field_value, bm.is_active_field,
											 ...
											 WHERE bo.formfield_id=$fd[id] AND bo.master_table_id!=0 AND bo.is_active='Y' ORDER BY bo.id DESC")->queryRow();
												...
												if (isset($options) && !empty($options)) {
													...
													?>
													<div class="col-md-12">
														<select name="<?php echo $tablefieldname; if ($inputType != "select") {   echo "[]"; } ?>" 	<?php if ($inputType == "multipleselect") {		echo " multiple='multiple' style='max-height:120px;'";}?> placeholder="<?php echo $formName; ?>" class="<?php echo $val_cls; ?> select2-me"	<?php if($fd['is_required'] == 'Y' && $val_cls == '') {	echo "required"; }?> id="<?php echo $tablefieldname; ?>" labelname="<?php echo $ln; ?>">
										...
										$tablefieldname = InfowizardQuestionMasterExt::getMasterName('bo_infowizard_formvariable_master', $fd['form_field_id'], 'formchk_id', 'formvar_id');
											...
											<div class="form-group col-md-6" id="div_<?php echo $tablefieldname;?>">
												...
												<div class="col-md-12">
													<textarea name="<?php echo $tablefieldname; ?>" class="form-control <?php echo $val_cls; ?>" row="2" id="<?php echo $tablefieldname; ?>" labelname="<?php echo $ln; ?>" <?php if ($fd['is_required'] == 'Y' && $val_cls == '') { echo "required"; } ?>><?php if(!is_array(@$fieldValues[$tablefieldname])) echo @$fieldValues[$tablefieldname];?></textarea>
												</div>
											</div>
											<?php } ?>
											<?php
										...
										?>	
										   ...
										   <div class="form-group col-md-12">                
											...
											<hr><table class="table table-striped table-bordered table-hover responsive-table" id="tbl_<?php $tblID="tbl_".$id; echo $id;?>" style="">
											<thead>
															...
															<tr class="add_more_<?php echo $id;?>">
																...
																?>
																	...
																	<td><?php //echo $datag."==".$allData23[$datag][$k]."===".$serviceID."==".$approvalval; ?>
																		<input class="form-control  <?php if(@$datag=='UK-FCL-00226_4'){ echo "sklmf_".$clsr;$clsr=$clsr+1; } ?><?php if(@$datag=='UK-FCL-00226_7'){ echo "skmf"; } ?>" name="<?php echo @$datag;?>[]" type="text" value="<?php if(isset($approvalval) && !empty($approvalval)){ echo $approvalval; }else{ if(isset($allData23[$datag]) && is_array($allData23[$datag])){ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag][$k];} }else{ if(isset($allData23[$datag][$k]) && !empty($allData23[$datag][$k])) { $ShowTableflg=1; echo @$allData23[$datag];} } } ?>" readonly> 
																	</td>
		...
		<?php }else if(isset($_SESSION['uid']) && !empty(isset($_SESSION['uid'])) && in_array($_GET['service_id'],array('119.0','226.0','227.0','228.0'))){ ?>
			<script src="/backoffice/themes/swcsNewTheme/fb/demo/default/custom/components/forms/wizard/subformwizard.js" type="text/javascript"></script>
		<?php }else if(in_array($_GET['service_id'],array('119.0','226.0','227.0','228.0','119.1','119.3','119.5','226.1','226.3','226.5','226.6','227.1','227.3','227.5','227.6','228.1','228.3','228.5','228.6'))){ ?>
			<script src="/backoffice/themes/swcsNewTheme/fb/demo/default/custom/components/forms/wizard/subformwizard_other_services.js" type="text/javascript"></script>
		<?php }else{ ?>
			<script src="/backoffice/themes/swcsNewTheme/fb/demo/default/custom/components/forms/wizard/subformwizard.js" type="text/javascript"></script>
		<?php } ?>
		<!--end::Page Resources -->
