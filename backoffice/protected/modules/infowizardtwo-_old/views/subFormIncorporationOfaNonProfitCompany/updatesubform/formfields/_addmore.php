...
foreach ($get_selected_field as $key => $valued) 
{
	...
	{ 
		...
		if(!in_array($valued['button_id'],$arrofIn))
		{
			...
			{	
				// table row print
				$npcdmr =  count($allData23[$fcode])/2;
				
				$Occupation = array_slice($fieldValues['UK-FCL-00117_0'], -$npcdmr);
	
				$ppo = array_slice($fieldValues['UK-FCL-00217_0'], -$npcdmr); 
				
				$ddo = array_slice($fieldValues['UK-FCL-00317_0'], -$npcdmr); 
...
$d_do = $ddo[$key];
	?>
	<input type="hidden" id="d_o<?= $i ?>" value="<?= $val ?>">
	<input type="hidden" id="d_ppo<?= $i ?>" value="<?= $d_ppo ?>">
	<input type="hidden" id="do_o<?= $i ?>" value="<?= $d_do ?>">
<?php } ?>
