...
class SubFormIncorporationOfaNonProfitCompanyController extends Controller {
        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormIncorporationOfaNonProfitCompany/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            }
if ($_POST['app_status'] == 'A') {
 ...
 $fetchsrnRecords = Yii::app()->db->createCommand("SELECT  service_id,submission_id,user_id,field_value FROM bo_new_application_submission where submission_id=$app_Sub_id")->queryRow();   
    ...
    $name_related_srn = $farr['UK-FCL-00331_0'];
    $c_name = $farr['UK-FCL-00090_0'];
...
if($farr['UK-FCL-00242_0']){
       $post1 = Yii::app()->db->createCommand("SELECT * from bo_postalcode_in_barbados where id=".$farr['UK-FCL-00242_0'])->queryRow(); 
       $pc1 = $post1['code'];
      }else{
        $pc1 = "";
      }

  $fulladdress =     @$farr['UK-FCL-00093_0'].' '.@$farr['UK-FCL-00309_0'].' '.@$farr['UK-FCL-00310_0'].' '. @$pc1.' '.@$parish1['lr_name'].' '.@$farr['UK-FCL-00096_0'] ; 
        ...
        $companyDetail->address = $fulladdress;
         $companyDetail->address_line1 = @$farr['UK-FCL-00093_0'];
         ...
         $companyDetail->postal_code_id = @$farr['UK-FCL-00242_0'];
    ...
    if ($_POST['service_id'] == '5.0') {
        $data = '';
        $path = '';
        $name = "CERTIFICATE_" . $app_Sub_id . ".pdf";
        $path = Yii::app()->basePath . "/caipo_certificate/" . $name;

        $download_certificate_call_back_url1 = CURL_URL."/backoffice/infowizardtwo/subFormPdf/SaveNewApprovalCertificate5_0/service_id/" . base64_encode($_POST['service_id']) . "/subID/" . base64_encode($_POST['app_Sub_id']) . "/dept_id/" . base64_encode($dept_id)."/utility/Utility5_0c";

        //$path = Yii::app()->basePath . "/caipo_certificate/" . $name;
        //$download_certificate_call_back_url = "http://52.172.209.7/backoffice/protected/caipo_certificate/" . $name;
...
// End Code of service provider 
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizardtwo/subFormIncorporationOfaNonProfitCompany/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizardtwo/subFormIncorporationOfaNonProfitCompany/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
...
// jitendra singh
    ...
    public function actionDownloadNewApp() {
        ...
        //echo $content; die;
        Utility5_0::generatePdfApp($content,$contentform4,$contentform9, $name);
        exit;
    }
    ...
    public function actionGenerateCertificate() {
        ...
        if (!empty($_POST)) {
            ...
            $content = DefaultUtility::dataSenetize($_POST['content']);
...
$name_res = Yii::app()->db->createCommand("SELECT submission_id 
 ...
 $company_detail = Yii::app()->db->createCommand("SELECT * 
                    ...
                    if($company_detail){
                        ...
                        if($checkformtype['UK-FCL-00044_0']==2 && $checkformtype['UK-FCL-00012_0']=="Name reservation for new Company" && $checkformtype['UK-FCL-00031_0']=='3'){
                            ...
                            if($banned_words){
                             ...
                             echo CJavaScript::jsonEncode(array('status'=>false,'user'=>$user_id,'msg'=>'Name Reservation for Non-Profit Company can only be used.'));
                        }                                   
                    }else{         
      ...
      public function actionCheckCompanyNameBySrnNo(){
        ...
        $company_detail = Yii::app()->db->createCommand("SELECT * from bo_new_application_submission WHERE submission_id='".$srn_no."'")->queryRow();
                 ...
                 echo CJavaScript::jsonEncode(array('status'=>true,'recomended_value'=>$checkSrnStatus['recomended_value'],'records'=>$checkSrnStatus,'msg'=>'success'));
                  ...
                  echo CJavaScript::jsonEncode(
                    array('status'=>'invalid',
                        ...
                        'msg1'=>'1) Please file Form 33 before filing this Form for name reservation of the Company. Further, approval of Form 33 is a pre-requisite for approval of this Form 2.',
                        'msg2' => '2) The status of Form 33 for SRN filled in above field no. 1 is not approved. Please insert a valid SRN.'
                    )
                );
            }          
        }else{
