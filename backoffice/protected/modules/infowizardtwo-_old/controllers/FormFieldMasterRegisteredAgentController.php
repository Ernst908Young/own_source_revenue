...
class FormFieldMasterRegisteredAgentController extends Controller
{
	/**
	 ...
	 */
        ...
        public static function getAllBodyOfIssuer($id){
			$id = DefaultUtility::dataSenetize($id);
    ...
    public function actionSaveAjax()
	{
		...
		if(isset($_POST['options']) && !empty($_POST['options']))
		{
			...
			foreach($_POST['options'] as $val){
				...
				$formvar_id  =DefaultUtility::dataSenetize($_POST['formvar_id']);
			    $type        =DefaultUtility::dataSenetize($_POST['type']);
	...
	/*  @ Get Master List
		...
		public  function actionGetMasterList($query){
			$query = DefaultUtility::dataSenetize($query);
			$result_data =array();
			$data =array();
	...
	/*  @ Save Master Data
		 ...
		 public function actionSaveAjaxMaster()
	    {
			...
			if(isset($_POST['master_table_id']) && !empty($_POST['master_table_id']))
			{
					...
					$formvar_id       = DefaultUtility::dataSenetize($_POST['formvar_id']);
					$type             = DefaultUtility::dataSenetize($_POST['type']);
					$master_table_id  = DefaultUtility::dataSenetize($_POST['master_table_id']);
					$val              = DefaultUtility::dataSenetize($_POST['title']);
	...
	/*  @ Deactivate Previous Saved Data
	    ...
	    {
				$formfield_id = DefaultUtility::dataSenetize($formfield_id);
			    $connection=Yii::app()->db; 
	...
	public function actionPageOption($page_id, $button_id){
		$page_id = DefaultUtility::dataSenetize($page_id);
		$button_id = DefaultUtility::dataSenetize($button_id);
		$connection=Yii::app()->db; 		
	...
	public function actionGetAddmoreData()
	{
		...
		{
			$button_id = DefaultUtility::dataSenetize($_GET['button_id']);
			$service_id = DefaultUtility::dataSenetize($_GET['service_id']);
			$add_more_button_di = DefaultUtility::dataSenetize($_GET['add_more_button_di']);
			$connection=Yii::app()->db; 	
		...
		if(!empty($_GET['app_Sub_id']) && !empty($_GET['service_id']) && !empty($_GET['form_id'])){
			$connection=Yii::app()->db;
			$app_Sub_id = DefaultUtility::dataSenetize($_GET['app_Sub_id']);
			$service_id = DefaultUtility::dataSenetize($_GET['service_id']);
			$form_id = DefaultUtility::dataSenetize($_GET['form_id']);
			$idds = DefaultUtility::dataSenetize($_GET['idds']);
			...
			{
				$approval_id = DefaultUtility::dataSenetize($approvalID['approval_id']);
			...
			if($_GET['table']=='Department'){
					...
					$command = $connection->createCommand($sql);
			 		...
			 		if($data['post_info'] && $data['approv_status'] !='V'  ||  $app_status=='PF'){
						...
						$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_registeredagent', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}		 		
						...
						$fieldValues2 = (array)json_decode($fieldValues['field_value']);
			 			...
			 			$this->renderPartial('/formBuilder/subformloggerview_registeredagent', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$fieldValues2,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'yes','selectedDept'=>$selectedDept));
			 		}
				...
				$core_department_id = DefaultUtility::dataSenetize($_GET['core_department_id']);
				...
				$processingformData = InfowizardQuestionMasterExt::alignInSequence($service_id,$form_id);
	 			...
	 			$this->renderPartial('/formBuilder/subformloggerview_registeredagent', array('aap' => $allActivePages, 'formData' => $formData,'fieldValues'=>$form_fieldValues,'processingformData'=>$processingformData,'service_id'=>$service_id,'sub_id'=>$app_Sub_id,'formCodeID'=>$form_id,'is_dept_active'=>'no','selectedDept'=>$selectedDept));
	 		}
			else{
