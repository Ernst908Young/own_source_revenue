...
$rt=$rstl['queans_mapp_id'];
$gj[$rt]=$rstl['exclude_question'];
  }
 // echo "<pre>"; print_r($gj); die;
   } else {
 $options=array();    $optionsss=array();   
}
            if(empty($apps)){
      ...
      <?php $rrr=InfowizardQuesansMappingController::getDetailOfQuesAns($apps['question_id']); 
	   $countno=1;
              ...
              <tr>  
              <td align="left" ><?php echo $count.'.'.$countno++; //echo"--"; echo $ans['queans_mapp_id'];  ?>  &nbsp;&nbsp;
			  <input type="checkbox" name="chkboxservice[<?php echo $apps['question_id'].','.$ans['queans_mapp_id']; ?>]" value="<?php echo $apps['question_id'].','.$ans['queans_mapp_id']; ?>"
			   <?php if(in_array($ans['queans_mapp_id'],$options)) { ?> checked="checked" <?php } ?>/>
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="exclude_question[]" name="exclude_question[<?php echo $apps['question_id'].','.$ans['queans_mapp_id']; ?>]" 
			  value="<?php $gf=$ans['queans_mapp_id']; if(!empty($gj[$gf])) { echo $gj[$gf]; } ?>" />
