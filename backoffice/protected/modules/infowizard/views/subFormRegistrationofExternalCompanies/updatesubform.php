                ...
                <div class="col-md-12 top_tab"><strong><?php echo $get_form_name['form_name']; ?><strong>
                            ...
                            <div class='portlet box green' id="<?php echo $btn_cls; ?>" style="display:none;">
                                <div class="portlet-body">
                                    ...
                                    $this->renderPartial('subformloggerview_registrationofexternalcompanies', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $form_data_, 'processingformData' => $processingformData));
                                    ?>  
                                </div>
                            </div>
                <?php
									...
									?>
										...
										$tablefieldname = InfowizardQuestionMasterExt::getMasterName('bo_infowizard_formvariable_master', $fd['form_field_id'], 'formchk_id', 'formvar_id');
											...
											<div class="form-group col-md-6" id="div_<?php echo $tablefieldname;?>">
												...
												<div class="col-md-12">
													<input type="text" autocomplete="off" id="<?php echo $tablefieldname; ?>" name="<?php echo $tablefieldname; ?>" class="datepicker form-control <?php echo $val_cls; ?>" labelname="<?php echo $ln; ?>" value="<?php if(!is_array(@$fieldValues[$tablefieldname]))echo @$fieldValues[$tablefieldname];?>" style="<?php echo $classEditOrNot;?>" <?php echo $calenderreq ?>>
												</div>
											</div>
										<?php } ?>
										<?php
...
$(document).ready(function () {
	...
	$('.add-more-btn').on('click', function () {
		...
		//var serviceArr = ['194.0','194.1','577.0','577.1','193.0','193.6'];
		//alert(div_id);
		$.ajax({
			type: "GET",
			dataType: 'json',
					...
					//console.log(selector);
						if ($(selector).is("input")) {
							if ($("input:radio[name='" + formchk_id + "']").attr('type') == 'radio') {
								...
								if($('#div_UK-FCL-01318_0:visible').length == 0 && serviceArr.includes(service_id))
								{
									$("input[name*='UK-FCL-01318_0']").removeClass('val');
									cls = '';
								}
							...
							if ($('#' + formchk_id).prop('multiple')) {
								...
								if(formchk_id =="UK-FCL-01300_0" && $("#UK-FCL-01300_0").val() == "Permanent" && serviceArr.includes(service_id)){	
									$("input[name*='UK-FCL-01318_0']").removeClass('val');
									cls = '';
								}
								//$("#" + formchk_id + "").select2("val", "");
