                ...
                <div class="table">
                    ...
                    <table class="table table-bordered" width="100%" id="sample_2">
                        ...
                        <tbody>
							...
							print_r($_SESSION); */
                                                        ?>
                                                        <form action="/backoffice/infowizard/payment/UnifiedPaymentResponse" name="sendPayment" method="post">
							<?php
                                                        $pay_req = array();
									...
									<td style="text-align:center;"><?php echo @$fee;?></td>
                                                                        ...
                                                                        <?php  $pay_req[$key]['user_id']= $_SESSION['RESPONSE']['user_id'];
                                                                               $pay_req[$key]['user_pass']= $userpass;
                                                                               $pay_req[$key]['int_type']= 'S';
                                                                               $pay_req[$key]['address']= $_SESSION['RESPONSE']['address'];;
                                                                               $pay_req[$key]['d_name']= @$userName;;
                                                                               $pay_req[$key]['head_desc']= $val['treasury_head_detail'];;
                                                                               $pay_req[$key]['total_Amount']= @$fee; 
                                                                               $pay_req[$key]['u_no']= $uno;;
                                                                               $pay_req[$key]['dept_name']= $val['name'];;
                                                                               $pay_req[$key]['estr']= @$fee.$val['treasury_head_detail'].@$userName.@$_SESSION['RESPONSE']['user_id'];;
                                                                        ?>
									
									
									</tr>								
							<?php } $json_pay_req = json_encode($pay_req); //echo $json_pay_req;die;?>
                                                                        <tr><td><input type="hidden" name="pay_req" value="<?php echo $json_pay_req;?>" />
                                                                                <input type="submit" name="Pay Now" value="Pay Now" class="btn btn-primary"></td></tr>
                                                                        </form>
                        </tbody>
                    </table>                   
                </div>
