...
//		$res_caf = $command->queryAll();
        ...
        if (isset($_GET['id']) && $_GET['id'] > 0) {
            ...
            $sql_s = "SELECT * FROM bo_information_wizard_service_master as sm  
				  ...
				  WHERE issuerby_id='$id' AND sp.is_active='Y' ORDER BY sp.service_id ASC";
            $connection = Yii::app()->db;
...
// jitendra singh
    ...
    public function actionDownloadNewApp() {
        ...
        $content = $this->renderPartial('app_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['application_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
    ...
    function actionSaveAllDocuments(){
		...
		$user_id = $_SESSION['RESPONSE']['user_id'];
		$sqlCheckPaymentPaidorNot = array();
		$detpStr  = array();
		$detpStr2  = array();
		
		$sqlCheckPaymentPaidorNot = Yii::app()->db->createCommand("SELECT * FROM bo_unified_payment_logs WHERE service_id = '$service_id' AND submission_id = '$app_id' AND user_id= '$user_id'")->queryAll();
		
		$paymentFlag = 0;
		$detpStr2 = array();
		foreach($sqlCheckPaymentPaidorNot as $key=>$val){
			$postInfo = json_decode($val['post_info'],true);
			if(isset($postInfo['detpstr'])){
				$detpStr = explode("|",$postInfo['detpstr']);
				$detpStr2 = explode('=',$detpStr[2]);
			}
			
			if(isset($detpStr2[1]) && !empty($detpStr2[1]=='S'))
			{
				//Payment Done
				$paymentFlag=1;
			}
		}
		if(!empty($sqlCheckFeeAplicable['fee_detail']) && $paymentFlag!=1)
		{		
			$pdsatus = 1;
		}
		...
		if($pdsatus==0){			
			$this->ForwardToDepartment($app_id, $sno, $service_id,'District');		
			echo "1";
			die;			
		}else{
			echo "0";
