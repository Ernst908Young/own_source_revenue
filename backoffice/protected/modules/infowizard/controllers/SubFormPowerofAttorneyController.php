...
// jitendra singh
    ...
    public function actionDownloadNewApp() {
        ...
        //echo $content; die;
        Utility15_0::generatePdfApp($content,$content1, $name);
        exit;
    }
	...
	function actionSaveAllDocuments(){
		...
		extract($_GET);
        $user_id = $_SESSION['RESPONSE']['user_id'];
        $this->getExistingCafDocuments($srn,$service_id,$dept_id,$user_id);     
        
        
         
         $pdsatus = 1;   $stat = 'PD';   // payment due
        ...
        $checkalreadycheck = Yii::app()->db->createCommand("SELECT * FROM bo_declaration_metadata WHERE service_id=$service_id AND application_id=$srn")->queryRow();
        if($checkalreadycheck){

        }else{
            $dec_metadata = new Declarationmetadata;
            $dec_metadata->service_id = $service_id;
            $dec_metadata->application_id = $srn;
             $dec_metadata->save();
        }
