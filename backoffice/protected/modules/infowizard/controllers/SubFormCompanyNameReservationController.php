        ...
        if (isset($allRes) && !empty($allRes)) {
          ...
          /*  echo "<pre>";
            ...
            if ($_POST['app_status'] == 'H') {
                ...
                $reverted_call_back_url = "/backoffice/infowizard/subFormCompanyNameReservation/updateSubForm/service_id/" . $_POST['service_id'] . "/pageID/1/subID/" . $_POST['app_Sub_id'] . "/formCodeID/1";
            }
    ...
    public function actionSaveData() {
        ...
        if (!empty($_POST) && !Yii::app()->request->isAjaxRequest) {
			...
			{   			
                ...
                $reverted_call_back_url = "/backoffice/infowizard/subFormCompanyNameReservation/updateSubForm/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
				$print_app_call_back_url ="/backoffice/infowizard/subFormCompanyNameReservation/downloadNewApp/service_id/".$_POST['service_id']."/pageID/1/subID/".$application_id."/formCodeID/1";
                ...
                Yii::app()->user->setFlash('success', "Your form has been submitted,Please upload document");
...
// jitendra singh
	...
	public function actionSaveUpdateSubForm() {
        ...
        if(isset($_POST) && !empty($_POST)) {
			...
			if($modellog->save()) {
				...
				//Yii::app()->user->setFlash('success', "Your application form has been updated.");
    ...
    public function actionDownloadNewApp() {
        ...
        $content = $this->renderPartial('app_pdf_view', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        $name = "CAPIO-Application_Form_" . time() . ".pdf";

        //echo $content; die;
        UtilityA2_0::generatePdfApp($content, $name);
        exit;
    }
	...
	{
	//52.172.216.52/backoffice/infowizard/subFormCompanyNameReservation/revokeNameReservation
...
WHERE bo_new_application_submission.application_status='A' AND bo_new_application_submission.service_id IN('4.0','5.0','6.0','7.0','8.0','9.0','10.0') group by bo_new_application_submission.submission_id ORDER BY bo_new_application_submission.submission_id DESC")->queryAll();
		...
		if($_POST){
			...
			if($modelRevoke->save()) 
			{				
				$userDetails=Yii::app()->db->createCommand("SELECT sso_users.email,sso_users.iuid,sso_users.user_id,sso_profiles.first_name,sso_profiles.last_name,sso_profiles.mobile_number,bo_new_application_submission.submission_id,bo_new_application_submission.application_status,bo_new_application_submission.application_updated_date_time
				FROM bo_new_application_submission 
				INNER JOIN sso_users ON sso_users.user_id = bo_new_application_submission.user_id
				INNER JOIN sso_profiles ON sso_profiles.user_id = sso_users.user_id
				WHERE bo_new_application_submission.submission_id='$revoke_app_id'")->queryRow();
				...
				Yii::app()->user->setFlash('success', "Notification sent successfully for Application Id : $revoke_app_id.");
			}else{
	...
	function actionAssignNewName()
	{
		...
		if(isset($_POST)){
			...
			if($ReservedNamemodel->save())
			{
				...
				$this->redirect('/backoffice/infowizard/subFormCompanyNameReservation/revokeNameReservation');
