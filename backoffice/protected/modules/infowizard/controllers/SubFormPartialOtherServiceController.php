<?php
class SubFormPartialOtherServiceController extends Controller {
	...
	public function actionSaveDataPartial() {
        if (!empty($_POST)) {
			...
			if(isset($_POST['submission_id']) && !empty($_POST['submission_id']) && empty($_POST['stype'])){				
			...
			if(!isset($_POST['submission_id']) && empty($_POST['submission_id']) || !empty($_POST['stype'])){				
				...
				$model->parent_sub_id = @$_POST['stype'];
			}
 ...
 */
            if ($model->save()) {
				...
				$hiddenfield = true ;
				if(isset($_POST['submission_id']) && empty($_POST['stype'])){		
					$hiddenfield = false ;
