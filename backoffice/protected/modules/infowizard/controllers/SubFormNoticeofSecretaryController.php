...
// jitendra singh
    ...
    public static function SavePDF($url, $path) {
      ...
      public function actionCheckCompanyNameBySrnNo(){
        ...
        $company_detail = Yii::app()->db->createCommand("SELECT * from bo_company_details WHERE srn_no='".$srn_no."' AND company_type = 'Incorporation Company' AND is_active = 1 AND user_id ='".$user_id."' ")->queryRow();

        if($company_detail){

           
          echo CJavaScript::jsonEncode(array('status'=>true,'company_name'=>$company_detail['company_name'],'records'=>$company_detail,'msg'=>'success'));
