...
// jitendra singh
    ...
    static function alignInSequence($serviceID = null, $formCodeID = null) {
       ...
       /* return print_r($allActivePagesASC);
        die();*/
        ...
        $resultarray = array();
           $checkfieldsdupli = array();
           foreach ($allFormFieldDataAsPerASCPrefrencewithPageCategoryAndFormFieldCombination as $key => $value) {
               if(array_key_exists($value['form_field_id'], $checkfieldsdupli)){

                    }else{
                         $resultarray[] = $value;
                         $checkfieldsdupli[$value['form_field_id']]=$value;
                    }
           }
        return $resultarray;
    }
    ...
    public function actionDownloadNewApp() {
        ...
        $content1 = $this->renderPartial('company_name_pdf_view_1', array('aap' => $allActivePages, 'formData' => $formData, 'fieldValues' => $fieldValues2, 'app_id' => $fieldValues['submission_id'], 'app_status' => $fieldValues['application_status'], 'application_created_date' => $fieldValues['application_created_date']), true);
        ...
        //echo $content; die;
        Utility16_0::generatePdfApp($content,$content1, $name);
        exit;
    }
	...
	public function actionGetSocityNameByregno(){
        ...
        if($s_detail){         
                  echo CJavaScript::jsonEncode(
                    array('status'=>true,
                     		'sname'=>$s_detail['name'],
				'address'=>$s_detail['address'],
                     		'rdate'=>$s_detail['reg_date'] ? date('d/m/Y',strtotime($s_detail['reg_date'])) : NULL,
                     		'msg'=>'success'
                    )
                );
