...
class LandownerConnect extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, land_title, district_id, village, keshra_khatian, type_of_land, distance_highway, name_highway, distance_airport, name_airport, distance_railway, name_railway, area_sqmt, existing_loan, comment, status, created_date', 'required'),
			array('is_sale, is_lease, district_id', 'numerical', 'integerOnly'=>true),
			array('user_id', 'length', 'max'=>10),
			array('land_title', 'length', 'max'=>255),
			array('existing_loan, status', 'length', 'max'=>1),
			array('modified_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, land_title, is_sale, is_lease, district_id, village, keshra_khatian, type_of_land, distance_highway, name_highway, distance_airport, name_airport, distance_railway, name_railway, area_sqmt, existing_loan, comment, status, created_date, modified_date', 'safe', 'on'=>'search'),
		);
	}
